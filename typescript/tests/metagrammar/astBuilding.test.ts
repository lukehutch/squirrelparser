...
describe('MetaGrammar - AST Building', () => {
  ...
  test('AST flattens combinator nodes', () => {
    ...
    const rules = MetaGrammar.parseGrammar(grammar);
    const parser = new Parser(rules, 'aaabbb');
    const [ast, _] = parser.parseToAST('Main');
  ...
  test('AST text extraction', () => {
    ...
    const rules = MetaGrammar.parseGrammar(grammar);
    const parser = new Parser(rules, '123');
    const [ast, _] = parser.parseToAST('Number');

    expect(ast).not.toBeNull();
    expect(ast!.text).toBe('123');
  });
  ...
  test('AST pretty printing', () => {
    ...
    expect(ast).not.toBeNull();
    const prettyString = ast!.toPrettyString();
