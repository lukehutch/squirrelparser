...
describe('Comprehensive Fix Tests', () => {
  ...
  test('FIX3-01-ref-transparency-lr-reexpansion', () => {
    ...
    const [ok, err, _] = parse(grammar, 'n+Xn;');
    expect(ok).toBe(true);
    expect(err).toBe(1);
  });
  ...
  test('FIX4-01-single-char-skip-junk', () => {
    ...
    const [ok, err, skip] = parse(grammar, 'aXXbc');
    expect(ok).toBe(true);
    expect(err).toBe(1);
    expect(skip.some(s => s.includes('XX'))).toBe(true);
  });
  ...
  test('FIX4-02-single-char-no-skip-containing-terminal', () => {
    ...
    const [ok, _, __] = parse(grammar, 'aXbYc');
  ...
  test('FIX4-03-multi-char-atomic-terminal', () => {
    ...
    const [ok, err, _] = parse(grammar, 'n+Xn+n');
    expect(ok).toBe(true);
    expect(err).toBe(1);
  });
  ...
  test('FIX4-04-multi-char-exact-skip-ok', () => {
    ...
    const [ok, err, _] = parse(grammar, 'abXYcd');
    expect(ok).toBe(true);
    expect(err).toBe(1);
  });
  ...
  test('FIX5-02-skip-pure-junk-ok', () => {
    ...
    const grammar: Record<string, Clause> = {
      S: new Seq([new Str('+'), new Str('n')]),
    };
    const [ok, err, skip] = parse(grammar, '+XXn');
    expect(ok).toBe(true);
    expect(err).toBe(1);
    expect(skip.some(s => s.includes('XX'))).toBe(true);
  });
  ...
  test('COMBINED-01-lr-with-skip-and-delete', () => {
    ...
    const [ok, _, __] = parse(grammar, 'n+Xn+Yn');
    expect(ok).toBe(true);
  });
  ...
  test('COMBINED-02-first-prefers-longer-with-errors', () => {
    ...
    const grammar: Record<string, Clause> = {
      S: new First([
        new Seq([new Str('a'), new Str('b'), new Str('c')]), // 3 chars
        new Str('a'), // 1 char
      ]),
    };
    const [ok, err, _] = parse(grammar, 'aXbc');
    expect(ok).toBe(true);
    expect(err).toBe(1);
  ...
  test('COMBINED-03-nested-seq-recovery', () => {
    ...
    const grammar: Record<string, Clause> = {
      S: new Seq([new Ref('A'), new Str(';'), new Ref('B')]),
      A: new Seq([new Str('a'), new Str('x')]),
      B: new Seq([new Str('b'), new Str('y')]),
    };
    const [ok, err, _] = parse(grammar, 'aXx;bYy');
    expect(ok).toBe(true);
    expect(err).toBe(2);
  });
});
