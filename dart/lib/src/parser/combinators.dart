...
class Seq extends Clause {
  ...
  const Seq(this.subClauses, {super.transparent});
  ...
  MatchResult match(Parser parser, int pos, {Clause? bound}) {
    ...
    while (i < subClauses.length) {
      ...
      final effectiveBound =
          (parser.inRecoveryPhase && next != null) ? next : bound;
          ...
          if (recovery != null) {
            ...
            if (inputSkip > 0) {
              children.add(SyntaxError(
                pos: curr,
                len: inputSkip,
                skipped: parser.input.substring(curr, curr + inputSkip),
              ));
            }
  ...
  (int, int, MatchResult?)? _recover(Parser parser, int curr, int i) {
    ...
    for (int inputSkip = 0; inputSkip < maxScan; inputSkip++) {
      ...
      for (int grammarSkip = 0; grammarSkip < maxGrammar; grammarSkip++) {
        ...
        if (failedClause is Str &&
            failedClause.text.length == 1 &&
            inputSkip > 1) {
...
class First extends Clause {
  final List<Clause> subClauses;
  const First(this.subClauses, {super.transparent});
    ...
    for (int i = 0; i < subClauses.length; i++) {
      ...
      if (!result.isMismatch) {
        if (parser.inRecoveryPhase && i == 0 && countErrors(result) > 0) {
          ...
          var bestErrors = countErrors(result);
          ...
          for (int j = 1; j < subClauses.length; j++) {
            ...
            if (!altResult.isMismatch) {
              ...
              final altErrors = countErrors(altResult);
              ...
              if (bestErrorRate >= errorRateThreshold &&
                  altErrorRate < errorRateThreshold) {
                shouldSwitch = true;
          ...
          return Match(this, 0, 0,
              subClauseMatches: [bestResult],
              isComplete: bestResult.isComplete);
        }

        return Match(this, 0, 0,
            subClauseMatches: [result], isComplete: result.isComplete);
      }
    }
    return mismatch;
  }
...
class Repetition extends Clause {
  final Clause subClause;
  final bool requireOne;

  const Repetition(this.subClause,
      {required this.requireOne, super.transparent});
  ...
  MatchResult match(Parser parser, int pos, {Clause? bound}) {
        ...
        if (parser.inRecoveryPhase) {
          ...
          if (recovery != null) {
            ...
            children.add(SyntaxError(
              pos: curr,
              len: skip,
              skipped: parser.input.substring(curr, curr + skip),
            ));
            hasRecovered = true;
            if (probe != null) {
              children.add(probe);
    ...
    return Match(this, 0, 0,
        ...
        isComplete: !incomplete && _allComplete(children));
  }
...
class Optional extends Clause {
  final Clause subClause;
  const Optional(this.subClause, {super.transparent});
...
class Ref extends Clause {
  ...
  const Ref(this.ruleName, {super.transparent});
...
class NotFollowedBy extends Clause {
  final Clause subClause;
  const NotFollowedBy(this.subClause, {super.transparent});
...
class FollowedBy extends Clause {
  final Clause subClause;
  const FollowedBy(this.subClause, {super.transparent});
