...
void main() {
  ...
  group('Pathological Grammars', () {
    test('PATH-01-deeply-nested-first', () {
      ...
      Clause buildDeepFirst(int depth, String terminal) {
        if (depth == 0) return Str(terminal);
        return First([Str('x'), buildDeepFirst(depth - 1, terminal)]);
      }
    ...
    test('PATH-02-deeply-nested-seq', () {
      ...
      Clause buildDeepSeq(int depth) {
        ...
        return Seq([Str('a'), buildDeepSeq(depth - 1)]);
      }

      final grammar = <String, Clause>{
        'S': Seq([buildDeepSeq(20), Str('end')]),
      };
    ...
    test('PATH-03-many-alternatives', () {
      ...
      final alts = List.generate(50, (i) => Str('opt$i'));
      final grammar = <String, Clause>{
    ...
    test('PATH-04-wide-seq', () {
      ...
      final elems =
          List.generate(30, (i) => Str(String.fromCharCode(97 + (i % 26))));
      final grammar = <String, Clause>{
        'S': Seq(elems),
      };
