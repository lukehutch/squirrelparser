...
void main() {
  group('Multi-level LR Recovery Tests', () {
    // Standard precedence grammar with multi-level left recursion
    final precedenceGrammar = <String, Clause>{
      'E': First([
        Seq([Ref('E'), Str('+'), Ref('T')]),
        Ref('T')
      ]),
    ...
    test('MULTI-LR-01-error-at-F-level-after-star', () {
      ...
      final r = testParse(precedenceGrammar, 'n+n*Xn', 'E');
      expect(r.$1, isTrue, reason: 'should parse with recovery');
    ...
    test('MULTI-LR-02-error-at-T-level-after-plus', () {
      ...
      final r = testParse(precedenceGrammar, 'n+Xn*n', 'E');
      expect(r.$1, isTrue, reason: 'should parse with recovery');
      expect(r.$2, greaterThanOrEqualTo(1),
          reason: 'should have at least 1 error');
    });
    ...
    test('MULTI-LR-03-nested-error-in-parens', () {
      ...
      expect(r.$2, greaterThanOrEqualTo(1), reason: 'should have errors');
    });

    // Simpler two-level grammar to isolate the issue
    final twoLevelGrammar = <String, Clause>{
      'A': First([
        Seq([Ref('A'), Str('+'), Ref('B')]),
        Ref('B')
      ]),
    ...
    test('MULTI-LR-04-two-level', () {
      ...
      final r = testParse(twoLevelGrammar, 'x+x-Yx', 'A');
      expect(r.$1, isTrue, reason: 'should parse with recovery');
      expect(r.$2, greaterThanOrEqualTo(1), reason: 'should have errors');
    });
    ...
    test('MULTI-LR-05-three-levels', () {
      ...
      final threeLevelGrammar = <String, Clause>{
        'A': First([
          Seq([Ref('A'), Str('+'), Ref('B')]),
          Ref('B')
        ]),
      ...
      final r = testParse(threeLevelGrammar, 'x+x*x-Yx', 'A');
      expect(r.$1, isTrue, reason: 'should parse with recovery');
      expect(r.$2, greaterThanOrEqualTo(1), reason: 'should have errors');
    });
  });

  group('Single-level LR Recovery (Working Cases)', () {
    ...
    test('SINGLE-LR-01-basic', () {
      ...
      expect(r.$1, isTrue, reason: 'basic LR recovery should work');
      expect(r.$2, equals(1),
          reason: 'single-level LR should have exact error count');
    });
    ...
    test('SINGLE-LR-02-multiple-expansions', () {
      ...
      final r = testParse(grammar, 'n+Xn+n', 'E');
      expect(r.$1, isTrue);
      expect(r.$2, equals(1), reason: 'single-level LR should skip exactly X');
    });
    ...
    test('SINGLE-LR-03-multiple-errors', () {
      ...
      final r = testParse(grammar, 'n+Xn+Yn', 'E');
      expect(r.$1, isTrue);
      expect(r.$2, equals(2), reason: 'should have 2 errors');
    });
  });

  group('LR_PENDING Fix Verification', () {
    ...
    test('LR-PENDING-01-no-spurious-recovery', () {
      ...
      final r = testParse(grammar, 'n+n*Xn', 'E');
      expect(r.$1, isTrue);
      // LR_PENDING prevents 4 errors, but Ref masking still causes 2
      expect(r.$2, lessThanOrEqualTo(3),
          reason: 'LR_PENDING should prevent excessive errors');
    });
  });
}
