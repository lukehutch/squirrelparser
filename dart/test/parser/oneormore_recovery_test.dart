...
void main() {
  group('OneOrMore First-Iteration Recovery Tests', () {
    test('OM-01-first-clean', () {
      ...
      final (ok, err, _) = testParse({'S': OneOrMore(Str('ab'))}, 'ababab');
    ...
    test('OM-04-skip-multiple-to-first', () {
      ...
      final (ok, err, skip) = testParse({'S': OneOrMore(Str('ab'))}, 'XXXXXab');
      expect(ok, isTrue, reason: 'should succeed');
    ...
    test('OM-07-vs-zeoormore-semantics', () {
      ...
      final zrEmpty = testParse({'S': ZeroOrMore(Str('ab'))}, '');
    ...
    test('OM-08-long-skip-performance', () {
      ...
      final (ok, err, skip) = testParse({'S': OneOrMore(Str('ab'))}, input);
    ...
    test('OM-09-exhaustive-search-no-match', () {
      ...
      final (ok, _, _) = testParse({'S': OneOrMore(Str('ab'))}, input);
      expect(ok, isFalse,
    ...
    test('OM-12-multi-char-terminal-first', () {
      ...
      final (ok, err, skip) =
          testParse({'S': OneOrMore(Str('hello'))}, 'XXXhellohello');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(1), reason: 'should have 1 error');
