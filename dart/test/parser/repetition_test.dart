...
void main() {
  test('R01-between', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('ab'))}, 'abXXab');
  ...
  test('R02-multi', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('ab'))}, 'abXabYab');
    expect(ok, isTrue, reason: 'should succeed');
  ...
  test('R05-before first', () {
    ...
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('ab'))}, 'XXab');
  ...
  test('R06-trailing captured', () {
    ...
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('ab'))}, 'ababXX');
  ...
  test('R07-single', () {
    final (ok, err, _) = testParse({'S': OneOrMore(Str('ab'))}, 'ab');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(0), reason: 'should have 0 errors');
  });

  test('R08-ZeroOrMore empty', () {
    final (ok, err, _) = testParse({'S': ZeroOrMore(Str('ab'))}, '');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(0), reason: 'should have 0 errors');
  });
  ...
  test('R10-long clean', () {
    final (ok, err, _) = testParse({'S': OneOrMore(Str('x'))}, 'x' * 100);
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(0), reason: 'should have 0 errors');
  });
  ...
  test('R11-long err', () {
    final (ok, err, skip) =
        testParse({'S': OneOrMore(Str('x'))}, '${'x' * 50}Z${'x' * 49}');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
  ...
  test('R14-very long err', () {
    final (ok, err, _) =
        testParse({'S': OneOrMore(Str('ab'))}, '${'ab' * 250}ZZ${'ab' * 249}');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
  });
