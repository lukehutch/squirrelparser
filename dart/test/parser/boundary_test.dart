...
void main() {
  group('Empty and Boundary Conditions', () {
    ...
    test('E06-Ref empty', () {
      final (ok, _, _) = testParse({'S': Ref('A'), 'A': Str('x')}, '');
      expect(ok, isFalse, reason: 'should fail');
    });

    test('E07-Single char match', () {
      final (ok, err, _) = testParse({'S': Str('x')}, 'x');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E08-Single char mismatch', () {
      final (ok, _, _) = testParse({'S': Str('x')}, 'y');
      expect(ok, isFalse, reason: 'should fail');
    });

    test('E09-ZeroOrMore single', () {
      final (ok, err, _) = testParse({'S': ZeroOrMore(Str('x'))}, 'x');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E10-OneOrMore single', () {
      final (ok, err, _) = testParse({'S': OneOrMore(Str('x'))}, 'x');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E11-Optional match', () {
      final (ok, err, _) = testParse({'S': Optional(Str('x'))}, 'x');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E12-Two chars match', () {
      final (ok, err, _) = testParse({'S': Str('xy')}, 'xy');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E13-Two chars partial', () {
      final (ok, _, _) = testParse({'S': Str('xy')}, 'x');
      expect(ok, isFalse, reason: 'should fail');
    });

    test('E14-CharRange match', () {
      final (ok, err, _) = testParse({'S': CharRange('a', 'z')}, 'm');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E15-CharRange boundary low', () {
      final (ok, err, _) = testParse({'S': CharRange('a', 'z')}, 'a');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E16-CharRange boundary high', () {
      final (ok, err, _) = testParse({'S': CharRange('a', 'z')}, 'z');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E17-CharRange fail low', () {
      final (ok, _, _) = testParse({'S': CharRange('b', 'y')}, 'a');
      expect(ok, isFalse, reason: 'should fail');
    });

    test('E18-CharRange fail high', () {
      final (ok, _, _) = testParse({'S': CharRange('b', 'y')}, 'z');
      expect(ok, isFalse, reason: 'should fail');
    });

    test('E19-AnyChar match', () {
      final (ok, err, _) = testParse({'S': AnyChar()}, 'x');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E20-AnyChar empty', () {
      final (ok, _, _) = testParse({'S': AnyChar()}, '');
      expect(ok, isFalse, reason: 'should fail');
    });
    ...
    test('E24-ZeroOrMore multi', () {
      final (ok, err, _) = testParse({'S': ZeroOrMore(Str('x'))}, 'xxx');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E25-OneOrMore multi', () {
      final (ok, err, _) = testParse({'S': OneOrMore(Str('x'))}, 'xxx');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E26-Long string match', () {
      final (ok, err, _) = testParse({'S': Str('abcdefghij')}, 'abcdefghij');
      expect(ok, isTrue, reason: 'should succeed');
      expect(err, equals(0), reason: 'should have 0 errors');
    });

    test('E27-Long string partial', () {
      final (ok, _, _) = testParse({'S': Str('abcdefghij')}, 'abcdefghi');
      expect(ok, isFalse, reason: 'should fail');
    });
  });
}
