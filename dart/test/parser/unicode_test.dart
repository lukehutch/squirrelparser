...
void main() {
  test('U01-Greek', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('α'))}, 'αβα');
  ...
  test('U02-Chinese', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('中'))}, '中文中');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
  ...
  test('U03-Arabic clean', () {
    final (ok, err, _) = testParse({'S': OneOrMore(Str('م'))}, 'ممم');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(0), reason: 'should have 0 errors');
  });

  test('U04-newline', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('x'))}, 'x\nx');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
  ...
  test('U06-space', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('x'))}, 'x x');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
  ...
  test('U08-Japanese', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('日'))}, '日本日');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
  ...
  test('U09-Korean', () {
    final (ok, err, skip) = testParse({'S': OneOrMore(Str('한'))}, '한글한');
    expect(ok, isTrue, reason: 'should succeed');
    expect(err, equals(1), reason: 'should have 1 error');
