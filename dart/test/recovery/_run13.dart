// Scratch runner: the battery against the candidate engine in
// experiments/recovery/_c13.dart. Same output contract as _score1.dart.
import 'package:squirrel_parser/squirrel_parser.dart';

import '../../experiments/recovery/_c13.dart' as cand;
import 'astdiff.dart';
...
void main(List<String> argv) {
  ...
  for (final c in corpora) {
    made[c.name] = (String s) =>
        cand.Squirrel(rules: rulesOf[c.name]!, topRuleName: c.top).recover(s);
  }
  ...
  for (var i = 0; i < cases.length; i++) {
    ...
    catN[k.category] ??= 0;
    catN[k.category] = catN[k.category]! + 1;
    if (dump && s.score < 1.0) {
      rows.add((
        s.score,
