// Scratch: fast smoke for m46 against m45 -- agreement on cost, tree and cover.
...
void main() {
  ...
  var costDiff = 0, shapeDiff = 0, shapeOk45 = 0, shapeOk43 = 0, coverBad = 0;
  var spanDiff = 0;
  final hist45 = <int, int>{}, hist43 = <int, int>{};
  for (final s in cases) {
    final e43 = g45.SuperDot3(rules: rules, topRuleName: 'JSON');
    final e45 = g46.SuperDot3(rules: rules, topRuleName: 'JSON');
    late int c43, c45, n43, n45;
    late MatchResult r43, r45;
    try {
    ...
    try {
      final b = e45.recover(s);
      c45 = e45.lastCost;
      r45 = b.root;
      n45 = b.errorSpans.length + b.missing.length;
    } catch (e) {
    ...
    if (c43 != c45) {
      costDiff++;
      if (costDiff <= 8) print('COST  "$s"  m45=$c43 m46=$c45');
    }
    if (n43 != n45) {
      spanDiff++;
      if (spanDiff <= 8) print('SPANS "$s"  m45=$n43 m46=$n45');
    }
    hist43[c43] = (hist43[c43] ?? 0) + 1;
    hist45[c45] = (hist45[c45] ?? 0) + 1;
    if (!covers(r45, s.length)) {
      coverBad++;
      if (coverBad <= 4) print('COVER "$s"');
    }
    final t43 = treeShape(r43), t45 = treeShape(r45);
    if (t43 == want) shapeOk43++;
    if (t45 == want) shapeOk45++;
    if (t43 != t45) {
      shapeDiff++;
  ...
  print('cases=${cases.length} costDiff=$costDiff shapeDiff=$shapeDiff '
      ...
      'shapeOk43=$shapeOk43 shapeOk45=$shapeOk45');
  print('hist43=$hist43');
  print('hist45=$hist45');
}
