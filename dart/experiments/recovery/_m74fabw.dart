...
class SuperDot3 {
    ...
    switch (node) {
      case _Term(:final editable, :final demands):
        ...
        if (!m.isMismatch && (m.len == 0 || _has(c, _input.codeUnitAt(pos)))) {
          _put(f, _key(pos + m.len, m.len == 0 ? c : _free), 0, _cleanRegret(m),
              _wPure);
        }
        ...
        if (silent || _permits(c, emits)) {
          ...
          _put(f, _key(pos, owed), 1, _widthOf(node.orig), _wFab); // FAB
        }
        return _finish(f);
  ...
  /// held the ladder at 2048 -- an explicit walk keeps the witness off the VM
  /// stack, and the chase already gave up its own recursion.
  ///
  /// The walk can go top-down, which a rebuild normally cannot, because `Match`
  /// re-derives its span from its children only when they are already there
  /// (match_result.dart:40) and holds the list it was handed. So a node built
  /// with an EMPTY list keeps the span this pass computed, and its children can
  /// be poured into that same list afterwards -- one stack, no second pass, and
  /// the parent's bounds come from `_xOf` rather than from the hull of the kids.
  MatchResult _reindex(MatchResult root) {
    MatchResult at(MatchResult m, List<MatchResult> kids) {
      final s = _xOf(m.pos);
      return Match(m.clause, s, _xOf(m.pos + m.len) - s,
          subClauseMatches: kids);
    }

    final kids = <MatchResult>[];
    final out = at(root, kids);
    final st = <(MatchResult, List<MatchResult>)>[(root, kids)];
    while (st.isNotEmpty) {
      final (m, into) = st.removeLast();
      for (final k in m.subClauseMatches) {
        final ks = <MatchResult>[];
        into.add(at(k, ks));
        if (k.subClauseMatches.isNotEmpty) st.add((k, ks));
      }
    }
    return out;
  }

  /// The cheapest text a leaf accepts. A one-character class spells itself with
  /// the low end of its first range; anything else is a literal or is empty,
  /// and a null class never matches the list pattern.
  ...
  bool _certified(int cost) {
    ...
    _root = null;
    lastVerified = false;
    lastRepairedDoc = _input;
    if (cost < 0) return true;
    // A clean parse is its own witness AND its own tree, so it goes in the same
    // slot: with no edits behind it, the report below writes itself as empty.
