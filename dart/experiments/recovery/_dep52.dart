...
class _Cell {
  ...
  /// PROBE: slot -> cell resolved on the previous relaxation, to test invariance.
  final Map<int, _Cell> probeDeps = {};
}
...
class SuperDot3 {
  ...
  /// function twice.
  int probeReads = 0, probeStable = 0, probeViolations = 0;
  int _probeSlot = -1;
  ...
  List<int> _read(_Cell? from, _Node node, int pos, int budget, int c) {
    ...
    if (from != null && _probeSlot >= 0) {
      probeReads++;
      final was = from.probeDeps[_probeSlot];
      if (was == null) {
        from.probeDeps[_probeSlot] = cell;
      } else if (identical(was, cell)) {
        probeStable++;
      } else {
        probeViolations++;
      }
  ...
  void _compute(_Cell cell) {
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        if (alts.length == 1) {
          _probeSlot = 0;
          final r = _merge(_read(cell, alts[0], pos, budget, c));
          _probeSlot = -1;
          return r;
        }
