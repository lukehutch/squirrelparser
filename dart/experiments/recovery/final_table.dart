...
import 'm73.dart' as g73;
import 'm74.dart' as g74;
import 'm75.dart' as g75;
import 'm76.dart' as g76;
import 'm77.dart' as g77;
import 'm78.dart' as g78;
...
const elegNotes = <String, (int, String)>{
  ...
  'm73': (9, 'I27+I28 ON m62\'s CORE: THE SMALLER ENGINE WAS THE OLDER ONE. m72 answers the true PEG language in 979 lines by shaving m71; counting WHERE the 190 lines over m62 sit says to build the other way round -- reconstruction and driver hold 122 of them, and m62 already searches the identical space (same cells to 1.0000, 880711 entries against 880362) with the obligation lattice, _meet and the same key packing already in it. So m73 is m62 verbatim plus I27\'s guards and I28\'s relax-then-certify and nothing else: 839 lines against 979, answers byte-identical to m71 (0 cost differences over 531 gate inputs, 0 over the 2387 brute-force truths), conformance 5/5, wrong on 98 of 2387 where m62 is wrong on 324. What it does NOT fix is the two columns m62 leads, and they turn out to be one defect seen twice: the certificate is still build-emit-reparse, so the battery pays 1.18x (298.4 against 253.3, medians of 22 one-engine-per-process runs), and that reconstruction is a native recursion, so LRmax/RRmax fall to 1024/2048 -- a parse tree is as deep as its input on the ladder grammars. Splitting the certificate prices the two halves separately, all three arms on ONE clock at n=21 because subtracting arms that never shared a clock is how the first attempt got this wrong: the whole certificate is 43.6 ms of the battery (14.5%, 21/21 paired rounds), of which _build is 24.3 (19/21) and emit-plus-fresh-parse 19.3 (21/21). So the larger half, 56%, is the very pass whose recursion costs the depth -- and the smaller half is NOT free, which is the correction: an earlier figure of 27.6 against 7.8 came from subtracting a no-verify arm in one run against a no-certificate arm in another. m72\'s split, also single-clock, reads the other way round (17.4 build against 21.7 emit and re-parse) -- the two _builds are different functions, m72\'s carrying _wholesale and m73\'s being m59\'s verbatim, so what survives both is that each half is about 20 ms and neither is free Kept as the row that proves the reconstruction, not the conformance, was the whole price -- which is what m74 then deletes'),
  'm75': (10, 'I32 PLUS THE (INVENTION, LOSS) TIE-BREAK: THE REPAIR IS SCAFFOLDING, AND THE TREE IS OVER THE INPUT. m74 treats the repaired STRING as the deliverable -- splice the edits, parse the result, keep THAT tree, map its positions back -- so every fabricated character becomes a node the document does not support. Measured rather than argued: put each leaf of the finished tree back to the pure parser at its own position over the UNTOUCHED input and count the ones that do not read their own span, and m74 places 535 such nodes across the 519-mutant battery where m75 places 0. Asked to repair [2,33,ture], m74 returns a boolean the author never wrote with ture gone. I32 keeps the search bit-for-bit and reads the witness out differently: input the grammar cannot use becomes a SyntaxError span sitting in the tree at the structural position that failed, grammar the input cannot fill becomes a ZERO-WIDTH span at that position with the demanded symbol NOT written -- a wide class cannot say which symbol, and choosing a representative would be serialization rather than evidence -- and everything else is the oracle\'s own subtree taken verbatim. _xOf and _reindex go with the remapping. The contract is checkable and was checked: every span is an interval of the real input, 519/519 tile it (children ordered, disjoint, inside the parent), 519/519 cover it (no input character dropped on the floor), 519/519 certified. It is also what SkipResult\'s docstring at skip_recovery.dart:91-96 has always promised and no engine delivered -- unparseable regions as SyntaxError CHILDREN of the tree. THE TIE-BREAK, DERIVED RATHER THAN TUNED: regret charged a fabrication the FLAT whole-Unicode width no matter what it wrote. Replace the one secondary slot with (invention, loss) lexicographic. Invention is the bits a repair asserts that the input did not justify, log2|C| per fabricated character and ZERO when the grammar forced it, which _width already computed in millibits and never applied to fabrication; loss is input characters not preserved. THEY ARE ORDERED, NOT SUMMED, and that is the point: there is no exchange rate between a bit and a character, and inventing one would be the arbitrary constant this project forbids. The order is forced -- an invented character is unfalsifiable, since nothing downstream can tell it from real data, while a destroyed character is still in the input the caller holds. Invention corrupts, loss only omits. It is FREE: relaxations are bit-identical at 1222729 with 0 of 519 inputs differing, so it changes which of the equally-cheap repairs survives and nothing else. It wins both cases named in the brief, where m74 wins one, and cuts input characters destroyed across the battery from 471 to 334. THE SHAPE COLUMN READS 474 AGAINST m74\'s 517 AND THAT IS THE METRIC BEING WRONG, NOT THE ENGINE: on all 43 inputs where m74 matches the pre-corruption shape and m75 does not, m74 buys the match with at least one node the input does not support -- 43 of 43 -- and there are 0 inputs where m75 matches and m74 does not. The battery is built by inverting a known corruption, so shape scores guessing the original document, which is exactly the invention the brief forbids. WHAT THIS DELIBERATELY DOES NOT DO: it does not delete the certificate, and that was tried, measured and reverted. With the parse removed the chase\'s own checks were taken as the proof, and m75 answered 2 verified on inputs whose true cost is -1, wrong on 28 of 31 strings over \'a\'* "ab", a grammar whose language is EMPTY, plus 292 cost differences and 319 wrong of 2387. The obligations are still approximated to one character, so the chase is not a membership proof and the parse cannot go until they are exact -- _repaired and _spelling survive for a YES/NO only, and their text never reaches the tree. ALSO REFUTED, cheaply, before building it: making the certificate CONDITIONAL on the derivation having used only exact obligations buys nothing, because obligations are enforced only in the tight pass and I28\'s whole result is that the tight pass never runs on JSON. MEASURED, one engine per process, constructed once, n=21 paired rounds through recover: battery 292.3 against m74\'s 300.7 at 14 of 21, latency 178.3 against 183.6 at 13 of 21 -- both weak separations, so call them indistinguishable rather than a win. Exactness is m74\'s exactly: 0 cost differences, wrong on 98 of 2387, conformance 5/5. LOC 746 against m74\'s 791.'),
  'm76': (6, 'CODEX\'S ENGINE, AND THE FIRST TO CLOSE D-A: EXACT REGULAR PEG OBLIGATIONS INSTEAD OF m75\'s ONE-CHARACTER FIRST SET. A committed choice, a possessive stop and a lookahead each create a predicate over the repaired suffix; m76 normalises it to a regular PEG and carries it as a canonical parsing-expression derivative, so a memo key holds the product-DFA state of every live obligation. That is the right abstraction and it is measured to work: on the 23-grammar gate that can SEE D-A, m75 and m77 are wrong on 711 of 6461 strings (390 too low, 248 false -1) and m76 on 12, with both empty-language star families -- ("ab")* "abc" at 327 and ("ab")* "aba" at 63 -- going to 0. It also closes D-B by construction: zero Parser constructions and zero .parse() calls in the whole file, verified by grep, which is the one thing D1 forbids by name and which m75/m77 still do. It reproduces I33 byte-for-byte having been written independently, which is the strongest evidence the project has that I33 is derived rather than fitted. SCORED 6 BECAUSE IT CRASHES ON GRAMMARS EVERYBODY WRITES: _norm rejects a rule reference reached while left-expanding an obligation by THROWING UnsupportedError, so every damaged input under a left-recursive grammar throws -- cost 0 survives because a clean parse builds no obligation, and any repair at all does not. That is 18 of the table\'s 113 ground-truth cases lost to an exception (cost 26/44 against m77\'s 44/44) on three ordinary expression grammars. The user\'s scope grant was about RECURSIVE PREDICATE BODIES; it was applied one level wider, to any obligation that left-expands into a recursive rule. m78 fixes exactly this and supersedes m76. TWO BLIND GATES, NEITHER REDUNDANT, is the durable lesson: my 23-grammar gate has no left-recursive grammar and called m76 nearly perfect, the table\'s truth cases have no empty-language star and called m77 perfect at 113/113, and each gate certified the engine the other condemned. The crsh column read 0 through all 18 throws because it counts battery crashes only and measureOne swallows truth-case throws in a bare catch, which is a pre-existing flaw in the instrument, not in the engine. Also 12 false -1 it shares with m78, described there. LOC 1294 is not comparable to m77\'s 763 as written: m76 is standalone and m77 borrows the 705-line library parser, so the honest comparison of complete artifacts is 1294 against 1468, and of recovery alone 1143 against 763 -- the 380-line price of exact obligations.'),
  'm77': (9, 'I33 ON m75: DESTRUCTION IS THE WIDEST DESCRIPTION, SO IT IS NOT A SEPARATE AXIS. m75\'s secondary key is the pair (invention, loss) -- bits asserted, then characters destroyed -- ordered rather than summed because no exchange rate between a bit and a character could be derived. Loss was never a second quantity; it is the LIMIT of the first one measured from the other side. Read a repair as a DESCRIPTION of the input: every input character the repair gives a role to is described by the class that consumed it and costs log2|C| bits, so a singleton costs nothing because the grammar already said which character it was, [0-9] costs 3322 millibits, and a character the repair DELETES gets no class at all and therefore costs the most any character can, log2|Sigma| = 16000. Deletion is not a different kind of harm, it is the widest possible reading -- the case where the description says nothing whatsoever -- so the axis is DELETED rather than added, which is the test a principle has to pass where a heuristic does not. The implementation is four places and 17 net lines (746 -> 763): a memoised fold _descOf over the oracle\'s own MatchResult objects, the budget-zero walk paying _descOf(m) for the subtree it settles in one step, a clean terminal read paying width*len, and SUB paying width*descSpan + widestClass against FAB\'s width*descSpan. DELETE needs no case whatsoever: it is already SUB on _junk, whose class is empty, so it invents 0 bits and pays exactly one widest description. THE ALPHABET HAD TO BE CORRECTED FIRST, and this is a pre-existing flaw in m75 that Codex found independently while building its own m76: _width priced an inverted class over 0x110000 CODE POINTS, but CharSet.match reads input.codeUnitAt(pos) and returns a match of length 1 (lib/src/parser/terminals.dart:97-113), so the parser\'s symbol is one UTF-16 CODE UNIT and |Sigma| = 65536. A supplementary code point never reaches a class as a single symbol; it arrives as two units matched separately. m75 was counting 1114112 things the parser cannot see. The flaw was INVISIBLE while width priced only fabrication, because a uniform over-count of every wide class scales the invention axis without reordering it, and it becomes load-bearing only under I33 where the same constant prices every discarded character. m77 sets _alphabetSize = 0x10000 and DERIVES _widestClass = log2(0x10000) = 16000 millibits through the same formula _width uses, so the two ends cannot drift apart -- one fewer typed constant, not one more. Re-running every gate afterwards returned byte-identical numbers, confirming rather than assuming that the error was outcome-neutral. WHY B021 FLIPS, and it is a measured number rather than an argued one: Character <- [^"\\\\] is stored INVERTED, so _width prices it at 65534 of 65536 members = 16000 millibits, THE SAME NUMBER as _widestClass after rounding. Absorbing a character into a JSON string therefore costs exactly what deleting it costs -- the honest statement of what a near-universal class tells you -- so m75\'s reading of 2[,33,true] as a String is eleven deletions wearing a String node, 220957 millibits against the structural reading\'s 30053. m75 scored that reading (invention 0, loss 0), the BEST pair the secondary key can give, so this was never the tie-break failing to prevent the reading; it was the tie-break preferring it. INVENTION STAYS STRICTLY AHEAD and the reason sharpens from \'no exchange rate\' to FALSIFIABILITY: a description can be checked against the input the caller still holds, an invention against nothing. The summed variant is REFUTED by the brief\'s own case -- with one MDL total, [,2, repairs by fabricating a digit (3322) instead of deleting the stray comma (16000), the repair the user ruled out by name. Summing is the arbitrary constant wearing a formula. A SECONDARY KEY MUST NOT MOVE A COST AND IT DOES NOT: _id77.dart reports 0 cost differences from m75 across battery (519), valid controls (7) and latency (12), with certified 519/7/12 unchanged and TILES, TOTAL and UNSUPPORTED all unchanged. Correctness is likewise untouched, as a tie-break must be: 98 wrong of 2387 with the identical split (0 too low, 41 too high, 57 false -1, 97 of them the one multi-character-lookahead grammar), conformance 5/5, _m77diff 0 diffs on all five grammars, dart test 308/308, core gate A 2996/2996 B 3252/3252 C no drift D clean. WHAT MOVED IS THE SHAPE, THE RIGHT WAY: shape goes 474 -> 500 of 519, twenty-six mutants that m75 repaired into a differently-shaped document now recovering the shape the author wrote, with 65 battery trees changed in total. ture stops being a String with two fabricated quotes and becomes Boolean[18,22) with an inserted r and a deleted r. B021 reaches a repair I would not have proposed, for the user\'s own reason: m77 substitutes the 2 into [ and fabricates the inner ], giving [[],33,true] at cost 2, because the obvious transpose is also cost 2 but has to RE-INSERT a digit -- invention 3322 against 0 -- and invention is the first key. The engine refuses to invent a digit exactly because it could be anything. The eleven characters get their structural roles back: 33 is a Number, true a Boolean, the brackets are brackets. Both hard requirements are unmoved: ,3true still reads 3 as a Number and inserts the separator (,3,true not ,true), and [,33,true] still DELETES the stray comma rather than inventing a value. ONE EXACT TIE, named so it is not hunted later: on "a":"1 inserting the closing quote reads 1 through [^"\\\\] at 16000 while substituting 1 -> " destroys it at 16000, with identical cost and identical invention, so the winner is whichever _keepBest sees first in split order. Both readings satisfy the tree contract. WHAT m77 DOES NOT DO: it does not touch the one-character obligation or the certificate re-parse. The residual 98 is unchanged and _certified still re-parses the repaired string. I33 is the secondary key and the measurements confirm it stayed there. SCORED 9, NOT 10, AND THE MISSING POINT IS CRITERION 5: the engine cannot be stated in one sentence that is true of the code, because the sentence would be \'minimum-edit repair by DP over (position, obligation) cells, with the tree built over the original input\' and the code ALSO retains a second full parse of the repaired string as a yes/no certificate -- the one thing D1 forbids by name. m77 is better than m75 on criteria 1 (it DELETES an axis), 2 (a typed 20087 becomes a derived log2 of the alphabet) and 3 (the fold is over the parser\'s own MatchResult objects), and slightly worse on 4 (746 -> 763); if m75 is a 10 then m77 is a 10 too, which is the tell that the scale was already saturated and that the certificate should have capped both. m75\'s committed 10 is left alone rather than silently revised, but by this criterion it is a 9.'),
  'm78': (8, 'I34 ON m76: AN OBLIGATION YOU CANNOT WRITE DOWN CONSTRAINS NOTHING. m76 carried exact regular PEG obligations and THREW on the two shapes that are not regular -- a rule reference reached while left-expanding, and zero-width repetition -- costing it every damaged input under a left-recursive grammar, which is to say every ordinary expression grammar. The throw was wrong on its own terms: refusing to answer is itself a claim, and a stronger one than the evidence supports. THE FIX IS FORCED, NOT CHOSEN, and that is what makes it a principle rather than a fallback: _settle is the ONLY place in the engine where a residual meets its polarity, and the vacuous residual is polarity-dependent -- fail discharges a negative obligation but kills a positive one, and an eps-accepting residual does exactly the reverse. So the inexpressible case cannot be spelled with any constant already in the algebra. It needs its own element. _opaque absorbs through every constructor (_pn, _po, _pseq, _derive, _canonical) and is discharged, imposing nothing, at the one point where the polarity is known. The engine may not enforce a requirement it cannot check, in either direction. MEASURED AGAINST m76 ON BOTH GATES, because each is blind where the other sees: the 18 exceptions become 15 correct answers and 1 wrong one (110/113 against 93/113, 0 throws), and 0 of 6461 strings on the 23-grammar gate change their answer at all -- so this buys back nothing, which is the check that mattered. Net 2 lines of code, 1294 -> 1296. WHAT REMAINS, characterised exactly rather than estimated: an obligation is discharged only when the window its lookahead reads is FREE OF EDITS. Paired inputs isolate it with the grammar and the true cost held fixed -- "xab" (edit before the window) is priced correctly at 1, "axb" (edit inside it) is not, and both delete one character and both read an original b, so the boundary is the edit-free window and not the original character. The chase then ends with _edits.length == cost but _atEnd(proof) false, and recoverCost fails closed to -1 rather than return an uncertified answer. That is 12 of 6461 and 3 of 113. It is m43\'s rule (the oracle is authoritative as far as the edit-free window reaches) reappearing in the obligation replay instead of the oracle, which is the reason to expect it is one mechanism and not three. TWO HYPOTHESES REFUTED BY MEASUREMENT BEFORE THE RIGHT ONE, recorded because the refutations cost real time: the _derive choice mark-check and the _mergeAlt oracle-commit prune were each patched and each changed 0 of 6461 answers, and _chosenEmission -- the obvious suspect, since a replay that cannot pick an invented character would explain everything -- reports 0 misses on every failing input. SCORED 8: it deletes a crash and a whole class of refusal, it adds an element that is forced by the algebra rather than tuned, and it keeps every property m76 bought (D-A closed, D-B closed by construction, I33 reproduced). It is not a 9 because 1296 lines is nowhere near the stated goal, and not a 10 because the edit-free-window limit above is a real hole that is now named but not closed.'),
  'm74': (10, 'I29+I30+I31: THE REPAIRED STRING IS THE WITNESS, AND ITS PARSE IS THE TREE. Abstracted to the axiom, the search answers ONE question -- what is the cheapest edit list that puts the input in the language -- and everything after that is arithmetic on positions. m62 through m73 all miss this: they carry _build/_child/_row/_collect/_emit/_verify, 159 lines whose job is to build a tree over the INPUT, walk that tree into a string, and re-parse the string to check it. The last step already produces a parse tree of the repaired string, and the reconstruction was built to produce a parse tree -- so the certificate was throwing away the exact object the 159 lines existed to compute. I31 keeps it: apply the edits to get y, hand y to the pure parser that has to accept it anyway, and re-index the tree it returns back onto the input through the same edit list. 88 lines replace 159, and it is the first change in the m-line where size and speed pull the same way, because the split measurement says the larger half is the one being deleted -- measured on m73 with all three arms on one clock at n=21, the certificate is 43.6 ms of the battery, _build 24.3 of it and emit-plus-fresh-parse 19.3. The kept half is not free, and keeping it is the point: it is the half that hands back the tree. I29 IS WHAT MAKES THE CHASE POSSIBLE: a fourth int beside (key, cost, regret) records WHY the value was written, so the witness is read off the table instead of searched for, and acyclicity stops being a check and becomes a proof -- every edge adds a non-negative increment, so a cycle would force every increment to zero and contradict the strict drop the last write on it made. I30, WHEN THE COMPARISON REFUSES A TIE THE ENUMERATION ORDER IS THE REST OF THE ANSWER: values are kept in SPLIT order (by end, then key), so a _Cons offers the SHORTEST head first, which is the reading a recursive-descent PEG parser reaches first. m73 could leave arrival order because _build re-sorted the candidates as it rebuilt; the chase has no reconstruction to sort in, so the order the search WROTE is the answer, and shape reads 513 instead of 517 without it. REFUTED, and it was the largest remaining size candidate: regret is not made redundant by that order. Deleting the whole regret apparatus (skipRegret, cleanRegret, the FAB width) leaves cost and certification bit-identical -- costsum 563, 519/519 certified -- and drops shape 517 to 488. They are DIFFERENT tie-breaks: regret picks among equal-cost repairs, enumeration order among equal-(cost, regret) derivations. Two more things had to be derived rather than tuned. The re-index maps a position in y back to one in x, and a boundary belongs to the leaf on its LEFT, which is the one that has to grow; position 0 is the sole boundary with nothing to its left, so a deletion there is absorbed rightwards instead -- 6 bad trees until that case was stated. And I26 goes one level further out: the re-index is a pass too, since a parse tree is as deep as its input on the ladder grammars and recursing there held RRmax at 2048. It can go TOP-DOWN, which a rebuild normally cannot, because Match re-derives its span from its children only when they are already there (match_result.dart:40) and keeps the list it was handed: a node built with an empty list keeps the span this pass computed, and its children are poured into that same list afterwards. LOC 787 -- the first engine in the m-line under m62\'s 789 while answering the true PEG language, against m71\'s 1028 and m72\'s 979. 36 of those lines came from four simplifications, not from golf: the three child-request sites in the driver run one protocol, so they hoist into _demand; the budget-zero walk\'s two arms differ only in whether the match read a character, so naming that difference collapses them; _reindex fills top-down through one stack; and a clean parse is its own tree, so it goes in the same slot as every other answer and recover loses its special case. TIMING, and it needs the entry point named because the two are not the same question. Medians of 22-23 one-engine-per-process runs: through recover -- what a caller actually calls, and the only place both engines have produced the same thing when the clock stops -- m74 is 289.2 against m62\'s 293.2 on the battery (faster in 15 of 22 paired rounds) and 197.6 against 219.3 on latency (22 of 22). Through recoverCost m62 leads the battery 253.3 to 291.2 (0 of 22), and it leads there by not having finished: completing the same job costs m62 +39.9 ms, where m74\'s recover is -2.0 ms against its own recoverCost because the witness is already built when the number comes back. This table\'s own columns split the same way -- battms times recover, latms times recoverCost -- so the battery column compares like with like and the latency column hands m62 the favourable entry point, where m74 wins anyway. Exactness is m71\'s exactly: 0 cost differences from m71 on all 2387 brute-force truths, wrong on 98 where m62 is wrong on 324, 226 fixed, 0 regressed. THE HONEST DEVIATION: SkipResult\'s docstring promises unparseable regions appear as SyntaxError CHILDREN of the tree. m74\'s re-indexed tree has none -- a deletion is absorbed by the leaf ending at that boundary, which is why cover still reads 519/519 -- so a caller walking for SyntaxError nodes finds the deletions only in errorSpans. m62 already reported substituted terminals that way; deletions are the part that moved. The residual 98 are m71\'s unchanged, 97 of them one grammar whose lookahead is multi-character, reachable only by a tape'),
  'm72': (10, 'I29+I30: THE WRITE KNOWS ITS OWN REASON, AND WHEN THE COMPARISON REFUSES A TIE THE ENUMERATION ORDER IS THE REST OF THE ANSWER. m71 answers the true PEG language in 1028 lines and 217 of them are _reconstruct -- a function whose whole job is to work out something the search already computed and threw away. Every memo value in this family is written by one funnel, _put -> _keepBest, and at the instant of the write the thing that produced it is a local variable: the spine\'s headKey, the alternation\'s branch index, or one of three leaf shapes. Keeping it costs one int per entry, a fourth slot beside (key, cost, regret). What it buys is not only the deletion of the search but the deletion of the DOUBT: m71 carries a _path set of cycle tokens because a re-derived predecessor chain might loop, and under I29 it cannot. Suppose the back-pointers closed a cycle at the fixed point; every edge adds a non-negative increment, so summing round the cycle forces every increment to zero and every value equal; take the cell on the cycle written LAST, at time t -- that write was a strict improvement so its value strictly dropped at t, but some cell on the cycle consumed this one\'s value BEFORE t and hence a strictly larger one, contradicting equality. A back-pointer closes a cycle only if the cycle has strictly negative weight, and every weight here is >= 0. The whole proof rests on one line of _keepBest, which refuses an equal entry. So _path, the candidate sort, both backtrack arms, _deltaOf and _ends existed only because the reason was discarded; _build can no longer fail to find a witness for a cost the search reported. _verify stays, since I5 checks the witness against the REAL parser -- a different question from self-consistency. COST IDENTITY IS NOT EVIDENCE OF WITNESS IDENTITY: the first build passed the cost gate outright, _subset72 reading 0 differences against m71 on all 2387 brute-force truths, and was still broken. Under I28 the engine demands a certificate and re-runs tight when one does not arrive, so a chase that silently fails is INVISIBLE to a cost comparison. _wit72 compares what a caller receives and read 1283 certificate differences, m71 certifying 2245 against m72\'s 962 -- more than half the witnesses destroyed by a change the cost gate called identical. Every failure was the _wPure sentinel read as a head key, because the budget-zero window writes _wPure into ANY node\'s cell and _child tail-calls _row for a junk-headed spine. THE CANONICAL FORM: a reason is a SUFFICIENT explanation, not a canonical one, and _wholesale prefers the oracle\'s whole-subtree match at cost == 0 -- no candidates, no backtracking, and when it declines the recorded reason answers. It belongs to _build and NOT to _row, since _row walks a spine\'s SUFFIX nodes and a suffix shares its orig with the whole sequence; putting it there dropped certificates 2245 -> 2197 and produced trees with gaps. I30: _keepBest refusing an equal entry means whoever is offered FIRST wins, so the enumeration order stops being a scheduling detail and becomes the semantics. PEG says take the reading a recursive-descent parser reaches first: earliest branch for an alternation (free, _mergeAlt walks in source order) and SHORTEST HEAD for a sequence, which m72 lacked because head answers arrive cheapest-first and the cheapest head is the one that swallowed the repair. Fixing it moved shape 513 -> 517/519 with nothing regressing. IT CANNOT GO IN THE COMPARISON: accepting an equal write destroys the proof, which needs the last write to have strictly dropped. THE MEASUREMENT WAS THE LAST THING STANDING AND IT WAS WRONG FOUR TIMES. Six hypotheses for the row\'s latms gap were each killed by their own test: the restart guard fires 0 times in 274176 resumes; sorting at consumption is worse; the ordered insert shifts only 186699 of 49873423 scanned entries; stride-3 packing gains 0.6%; and _wholesale\'s rebuild is 4-7x FASTER than m71\'s, not slower. _work72 reads the step ratio at 1.0027 and the cell ratio at 0.9944 -- the engines do the same work. FIRST ERROR, THE APPARATUS: harnesses sharing one VM share its JIT state, and _recon72 timed the SAME build at 234.1, 257.4 and 270.2 ms in consecutive reps; _cold72 runs one engine per process, rotated across rounds. SECOND ERROR, THE SAMPLE SIZE: medians of 7 concluded I30\'s ordering was FREE at 1.001, and a second n=7 sweep put the same ratio at 1.047 -- a better instrument read at too small an n reproduces exactly the failure it was built to fix, so USE n >= 21. THIRD ERROR: bisecting _keepBest had been dismissed at ~2% as not the bottleneck, and split order is a TOTAL order computable from the key alone, (endOf(key), key), so the walk that looks for a key bisects and a miss stops exactly where the new key belongs. Folding it in needed a PROOF and not a benchmark, because under I30 the enumeration order IS the tie-break rule and any change to the list is a change to the LANGUAGE: _bseq compares certificate, spans, missing set and tree shape over 519 battery inputs and 12 latency cases -- 531 checked, 0 cost differences, 0 full-result differences -- with _m72bs deliberately keeping the LINEAR form so it does not assume the ordering it exists to check. FOURTH ERROR, THE PROTOCOL, AND IT INVALIDATES WHAT THE FIRST THREE MEASURED: _cold72 built a FRESH engine inside every timed call, and the lowering is late final, so every call re-lowered the grammar inside the clock and threw away the previous input\'s caches. THE TEMPTING EXPLANATION IS WRONG -- _ctor72 prices fresh-per-call at 11.9 ms per round for m71 AND 11.9 for m72, identical, of which the lowering is 0.57 ms per construction, AND A CONSTANT ADDED TO BOTH CANNOT INVERT THEIR ORDER; it inflates the ratio instead, 1.061 fresh against 1.024 reused in one process. The ablation arms were confounded too: _m72app is LINEAR where m72 BISECTS, so app/m72 charges I30 and credits the bisect inside one number, which is why it read 1.0009 and looked like I30 was free. Confound-free at n=21: m71 217.9, app 221.5 (no I30, linear), bs 252.8 (I30, linear), m72 222.0 (I30, bisected). I30\'S ORDERED INSERT COSTS 12.4% AND THE BISECT GIVES 12.2% BACK, at 21/21 and 20/21 paired rounds -- the only clean separations in the occasion. So I30 is not free and is not 3.8%: it costs 12.4% and it is PAID FOR, and the ordering that makes enumeration the tie-break rule is affordable only because split order is the total order the walk can bisect -- the insight and the thing that pays for it are one fact about the key. Net, I30 buys 4 shape points for nothing: unordered-and-linear 221.5 against ordered-and-bisected 222.0. WITHDRAWN: m72 is 2.7% slower at every one of 21 ranks; I29 costs 3.9%; I30 costs 3.8%; the bisect is worth 5.0%. THE TABLE\'S OWN COLUMN IS THE WORST OF THE FOUR: across four full runs m72\'s latms sat BELOW m71\'s every time, 210.2 against 225.0, ratio 0.934, an ordering NO single-engine protocol reproduces in either arm -- 45 engines share one VM here, so treat latms as comparative-within-a-run and let no claim rest on it. WHEN TWO INSTRUMENTS DISAGREE ABOUT THE ORDER OF TWO THINGS RATHER THAN THE SIZE OF A GAP, NO SAMPLE SIZE SETTLES IT: the move is a control that changes exactly one thing. MEASURED construct-once, one engine per process: 979 LOC against m71\'s 1028 and m70\'s 1331, conformance 5/5 and shape 517/519 both identical to m71, exactness byte-identical (98 wrong, 226 fixed, 0 regressed, 0 cost differences), 0 certificate differences with 2245 certified by each. Against m71 a genuine improvement -- 49 lines smaller, latency 1.0050 at 7/21 and battery 0.9925 at 10/15, neither resolvable. AGAINST m62 IT DOES NOT DOMINATE AND THE GAP IS WIDER THAN THIS TABLE SHOWS: m62 is 789 LOC, 4.3% faster on latency (2 of 21 paired rounds) and 26.4% faster on the battery (0 of 15), where these columns read those same two gaps as 3.75% and a dead tie, and it holds RRmax >=4096 against m72\'s 2048 -- while scoring conformance 3/5 to m72\'s 5/5 and 324 wrong to m72\'s 98. Smaller and faster, or correct; nothing in this table is both.'),
...
final engines = <Eng>[
  ...
  Eng('m73', (r, t) {
    final e = g73.SuperDot3(rules: r, topRuleName: t);
    return (e.recover, () => e.lastCost, e.recoverCost);
  }),
  Eng('m74', (r, t) {
    final e = g74.SuperDot3(rules: r, topRuleName: t);
    return (e.recover, () => e.lastCost, e.recoverCost);
  }),
  Eng('m75', (r, t) {
    final e = g75.SuperDot3(rules: r, topRuleName: t);
    return (e.recover, () => e.lastCost, e.recoverCost);
  }),
  Eng('m76', (r, t) {
    final e = g76.SuperDot3(rules: r, topRuleName: t);
    return (e.recover, () => e.lastCost, e.recoverCost);
  }),
  Eng('m77', (r, t) {
    final e = g77.SuperDot3(rules: r, topRuleName: t);
    return (e.recover, () => e.lastCost, e.recoverCost);
  }),
  Eng('m78', (r, t) {
    final e = g78.SuperDot3(rules: r, topRuleName: t);
    return (e.recover, () => e.lastCost, e.recoverCost);
  }),
