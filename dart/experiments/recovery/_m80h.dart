...
// INSTRUMENTED COPY -- histogram of ways-map sizes, cells, rounds.
int cells = 0, rounds = 0, entries = 0;
final hist = <int, int>{};
...
class _Way {
  const _Way(this.cost, this.got, this.net, this.synth, this.nodes);
...
const _Ways _none = {};
const _Way _empty = _Way(0, 0, 0, false, null);
...
bool _beats(_Way a, _Way? b) =>
    ...
    a.cost < b.cost ||
    (a.cost == b.cost &&
        (a.net > b.net || (a.net == b.net && a.got > b.got)));
...
class SuperDot3 {
  ...
  _Ways _fix(Map<Clause, List<_Cell?>> memo, Clause c, int pos,
    ...
    while (true) {
      ...
      // monotone; lifted to a map of endings, monotone means merge. Replacing
      ...
      if (cell.ways.isEmpty) {
        cell.ways = r; // merging into nothing is the thing itself
      } else {
        final merged = {...cell.ways};
        for (final e in r.entries) {
          if (_beats(e.value, merged[e.key])) merged[e.key] = e.value;
        }
        cell.ways = merged;
      }
    ...
    cell
      ...
      ..gen = gen[pos];
    cells++;
    entries += cell.ways.length;
    hist[cell.ways.length] = (hist[cell.ways.length] ?? 0) + 1;
    return cell.ways;
  }
    ...
    switch (c) {
      ...
      case FollowedBy f:
        ...
        if (_pure(f.subClause, pos).isNotEmpty) {
          return {pos: _Way(0, 0, 0, false, _N(_T.leaf(Match(f, pos, 0)), null))};
        }
        ...
        return {
          pos: _Way(w.length, 0, 0, true, _N(_T.leaf(Filled(f, pos, w)), null))
        };
      ...
      case NotFollowedBy n:
        ...
        return {pos: _Way(0, 0, 0, false, _N(_T.leaf(Match(n, pos, 0)), null))};
      ...
      case Terminal t:
        ...
        return m == null
            ...
            : {
                pos + m.len:
                    _Way(0, m.len, explains, false, _N(_T.leaf(m), null))
              };
  ...
  _Ways _wrap(Clause c, int pos, _Ways w) => {
        for (final e in w.entries)
          e.key: _Way(e.value.cost, e.value.got, e.value.net, e.value.synth,
              _N(_T.node(c, pos, e.value.nodes), null)),
      };
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    for (var k = 1; k <= _budget && pos + k <= _in.length; k++) {
      ...
      for (final e in _clause(sub, pos + k).entries) {
        _put(
            out,
            e.key,
            _Way(k + e.value.cost, e.value.got, e.value.net, true,
    ...
    if (w != null && w.length <= _budget) {
      _put(out, pos,
          _Way(w.length, 0, 0, true, _N(_T.leaf(Filled(sub, pos, w)), null)));
    }
    return out;
  }
}
