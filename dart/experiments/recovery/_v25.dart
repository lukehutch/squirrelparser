...
class _Way {
  const _Way(this.end, this.del, this.gap, this.net, this.key,
      {this.leaf, this.cap, this.from = 0, this.link, this.prev, this.mark});
  ...
  _Way.owe(int p, int n)
      : this(p, 0, n, 0, p, mark: SyntaxError(pos: p, len: 0));
  ...
  _Way over(MatchResult n, [int k = _peg]) =>
      _Way(end, del, gap, net, _min(key, k), leaf: n);
  ...
  _Way capped(Clause c, int pos, [int k = _peg]) =>
      _Way(end, del, gap, net, _min(key, k), cap: c, from: pos, link: this);

  /// No longer PEG's reading -- but still whatever else it was.
  _Way get demoted => _Way(end, del, gap, net, _min(key, _far),
      leaf: leaf, cap: cap, from: from, link: link, prev: prev, mark: mark);
}
...
class Squirrel {
  ...
  /// THE FIRST ROUND IS THE FROZEN PARSER, SO IT SHOULD HAND OUT WHAT A FROZEN
  /// PARSER'S MEMO ENTRY HOLDS: one reading, PEG's own, or none.
  ///
  /// A clean way that is not PEG's reading is already dead when round 0 makes
  /// it. [_Way.then] takes the MINIMUM key, so any chain holding one is at most
  /// `_far`, and [recover] refuses exactly `_far`; buying it back would take a
  /// repair, and this round has nothing to spend. Cutting here deletes the
  /// entire cross product that a sequence of repetitions explores over input
  /// with nothing wrong with it, and it deletes it at the SOURCE: the next cell
  /// out expands from one way rather than from n, so the narrowing propagates
  /// and no cell ever materialises the wide set in the first place.
  ///
  /// THE CUT IS ON THE WAY OUT, NOT INSIDE THE FIXED-POINT LOOP. A
  /// left-recursive cell grows by re-reading its own ways as the next seed
  /// (line `_afford(e.ways)`), and a seed cut down to one way makes that
  /// fixed point converge in more passes than it needs -- measured at +2.3%
  /// iterations and ~5% on the battery when the cut sat in [_prune]. Storing
  /// the full set and handing out one costs nothing, because the callers that
  /// build the wide set are themselves fed by this same cut.
  ///
  ...
  /// purpose -- but those want a slot that reads CLEANLY, not one that reads as
  /// PEG would, and their chain has already paid, so its key is below `_far`
  /// and the root will take it. Only in the first round are the two questions
  /// the same one, and only there is nothing else affordable.
  List<_Way> _only(List<_Way> ws) {
    if (_round != 0 || ws.length <= 1) return ws;
    _Way? top;
    for (final w in ws) {
      if (!w.peg) continue;
      if (top == null ||
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    if (e.has && e.gen == _version[pos] && e.at >= _budget) {
      return _only(e.at == _budget ? e.ways : _afford(e.ways));
    }
    e.inPath = true;
    while (true) {
    ...
    for (_round = 0; _round <= ceiling; _round++) {
      ...
      for (final w in _ways(rules[topRuleName]!, 0)) {
        ...
        final a = _Way(w.end, w.del + tail, w.gap, w.net,
            ...
            leaf: w.leaf, cap: w.cap, from: w.from, link: w.link);
