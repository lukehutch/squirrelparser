// m68 -- THE CERTIFICATE RETIRES THE LATTICE: m67 with the obligation
// machinery deleted from the relaxed core.
//
//   I24 UNDER A CERTIFICATE, THE FAST ENGINE ONLY NEEDS TO BE A FLOOR.
//       The I6/I7 obligation lattice existed to make the RELAXED engine
//       predicate-exact, back when it was the final authority. Under the
//       verified-witness architecture it is not: an under-report can never
//       verify (its witness would realize a cost below its own claim), so
//       every predicate mistake fails verification and routes to the tape,
//       which is exact. On lookahead-free grammars the union relaxation is
//       trivially a floor -- and the whole performance corpus (JSON) is
//       lookahead-free, so the lattice was inert on every measured input.
//       The router condition simplifies to its bluntest form: any lookahead
//       anywhere sends the grammar to the tape. The price, named: predicate
//       gates run at tape speed (they are tiny), and their witness shapes
//       are the tape's projection, not the lattice's derivation.
//
// Everything else is m67 verbatim (I18/I20/I21/I22/I23 and ancestors).
// PARAMETERS: NONE.
...
class SuperDot3 {
  int dbgRelaxedCost(String s) { _relaxedRecover(s); return lastCost; }
  bool get dbgWide => _wideG;
  int get dbgMass => _massG;
  String dbgSpell(Clause c) => _spelling(c);
