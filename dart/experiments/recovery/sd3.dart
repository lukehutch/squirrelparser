...
class SuperDot3 {
  ...
  late String _in;
  late int _n;
  late int _bud; // the budget of the current deepening pass, for memo keying
  ...
  Map<int, int> _ends(Clause c, int pos, int b) {
    ...
    final key = (_id(c) * (_n + 2) + pos) * (_bud + 1) + b;
    final hit = _memo[key];
    if (hit != null) return hit;
    _memo[key] = const {}; // left-recursion cut while computing
