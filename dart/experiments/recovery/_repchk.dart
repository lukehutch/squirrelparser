...
// PROBE ONLY. Does the left-to-right sweep compute the same SET of ways as the
// wave, ignoring the order they come out in?
int nRep = 0, nRepDiff = 0;
final List<String> repDiffs = [];
...
class SuperDot3 {
  ...
  _Ways _rep(Repetition r, int pos) {
    ...
    // ---- the sweep, computed alongside and compared on CONTENT only ----
    _Way? sw = _nil(pos);
    for (var at = pos; at >= 0;) {
      final w = _at(sw, at)!;
      for (var x = _element(r.subClause, at); x != null; x = x.next) {
        ...
        sw = _put(sw, _extend(w, x));
      }
      var nxt = -1;
      for (var p = sw; p != null; p = p.next) {
        if (p.end > at && (nxt < 0 || p.end < nxt)) nxt = p.end;
      }
      at = nxt;
    }
    String key(_Way w) =>
        '${w.end}/${w.cost}/${w.got}/${w.net}/${w.site}/${w.tail}/${w.synth}';
    final a = <String>[];
    ...
    final b = <String>[];
    for (var p = sw; p != null; p = p.next) {
      if (r.requireOne && p.end == pos) continue;
      b.add(key(p));
    }
    a.sort();
    b.sort();
    ...
    if (a.join(' ') != b.join(' ')) {
      nRepDiff++;
      if (repDiffs.length < 6) {
        repDiffs.add('budget=$_budget pos=$pos\n    wave  ${a.join("  ")}'
            '\n    sweep ${b.join("  ")}');
      }
    }
    // ---- end probe ----
