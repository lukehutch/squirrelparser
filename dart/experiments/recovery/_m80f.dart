...
class SuperDot3 {
  ...
  /// inside every ordered choice in the grammar. Measured on the battery, asking
  /// the pure table first is 15.6x, and it is not an approximation: a free
  /// reading beats every repaired one under [_beats] anyway, so the repair
  /// search it skips could never have won.
  ...
  _Ways _first(First f, int pos) {
    ...
    for (final a in f.subClauses) {
      ...
      if (w.values.any((v) => v.cost == 0)) return _wrap(f, pos, w);
    }
