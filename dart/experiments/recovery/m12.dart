...
class SuperDot3 {
  ...
  Map<int, int> _compute(Clause c, int pos, int b) {
    ...
    if (c is Seq || c is First || c is Optional || c is Repetition) {
      return _machine(c, pos, b, null, null);
    }
  ...
  Map<int, int> _machine(Clause c, int pos, int b, List<int?>? valOut,
      Map<int, (int, int, int, Clause)>? pred) {
    ...
    final val = valOut ?? List<int?>.filled(_dots(c) * width, null);
    final out = <int, int>{};
    final limit = (b + 1) * _M; // regret is always < _M, so cost = d ~/ _M
    val[0] = 0; // (dot 0, pos)
    for (var st = 0; st < val.length; st++) {
      final d = val[st];
      if (d == null) continue;
      ...
      for (final (sub, to) in _arcs(c, dot)) {
        ...
        for (var j = 0; j <= rem && p + j <= _n; j++) {
          ...
          for (final f in _ends(sub, p + j, rem - j).entries) {
            ...
            final cur = val[k2];
            if (cur != null && cur <= tot) continue;
            val[k2] = tot;
            if (pred != null) pred[k2] = (st, j, f.value, sub);
          }
        }
      }
    }
  ...
  MatchResult _build(Clause c, int pos, int end, int cost, int b) {
    ...
    final width = _n - pos + 1;
    final val = List<int?>.filled(_dots(c) * width, null);
    final pred = <int, (int, int, int, Clause)>{};
    _machine(c, pos, b, val, pred);
    var st = -1;
    for (var dot = 0; dot < _dots(c); dot++) {
      final k = dot * width + end - pos;
      if (val[k] == cost && _arcs(c, dot).any((a) => a.$1 == null)) {
        st = k;
        break;
      }
    }
    if (st < 0) throw StateError('no accepting state for ${c.runtimeType}');
    final parts = <MatchResult>[];
    while (pred[st] != null) {
      final (prev, j, subDelta, sub) = pred[st]!;
      final e = pos + st % width, start = pos + prev % width;
      parts.add(_build(sub, start + j, e, subDelta, b - _cost(val[prev]!) - j));
      if (j > 0) {
        _span(start, j);
