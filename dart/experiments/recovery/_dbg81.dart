...
class SuperDot3 {
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= cap; _budget = _budget == 0 ? 1 : _budget * 2) {
      ...
      _Way? best;
      final _dbgEnds = <String>[];
      for (var e = _element(top, 0); e != null; e = e.next) {
        _dbgEnds.add('end=${e.end} cost=${e.cost} net=${e.net} synth=${e.synth}');
      }
      print('  [budget $_budget] cap=$cap  top ways: '
          '${_dbgEnds.isEmpty ? "NONE" : _dbgEnds.join(" | ")}');
      for (var e = _element(top, 0); e != null; e = e.next) {
