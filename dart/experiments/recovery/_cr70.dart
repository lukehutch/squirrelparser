// m69.dart -- I25: A REPRESENTATIVE CHOSEN ALONE CANNOT MEET A CONSTRAINT
// IMPOSED BY SOMEBODY ELSE.
//
// m68 with one defect repaired, found by Codex round five and confirmed here
// on three grammars.
//
// The tape proposes characters to fabricate or substitute. m63/m65/m68 built
// that proposal one terminal at a time: the lowest member of a CharSet, code
// unit 0 for AnyChar. Each proposal is a legal answer to the question "what
// would satisfy ME" -- and that is the wrong question, because at a single
// position several terminals are consulted at once. `&[a-z]` says a-z and
// `[0-9m-q]` says 0-9m-q; the lowest member of each, 'a' and '0', satisfies
// its own asker and fails the other. No proposal ever lands in the
// intersection, so the search finds nothing and answers -1:
//
//   S <- &[a-z] [0-9m-q]   on ""   truth 1 ("m"), m65 and m68 both -1
//   S <- &[a-z] [0-9m-q]   on "z"  truth 1 ("m"), m65 and m68 both -1
//   S <- ![a-l] [a-z]      on ""   truth 1 ("m"), m65 and m68 both -1
//
// (Truth by exhaustive enumeration of every string of length <= 2 over
// printable ASCII, in `_isect.dart`. m62 answers all three correctly: its
// relaxed reader fuses `&C T` into `C n T` (I4), so the conjunction is one
// character class and one representative of it suffices. m68 deleted fusion
// and therefore routes every lookahead grammar to the tape, which is where
// the incomplete alphabet is fatal.)
//
// The repair is to stop choosing per terminal. Cut the code-unit line at
// every CharSet range boundary and every literal character; inside one
// interval of that partition every stock terminal answers identically, so
// one representative stands in for the whole interval without loss. A
// touched terminal then proposes EVERY representative it accepts. The union
// over the touched terminals now contains a representative of their
// intersection whenever that intersection is non-empty -- because an
// intersection of unions-of-intervals is itself a union of intervals, and
// that interval's representative is in every one of their proposal sets.
//
// Completeness, not heuristics: the alphabet is derived from the grammar, it
// is finite, and it is exactly as coarse as the grammar can distinguish.
// Zero tuning parameters, as before.
//
// Everything else is m68 verbatim: the certificate router, the relaxed
// fixpoint core, the derived horizon, the post-hoc (edits, regret) ranking.
...
class SuperDot3 {
  ...
  int _cleanRegret(MatchResult root) {
    ...
    final opened = <bool>[false];
    while (stack.isNotEmpty) {
      final m = stack.last;
      if (_cleanRegrets.containsKey(m)) {
        stack.removeLast();
        opened.removeLast();
        continue;
      }
      ...
      if (subs.isEmpty) {
        _cleanRegrets[m] = _widthOf(m.clause!) * m.len;
        stack.removeLast();
        opened.removeLast();
        continue;
      }
      if (!opened.last) {
        ...
        for (var i = subs.length - 1; i >= 0; i--) {
          if (!_cleanRegrets.containsKey(subs[i])) {
            stack.add(subs[i]);
            opened.add(false);
          }
        }
  ...
  int recDepth = 0, recMax = 0;
  MatchResult? _build(
      _Node node, int pos, int key, int cost, int reg, int budget, int c) {
    if (++recDepth > recMax) recMax = recDepth;
    try {
      return _build0(node, pos, key, cost, reg, budget, c);
    } finally {
      recDepth--;
    }
  }
  ...
  List<MatchResult>? _row(
    ...
    if (++recDepth > recMax) recMax = recDepth;
    try {
      return _row0(node, pos, key, cost, reg, budget, c);
    } finally {
      recDepth--;
    }
  }

  List<MatchResult>? _row0(
      _Node node, int pos, int key, int cost, int reg, int budget, int c) {
    if (node is! _Cons) {
