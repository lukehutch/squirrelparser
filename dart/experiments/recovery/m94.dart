// m94 -- I60: A REPETITION ONLY MOVES FORWARD, SO POSITION ORDER IS ALREADY
// TOPOLOGICAL ORDER.
//
// m94 is m92 with `_rep` swept instead of waved. The numbers must be identical;
// only the time moves.
//
// The VM's own sampler, over the whole battery, puts 66.45% of m92's inclusive
// time under `_rep` and 18.7% of its EXCLUSIVE time in hash-table machinery --
// the largest single category in the engine, and one that appears nowhere in the
// cost model. Most of that hashing is `_rep`'s own: a `Map<int,_Way>` and two
// `Set<int>` allocated on every call.
//
// The map and the waves both exist to answer "which endings are still moving?".
// They do not have to. `_rep` refuses an iteration that explains no character,
// and a way that explains a character must end past where it began, so EVERY
// edge in this relaxation runs strictly left to right. A graph whose edges all
// point forward is already topologically sorted by position: sweep the endings
// in increasing order and each one is reached with every way into it already
// built. One visit per ending, no frontier to remember, no second pass.
//
// What that deletes: the map, both sets, the wave loop, and the copy that turned
// the map back into a way list on the way out -- the ways ARE the list.
//
