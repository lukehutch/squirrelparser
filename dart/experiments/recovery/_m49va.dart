...
class SuperDot3 {
  ...
  /// rewrite. MEASURED, with the fusion deleted: every cost, every witness and
  /// every brute-force verdict identical across all four gates, and 411 -> 454
  /// `_compute` calls (`_steps49`), a ~30% penalty on `!'"' .` against the `[^"]`
  /// spelling of the same class and 2x on `(&[a-z] !'q' .)*`. So it stays -- for
  /// the constant factor, and because it keeps the two spellings the same WORK and
  /// not merely the same answer -- and nothing depends on it being correct.
        ...
        for (final alternative in alts) {
          for (final e in _ends(alternative, pos, budget, c).entries) {
            ...
            if (e.value < _costUnit &&
                _endOf(e.key) > committed) continue;
