...
int nLoopIter = 0, nLR = 0, nImpDiff = 0, nImpOldTrue = 0, nImpNewTrue = 0;
...
class Squirrel {
  ...
  static bool _improved(List<_Way> a, List<_Way> b) {
    var exact = a.length != b.length;
    if (!exact) {
      for (var i = 0; i < a.length; i++) {
        if (a[i].end != b[i].end || _rank(a[i], b[i]) != 0) {
          exact = true;
          break;
        }
      }
    ...
    if (b.isEmpty) {
      ...
      old = ra > rb || _rank(a.first, b.first) < 0;
    }
    if (old) nImpOldTrue++;
    if (exact) nImpNewTrue++;
    if (old != exact) nImpDiff++;
    return old;
  }
