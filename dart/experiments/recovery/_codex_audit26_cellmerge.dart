...
class _Way {
  const _Way(
      ...
      [this.node, this.prev, this.mark = -1]);

  /// Read nothing at [p], perfectly: the identity of the chain product.
  const _Way.unit(int p) : this(p, 0, 0, 0, _peg, null, p);

  /// Deny the characters in `[from, to)`.
  _Way.skip(int from, int to)
      : this(to, to - from, 0, 0, from, null, from, null, null, from * 2 + 1);

  /// Owe [n] obligations at [p], having matched nothing for them.
  _Way.owe(int p, int n) : this(p, 0, n, 0, p, null, p, null, null, p * 2);
  ...
  /// Deferred tree recipe. A [clause] with no [node] is a terminal leaf; one
  /// with a node wraps that child/chain. A null clause is a chain link whose
  /// node is its child. [mark] encodes no mark as -1, an owe as `pos * 2`, and
  /// a skip as `pos * 2 + 1`; a skip ends at this link's [end].
  final Clause? clause;
  final int pos, mark;
  final _Way? node, prev;
  ...
  _Way over(Clause c, int p, [int k = _peg]) =>
      _Way(end, del, gap, net, _min(key, k), c, p, this);
...
class Squirrel {
  ...
  /// Reused by [_prune]. The method calls no parser code, so one stamped dense
  /// end index is safe for every invocation and needs neither clearing nor a
  /// transient hash table.
  late List<_Way?> _atEnd;
  late List<int> _seenEnd;
  final List<int> _usedEnds = [];
  int _pruneStamp = 0;
  ...
  List<_Way> _prune(List<_Way> ws) {
    ...
    final stamp = ++_pruneStamp;
    _usedEnds.clear();
    var far = -1;
    for (final w in ws) {
      if (w.peg && w.end > far) far = w.end;
      if (_seenEnd[w.end] != stamp) {
        _seenEnd[w.end] = stamp;
        _usedEnds.add(w.end);
        _atEnd[w.end] = w;
      } else if (_rank(w, _atEnd[w.end]!) < 0) {
        _atEnd[w.end] = w;
      }
    }
    final out = [
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    while (true) {
      ...
      final got = old.isEmpty
          ? _prune(fresh)
          : fresh.isEmpty
              ? old
              : _prune(fresh..addAll(old));
  ...
  List<_Way> _lift(Ref c, int pos, List<_Way> ways) => [
        for (final w in ways)
          if (w.net > 0 || w.free || _determined(c)) w.over(c, pos)
      ];
  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    // Chaining is monotone within a PEG lane and within a non-PEG lane, but not
    // across them: a repaired suffix erases PEG precedence and can expose a
    // non-PEG prefix's greater net. Retain the best of each kind at every end.
  ...
  /// The first gap in the greedy subsequence alignment, without building its
  /// tree. A partial literal always has at least one gap.
  ...
  /// Materialize the one deferred tree that won the search.
