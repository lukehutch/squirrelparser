...
class Squirrel {
  /// PROBE knob: how many rungs past first admission to keep exploring.
  static int extraHold = 1;
  ...
  lib.MatchResult recover(String s) {
    ...
    for (_budget = 1;
      ...
      for (var ri = 0; ri < roots.length; ri++) {
        ...
        if (admittedAt >= 0) {
          // PROBE: after the admitting rung, dump every further complete
          // admission instead of competing with [best].
          print('PROBE rung=$_budget charge=${_charge(whole, 0)} '
              'ev=${whole.evidence} del=${whole.deleted} '
              'miss=${whole.missing + whole.missingAtEnd} '
              'pref=${whole.preferred} '
              'absorbed=${whole.absorbed(0)}');
        }
      ...
      if (best != null) {
        if (admittedAt < 0) {
          admittedAt = _budget;
          print('PROBE admitted rung=$_budget '
              'charge=${_charge(best!, 0)} ev=${best!.evidence} '
              'del=${best!.deleted} miss=${best!.missing + best!.missingAtEnd}');
          continue; // PROBE: hold the ladder one more rung.
        }
        final b = best;
