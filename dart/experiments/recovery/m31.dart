// m31 -- m26 with ONE mechanism split in two. NEGATIVE RESULT, and the reason is
// worth more than the engine: it proves m26's A3 violation is LOAD-BEARING.
//
// m26 stores TWO different quantities in the single integer `b`:
//
//   the cost to REACH an item     -- top-down, left-to-right, path-dependent.
//                                    This is what bounds the descent and what
//                                    keeps the search local to the damage.
//   the cost to COMPLETE an item  -- bottom-up. This is the end-map, and A3 is
//                                    exactly the claim that it must not be keyed
//                                    on any budget.
//
// Conflating them is why fixing either one breaks the other: m30 removed the
// budget arithmetic, which made the memo budget-free (count Theta(|G|*n*K)) but
// destroyed the descent bound, costing 14x and the stack. Here reach counts UP
// and is passed only to the SHORTCUT, while every memo entry is computed at the
// round budget and therefore exactly once.
//
// It does not work, and the failure identifies the real obstacle. m26's `b == 0`
// fast path returns `c.match(_parser, pos)` -- the SINGLE GREEDY end. But the
// budget-free cost-0 value of a Repetition has an end at EVERY iteration boundary,
// and that of a First has one per alternative. m26 memoises that impoverished
// answer under budget 0 and then RECOMPUTES the entry when it is later asked at a
// larger budget -- and the recomputation is what restores the missing cost-0 ends.
// So the "violation of A3" is not an accident to be deleted: it is the mechanism
// that masks an incomplete fast path. Remove the recomputation (m30) and you pay
// 14x; keep the recomputation and pay K^0.47. Both are symptoms of the same
// unsound base case.
//
// The fix that follows: make level 0 a REAL memoised budget-free evaluation --
// `_ends0(c, dot, pos)`, the same `_compute` with every edit move disabled -- rather
// than a single oracle call. Then it is complete at any dot (m26's is valid only at
// dot 0), the shortcut becomes sound, the recomputation becomes unnecessary, and
// the count drops to the Theta(|G|*n*K) that m30 already demonstrated.
//
...
class SuperDot3 {
  ...
  /// The budget of the round in progress. Every entry is computed at THIS value
  /// and no other, which is what makes the budget stop being a memo key.
  int _b = 0;

  /// The reach shortcut, DEFAULT OFF because it is unsound as written -- see the
  /// header. Left in place because toggling it is the experiment that proves the
  /// claim: on, JSON repairs cost 4 where the truth is 1; off, m31 agrees with
  /// m26 on every case tried.
  static bool _shortcut = false;
  static void debugShortcut(bool v) => _shortcut = v;
  ...
  /// `reach` is the cost already spent GETTING here, not a budget to spend.
  Map<int, int> _ends(Clause c, int dot, int pos, int reach) {
    if (pos > _n || reach > _b) return const {};
    // REACH EXHAUSTED. Nothing further can be spent arriving at this item, so the
    // only usable answer is the parser's own -- which is m26's `b == 0` fast path,
    // stated in terms of reach instead of budget. Deliberately NOT memoised: this
    // is a shortcut for THIS CALL, not a value of the entry. That distinction is
    // the whole point. m26 memoised it under budget 0 and then had to recompute
    // the same entry at every larger budget it was later asked with.
    if (_shortcut && reach == _b && dot == 0) {
      final m = c.match(_parser, pos);
      return m.isMismatch ? const {} : {pos + m.len: _score(m)};
    }
    final key = (_base(c) + dot) * (_n + 2) + pos;
    return _entries.putIfAbsent(key, _Entry.new).ends(this, c, dot, pos, _b);
  }
  ...
  Map<int, int> _compute(Clause c, int dot, int pos, int b) {
    ...
    if (c is First || c is Optional) {
      ...
      for (final sub in _alts(c)) {
        for (final e in _ends(sub, 0, pos, 0).entries) {
  ...
  Map<int, int> _chain(Clause c, int dot, int pos, int b) {
    ...
    for (final h in _ends(sub, 0, pos, 0).entries) {
      ...
      for (final t in _ends(c, to, h.key, _cost(h.value)).entries) {
    ...
    if (pos < _n && b >= 1) {
      ...
      for (final t in _ends(c, dot, pos + 1, 1).entries) {
  ...
  int recoverCost(String input, {int maxCost = 40}) {
    ...
    for (var k = 0; k <= maxCost; k++) {
      // Every entry is computed at the round budget, so a round's memo is not a
      // filterable superset of the next round's -- it is a value of a different
      // budget and is discarded. m28 established that this costs nothing:
      // sum over b <= K of T(b) is dominated by T(K).
      _b = k;
      _entries.clear();
      _verAtPos = List.filled(_n + 2, 0);
      final best = _ends(_goal, 0, 0, 0)[_n];
      if (best != null) {
        _bestInner = best;
        lastCost = _cost(best);
