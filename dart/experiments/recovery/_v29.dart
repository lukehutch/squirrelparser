// r6.dart -- r5 with the two questions it was answering EAGERLY answered late,
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        // GIVE THE SLOT UP AND CARRY ON. The slot cannot be read where it
        // stands, so the production either moves, resynchronizes past input it
        // does not want, or says this part was never supplied and goes on to
        // the next -- and only the last of those can repair a part that is
        // MISSING rather than surrounded by junk. Priced at the fewest
        // characters any derivation of the slot could have consumed, so it is
        // the same currency as a discard and [_rank] compares them directly.
        {
