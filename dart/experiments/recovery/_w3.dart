// r8.dart -- r6 with the repair its sequences never had: giving one part up.
//
...
//
// And it owes ONE mark for the whole slot, not one per character: the slot is a
// single part that was not supplied, and splitting the claim would describe a
// run of separate omissions that nothing observed. Same score, 1642 -> 1587 ms.
//
      ...
      for (final w in cur) {
        ...
        void giveUp() {
          ...
          if (fill > 0 && fill < _never && w.del + w.gap + fill <= _budget) {
            // ONE mark, priced at what the whole slot would have cost. The slot
            // is a single part that was not supplied, and saying so once is the
            // whole of the claim -- splitting it into one mark per character
            // would describe a run of separate omissions that nothing observed.
            next.add(w.then(_Way.owe(w.end, fill)));
          }
        }
        if (i == 0) {
  ...
  MatchResult recover(String s) {
    ...
    for (_round = 0; _round <= ceiling; _round++) {
      ...
      for (final w in _ways(rules[topRuleName]!, 0)) {
        ...
        // CLAIM THAT IT DID NOT. A reading may not make both: if characters
        // remain that the reading refused to look at, it has not earned the
        // right to say anything is missing -- it should have used them.
        if (tail > 0 && w.gap > 0) continue;
