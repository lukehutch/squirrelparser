// m52 -- recovery is the parser over a wider value, and terminals that may lie,
// relaxed IN PLACE by a worklist over the memo instead of a recursive descent
// through it, and a cell HOLDS the cells it reads instead of looking them up.
//
...
//       This is the LAST thing I8 pays for that the recursive descent did not, and
//       it is the other half of what I9 started.
//
//       I9 removed the allocation per relaxation. What remained was the LOOKUP per
//       READ: every `_read` packed `(node, pos, c)` into an integer and asked a hash
//       map for the cell. m49's descent never did that on its hot edges -- it passed
//       the answer back on the native stack -- and that lookup is most of the
//       per-relaxation constant that made m50 cost 2.0x the battery time for 2.10x
//       the relaxations, and m51 1.29x for 1.74x.
//
//       THE EDGES DO NOT MOVE. Read the three cases: an `_Alt` at `pos` reads its
//       j-th alternative at `pos` under the same obligation, every time. A `_Cons`
//       reads its head at `pos` under the same obligation, every time. And it reads
//       its tail at whatever its head's i-th answer ended, under whatever that
//       answer owed -- which is also every time, because `_keepBest` APPENDS a new
//       key and LOWERS an existing one IN PLACE: the key at index i of a value never
//       changes once written. So a cell's reads are numbered by a slot that depends
//       on the grammar and on the head's answers, and the cell at each slot is the
//       same cell for the life of the search.
//
//       MEASURED BEFORE IT WAS BUILT (`_depprobe.dart`, a probe engine that recorded
//       the cell at each slot and compared): across JSON, left recursion, right
//       recursion and a lookahead grammar, 88226/2026/535/149 reads, of which
//       62397/1257/348/86 -- 58% to 71% -- asked for a cell already resolved at that
//       slot, and ZERO of them resolved to a different cell. So two thirds of the
//       hash lookups in the engine were provably redundant, and the invariant that
//       lets them go is I9's append-only value, which is why this insertion could not
//       have been made before it.
//
//       So a cell HOLDS its dependencies: `_Cell.deps`, indexed by slot. This is the
//       memo's own idea applied one level down -- I1 memoises what a clause DERIVES,
//       and this memoises which cell a clause READS -- and it is m25's `_Entry`
//       lesson, which the m-line has learned once already: resolve an address once
//       and keep the object, never the key.
//
...
class _Cell {
  ...
  /// counts. I10 used to keep the list short by emptying it on every wake; I11's
  /// transpose does better, because one slot per grammar edge means a duplicate
  /// reverse edge can no longer be CONSTRUCTED -- 54742 slots, which is exactly
  /// what the set reached and what emptying by hand did not (133069).
  ...
  /// I12: NOTHING IN MY SUBTREE WAS EVER CUT BY THE BUDGET, so this value is the
  /// COMPLETE fixed point and not merely the fixed point at `budget` -- and then a
  /// deeper round has nothing to ask me. Budget prunes at exactly three sites (a
  /// read past `budget < 0`, and the two `delta >= limit` tests), so "was anything
  /// dropped for want of budget, here or below" is one bit that every relaxation
  /// already has in hand. It is PUBLISHED STATE, so I9's fixed point test covers it
  /// verbatim: a cell whose bit changed has improved and must wake its readers.
  bool complete = false;
}
...
class SuperDot3 {
  ...
  /// I12's bit for the relaxation in progress: did anything get dropped for want of
  /// budget, at any of the four cut sites or in any cell this one read. A field for
  /// the same reason as `_improved`, and safe for the same reason -- no relaxation
  ...
  void _relax(_Cell cell) {
    ...
    _cut = false;
    _compute(cell);
    // I12: the bit is part of what this cell publishes, so a cell that learned it
    // is complete -- or learned it is not, because a child gained an answer whose
    // continuation the budget then dropped -- has changed and must say so. Values
    // reach their fixed point in finitely many relaxations, and once they have, the
    // set of dropped continuations is fixed too, so the bit settles one relaxation
    // later: this adds no cycle to the termination argument.
    if (cell.complete == _cut) {
      cell.complete = !_cut;
      _improved = true;
    }
    if (!_improved) return;
  ...
  /// function twice.
  /// I11: `slot` is which of `from`'s edges this is, or -1 from outside. The budget
  /// test stays in front of the cache: a budget is not part of a cell's identity and
  /// RISES between relaxations, so a slot refused for want of budget once is a slot
  /// that will be asked again.
  List<int> _read(_Cell? from, int slot, _Node node, int pos, int budget, int c) {
    if (pos > _inputLen) return const [];
    // I12, CUT SITE 1 of 4: refusing a read for want of budget is precisely what a
    // deeper round would not refuse.
    if (budget < 0) {
      _cut = true;
      return const [];
    }
    ...
    if (from == null) {
      ...
      cell = deps[slot];
      // I11, SECOND HALF: THE REVERSE EDGE IS THE FORWARD EDGE'S TRANSPOSE, so it is
      // declared HERE -- once, when the forward edge is -- and never again. A forward
      // edge is permanent, so the reverse edge it implies is permanent too, which is a
      // STRONGER invariant than I10's "either queued or listed by every cell it last
      // read", and it needs no clearing to maintain. So I10 is not improved here, it
      // is DELETED: what it was maintaining by hand is now maintained by construction.
      if (cell == null) {
        deps[slot] = cell = _cell(node, pos, c);
        cell.readers.add(from);
      }
    }
    // I12: A DEEPER ROUND RE-ASKS ONLY WHAT WAS CUT. Raising the budget is still
    // the only thing that dirties a settled cell from outside its dependencies, but
    // a cell that was never cut answers the deeper round with the value it already
    // has, so dirtying it is 5.8 rounds of measured waste. Completeness is inherited
    // downward the same way it is defined: a reader of an incomplete cell is
    // incomplete, and a cell with no value yet is the incomplete case par excellence.
    if (!cell.complete) _cut = true;
    final raised = cell.budget < budget;
    if (raised) cell.budget = budget;
    if (cell.value == null || (raised && !cell.complete)) _dirty(cell);
    if (from == null) _run();
  ...
  List<int> _ends(_Node node, int pos, int budget, int c) =>
      List<int>.of(_read(null, -1, node, pos, budget, c));
  ...
  void _compute(_Cell cell) {
    ...
    if (budget == 0) {
      // I12, CUT SITE 0 of 4, AND THE ONE THAT MATTERS MOST: this branch is not a
      // shortcut through the recurrence, it is the recurrence RESTRICTED to repairs
      // that spend nothing -- every edit is dropped, including the mismatch case
      // below, which returns nothing at all. A budget-0 answer is therefore never
      // the complete answer, and a deeper round must re-ask. Finding this cost the
      // gate: without it every cell in the file was pronounced complete at k=0 and
      // the ladder never ran, so every repair came back -1.
      _cut = true;
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        if (alts.length == 1) {
          return _merge(_read(cell, 0, alts[0], pos, budget, c));
        }
        ...
        for (var slot = 0; slot < alts.length; slot++) {
          final ends = _read(cell, slot, alts[slot], pos, budget, c);
          for (var i = 0; i < ends.length; i += 2) {
            final key = ends[i], delta = ends[i + 1];
            if (delta >= limit) {
              _cut = true; // I12, CUT SITE 2 of 4.
              continue;
            }
  ...
  void _chain(_Cell cell, _Cons node) {
    ...
    final heads = _read(cell, 0, node.head, pos, budget, c);
    for (var i = 0; i < heads.length; i += 2) {
      ...
      final rest = _read(cell, 1 + (i >> 1), node.tail, headEnd,
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        for (final alternative in alts) {
          ...
          if (_deltaOf(_read(null, -1, alternative, pos, budget, c), key) != delta) {
            continue;
          }
  ...
  List<MatchResult>? _row(
    ...
    for (final i in order) {
      ...
      final rest =
          _deltaOf(_read(null, -1, node.tail, headEnd, restBudget, headOwed), key);
  ...
  int recoverCost(String input) {
    ...
    for (var k = 0; k <= maxCost; k++) {
      ...
      var best = _impossible;
      final ends = _read(null, -1, goal, 0, k, _free);
      for (var i = 0; i < ends.length; i += 2) {
