// m49 -- recovery is the parser over a wider value, and terminals that may lie.
//
...
class _Entry {
  ...
  /// STILL OWED THERE, packed into one key. Null if nothing has been computed.
  Map<int, int>? endsMap;
  ...
  /// Ends lie in [0, n], obligations in a finite lattice and Deltas are bounded
  /// non-negative integers, so the chain ascends only finitely often and the loop
  /// below terminates.
...
class SuperDot3 {
  ...
  late final int _goalFromNothing = () {
    ...
    int cheapest(bool trustPredicates) {
      ...
      final cost = <int, List<Map<int, int>>>{};
      var improved = true;
      List<Map<int, int>> row(int c) => cost.putIfAbsent(c, () {
            improved = true;
            return List.generate(_nodeCount, (_) => <int, int>{});
          });
      ...
      Map<int, int> chain(_Cons node, int c) {
        final out = <int, int>{};
        // A cons whose tail is itself is a repetition: zero iterations, which emit
        // nothing and so still owe what they were given.
        if (identical(node.tail, node)) out[c] = 0;
        for (final head in row(c)[node.head.id].entries) {
          for (final tail in row(head.key)[node.tail.id].entries) {
            _keepBest(out, tail.key, head.value + tail.value);
          }
        }
        return out;
      }
      ...
      Map<int, int> leaf(_Term node, int c) {
        ...
        for (final c in cost.keys.toList()) {
          for (final node in all) {
            final now = switch (node) {
              ...
              _Alt(:final alts) => () {
                  ...
                  for (final alt in alts) {
                    row(c)[alt.id].forEach((k, v) => _keepBest(out, k, v));
                  }
                  return out;
                }(),
            };
            ...
            for (final e in now.entries) {
              if ((known[e.key] ?? _impossible) > e.value) {
                known[e.key] = e.value;
                improved = true;
              }
            }
      ...
      for (final e in row(_free)[_goal.id].entries) {
        if (_permitsEnd(e.key) && e.value < best) best = e.value;
      }
      return best;
    }
  ...
  /// actually demanded, against the size of the whole cell space. The ratio
  /// answers whether demand-driven descent prunes -- a bottom-up agenda has to
  /// seed the whole space, so if the ratio is near 1 the agenda costs nothing
  /// extra, and if it is small the agenda is strictly more work.
  int get lastCells => _entries.length;
  int get lastSpace =>
      (_nodeCount + 1) * (_inputLen + 2) * (_classes.length + 1);

  /// MEASUREMENT ONLY. Deepest nesting of `_compute` the last call reached.
  /// Against the input length this gives the NESTINGS PER POSITION, and the
  /// native frames per position is that times the frames per nesting
  /// (`_ends` -> `_Entry.ends` -> `_compute` -> `_chain`), which is what decides
  /// whether the stack ceiling can be raised by inlining or has to be removed.
  int _depth = 0;
  int lastMaxDepth = 0;

  /// MEASUREMENT ONLY. Histogram of end-map widths over the cells the last call
  /// settled. The mean is the recompute factor a POSITION-ORDERED WORKLIST would
  /// pay on a cons cell -- one settle event for the head, then one per distinct
  /// tail cell -- so it decides whether the native recursion can be replaced by a
  /// worklist without losing the descent's measured 1.13 computes per cell.
  Map<int, int> get lastWidths {
    final h = <int, int>{};
    for (final e in _entries.values) {
      final w = e.endsMap?.length ?? 0;
      h[w] = (h[w] ?? 0) + 1;
    }
    return h;
  }
  ...
  /// Record `delta` for a key unless a better one is already there.
  static void _keepBest(Map<int, int> out, int key, int delta) {
    if ((out[key] ?? delta) >= delta) out[key] = delta;
  }
  ...
  ///
  /// `_free` is -1, so the debt is shifted by one and the stride is `n + 2`, the
  /// same span the memo uses for a position.
  int _key(int end, int owed) => (owed + 1) * (_inputLen + 2) + end;
  int _endOf(int key) => key % (_inputLen + 2);
  int _oweOf(int key) => key ~/ (_inputLen + 2) - 1;
  ...
  /// EMITTED character lies in class `c` (I6) -- PAIRED WITH THE OBLIGATION STILL
  /// OWED THERE (I7), each mapped to its minimum Delta, given that at most
  /// `budget` edits may be spent. This is `Parser.match`: bounds check, find the
  /// entry, ask it.
  ...
  /// MEASUREMENT ONLY wrapper, so `lastMaxDepth` can be read. No `finally`: a
  /// StackOverflowError abandons the call anyway, and the depth is only read
  /// after a call that returned.
  Map<int, int> _compute(_Node node, int pos, int budget, int c) {
    if (++_depth > lastMaxDepth) lastMaxDepth = _depth;
    final r = _computeInner(node, pos, budget, c);
    _depth--;
    return r;
  }

  Map<int, int> _computeInner(_Node node, int pos, int budget, int c) {
    _steps++;
    ...
    if (budget == 0) {
      ...
      // already there.
      if (m.len == 0) return {_key(pos, c): _cleanRegret(m)};
      return _permitsFirst(c, pos)
          ? {_key(pos + m.len, _free): _cleanRegret(m)}
          : const {};
    }
    switch (node) {
      case _Cons():
      ...
      case _Term(:final editable, :final demands):
        ...
        if (demands != _free) return {_key(pos, _meet(c, demands)): 0};
        final out = <int, int>{};
        final m = node.orig.match(_parser, pos);
        ...
        if (!m.isMismatch) {
          if (m.len == 0) {
            out[_key(pos, c)] = _cleanRegret(m);
          } else if (_permitsFirst(c, pos)) {
            out[_key(pos + m.len, _free)] = _cleanRegret(m);
          }
        }
        ...
        if (!silent && !_permits(c, emits)) return out;
        ...
        if (pos < _inputLen) {
          ...
          _keepBest(
              out, _key(pos + 1, owed), _costUnit + 2 * _skipRegret(pos, pos + 1));
        }
        ...
        _keepBest(out, _key(pos, owed), _costUnit + _widestClass);
        return out;
    }
  }
  ...
  Map<int, int> _chain(_Cons node, int pos, int budget, int c) {
    ...
    for (final head in _ends(node.head, pos, budget, c).entries) {
      final headEnd = _endOf(head.key);
      ...
      final rest = _ends(node.tail, headEnd, budget - _editCount(head.value),
          _oweOf(head.key));
  ...
  MatchResult? _build(
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        for (final alternative in alts) {
          if (_ends(alternative, pos, budget, c)[key] != delta) continue;
  ...
  List<MatchResult>? _row(
    ...
    final order = heads.keys.toList()
      ..sort((a, b) {
        final byEnd = _endOf(a) - _endOf(b);
        return byEnd != 0 ? byEnd : a - b;
      });
    for (final headKey in order) {
      final headEnd = _endOf(headKey);
      if (loops && headEnd == pos) continue;
      final headDelta = heads[headKey]!;
      ...
      final rest = _ends(node.tail, headEnd, restBudget, headOwed)[key];
  ...
  int recoverCost(String input) {
    ...
    for (var k = 0; k <= maxCost; k++) {
      ...
      var best = _impossible;
      _ends(goal, 0, k, _free).forEach((key, delta) {
        if (_endOf(key) == _inputLen &&
            _permitsEnd(_oweOf(key)) &&
            delta < best) {
          best = delta;
          _bestGoalKey = key;
        }
