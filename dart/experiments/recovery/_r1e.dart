...
class Squirrel {
  ...
  /// Instrumentation. [nParse] full parses; [nEval] memo bodies evaluated.
  int nParse = 0, nEval = 0;

  /// Reuse opportunity, measured directly rather than argued. Before each round
  /// the memo of the un-repaired parse is snapshotted; after each candidate
  /// trial the new memo is compared against it entry by entry. [nSame] entries
  /// came out identical -- an incremental scheme could have kept those without
  /// re-deriving them. [nDiff] genuinely changed. [nNew] did not exist before.
  int nSame = 0, nDiff = 0, nNew = 0;

  Map<(Clause, int), (int, bool)>? _snap;

  Map<(Clause, int), (int, bool)> _snapshot() {
    final out = <(Clause, int), (int, bool)>{};
    for (final e in _memo.entries) {
      for (final q in e.value.entries) {
        final r = q.value.result;
        if (r != null) out[(e.key, q.key)] = (r.len, r.isMismatch);
      }
    }
    return out;
  }
  ...
  void _compare() {
    ...
    for (final e in _memo.entries) {
      for (final q in e.value.entries) {
        final r = q.value.result;
        if (r == null) continue;
        final was = base[(e.key, q.key)];
        if (was == null) {
          nNew++;
        } else if (was.$1 == r.len && was.$2 == r.isMismatch) {
          nSame++;
        } else {
          nDiff++;
        }
  ...
  MatchResult? _round(MatchResult root, (int, int) base) {
    // The memo here is exactly the table of the parse that produced `root` --
    // the state a trial repair would be applied ON TOP of. Snapshot it, so each
    // trial's re-derivation can be diffed against what it started from.
    _snap = _snapshot();
    _frontier.clear();
    _seen.clear();
    _walked.clear();
