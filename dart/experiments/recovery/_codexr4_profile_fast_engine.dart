...
class Squirrel {
  ...
  int lookups = 0, expansions = 0, memoHits = 0, pathHits = 0;
  final Map<String, int> lookupsBySite = {};
  final Map<String, int> lookupsByClause = {};
  final Map<int, int> lookupsByBudget = {};

  List<_Way> _ask(String site, Clause c, int pos) {
    lookupsBySite[site] = (lookupsBySite[site] ?? 0) + 1;
    return _ways(c, pos);
  }

  static String _kind(Clause c) => c is Terminal
      ? 'Terminal'
      : c is Ref
          ? 'Ref'
          : c is Seq
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    if (c is Terminal) return _terminal(c, pos);
    if (c is Ref) {
      return _lift(c, pos, _ask('ref-body', rules[c.ruleName]!, pos));
    }
    lookups++;
    final kind = _kind(c);
    lookupsByClause[kind] = (lookupsByClause[kind] ?? 0) + 1;
    lookupsByBudget[_budget] = (lookupsByBudget[_budget] ?? 0) + 1;
  ...
  List<_Way> _expand(Clause c, int pos) {
    if (c is Ref) return _lift(c, pos, _ask('ref-body', rules[c.ruleName]!, pos));
    ...
    if (c is FollowedBy || c is NotFollowedBy) {
      ...
      final full = _budget;
      _budget = 0;
      final ok = _ask('predicate-body', sub, pos).any((w) => w.free);
      _budget = full;
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        final full = _budget;
        final spent = w.del + w.gap;
        if (spent > full) continue;
        _budget = full - spent;
        final here = _ask('seq-slot', sub, w.end);
        _budget = full;
        for (final v in here) {
          ...
          for (var k = pos + 1; k <= pos + _budget && k <= _in.length; k++) {
            ...
            for (final s in c.subClauses) {
              ...
              for (final x in chain) {
                for (final v in _ask('seq-move-probe', s, x.end)) {
                  if (!v.free) continue;
        ...
        for (var k = w.end + 1; k <= w.end + room && k <= _in.length; k++) {
          ...
          final at = _ask('seq-resync-probe', sub, k);
          _budget = full;
  ...
  List<_Way> _first(First c, int pos) {
    ...
    var settled = false;
    for (final s in c.subClauses) {
      final ws = _ask('choice-arm', s, pos);
      for (final w in ws) {
  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    while (frontier.isNotEmpty) {
      ...
      for (final w in frontier) {
        for (final v in _ask('repetition-body', c.subClause, w.end)) {
    ...
    if (all.isEmpty) {
      for (final v in _ask('plus-empty-body', c.subClause, pos)) {
  ...
  List<_Way> _opt(Optional c, int pos) {
    final ws = _ask('optional-body', c.subClause, pos);
    return [
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= ceiling; _budget++) {
      for (final w in _ask('top-round', rules[topRuleName]!, 0)) {
