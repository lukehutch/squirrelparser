// m71 -- THE STAR'S STOP IS AN ORDERED CHOICE'S SECOND BRANCH: m62 verbatim,
// with the one construct that was skipping PEG's commitment put back under it.
//
//   I27 A GREEDY CONSTRUCT IS A COMMITMENT, AND `A*` IS ONE OF THEM. PEG has
//       three greedy constructs and m62 desugars them into two node types:
//       `First` and `Optional` become `_Alt`, and `Repetition` becomes a
//       `_Cons` self-loop. `_Alt` results go through `_mergeAlt`, which holds
//       I3's veto -- at cost 0 the input is unrepaired, so the ORACLE is
//       authoritative about where PEG commits. The self-loop's stop went
//       through `_put` directly and never met that veto. So conformance was
//       never really about the star at all: it was about which of two
//       desugarings a construct happened to land in.
//
//       `A*` is `X <- A X / eps`. Its stop IS an ordered choice's second
//       branch, legal exactly where `A` fails -- and `!A`, for a
//       one-character `A`, is a condition the obligation channel already
//       states: the complement of the head's class, plus the end mark. That
//       is literally what `_looks` computes for `NotFollowedBy`. Meeting it
//       into the stop's obligation makes the stop carry its own proof, and an
//       empty meet means no repaired character can satisfy both, so the stop
//       does not exist.
//
...
// it turns are the two the entire m65-m70 tape line exists to answer.
//
// THE HONEST LIMIT. `_oneCharClass` returns null for a multi-character head,
// and a one-character obligation cannot state `!A` for those, so their stop
// stays free -- m62's behaviour, unchanged and still wrong there. Over-stating
// it (demanding `char not in first(A)`) would be SUFFICIENT for `A` to fail
// but not NECESSARY, and would forbid legal stops. `_starwide71.dart` is the
// gate that measures exactly this gap rather than leaving it to a recollection.
//
...
class SuperDot3 {
      ...
      case _Cons():
        ...
        if (f.pc == 0) {
          if (loops) {
            ...
            if (!_unmeetable(owed)) _put(f, _key(pos, owed), 0, 0);
          }
  ...
  /// I28: A PROOF IS WORTH MORE THAN A TIGHTER RELAXATION, SO ASK FOR THE PROOF
  ...
  /// forbids -- so running without them is a relaxation of running with them:
  /// `costOff <= costOn <= costTrue`. And I5 already says the witness is a
  /// proof, so if the cheap pass's witness verifies, `costOff` is ACHIEVED,
  /// giving `costTrue <= costOff` and pinning all three equal. The guards
  /// cannot change an answer that already carries its own proof; the only
  /// answers they can change are the ones with no proof attached.
  ///
  ...
  /// lower bound and stops the moment a witness attains it.
  bool _guarded = false;
  ...
  MatchResult? _build(
    ...
    // I27: the search's budget-zero walk hands a self-loop's stop forward with
    // `!A` attached, so the key this shortcut has to recognise is the guarded
    // one. A reconstruction that looks for the free key finds nothing.
    final selfLoop =
    ...
    if (!pure.isMismatch &&
        ...
        (pure.len == 0
            ? key ==
                _key(pos,
                    selfLoop == null ? c : _notFirst(selfLoop.head.orig, c))
            : key ==
                    _key(
                        end,
                        selfLoop == null
                            ? _free
                            : _notFirst(selfLoop.head.orig, _free)) &&
                _permitsFirst(c, pos))) {
      return pure;
    }
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        for (var i = 0; i < alts.length; i++) {
          final alt = alts[i];
          final owe = guards == null ? c : _meet(c, guards[i]);
          if (_unmeetable(owe)) continue;
          if (_deltaOf(_ends(alt, pos, budget, owe), key) != (cost, reg)) {
            continue;
          }
          final m = _child(alt, pos, key, cost, reg, budget, owe);
          if (m != null) {
            _path.remove(state);
  ...
  List<MatchResult>? _row(
    ...
    return loops &&
            key == _key(pos, _guarded ? _notFirst(node.head.orig, c) : c) &&
            cost == 0 &&
            reg == 0
        ? const []
        : null;
