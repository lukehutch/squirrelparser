...
class SuperDot3 {
  ...
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    ...
    var at = n;
    if (n > 0) {
      final last = out[n - 4], le = _endOf(last);
      if (le > end || (le == end && last > key)) {
        for (at = 0; at < n; at += 4) {
          final e = _endOf(out[at]);
          if (e > end || (e == end && out[at] > key)) break;
        }
      }
    }
    if (at == n) {
  ...
  static int reconUs = 0;

  MatchResult? _build(_Node node, int pos, int key, int cost, int reg, int c) {
    final sw = Stopwatch()..start();
    final r = _reconstruct(0, node, pos, key, cost, reg, c) as MatchResult?;
    reconUs += sw.elapsedMicroseconds;
    return r;
  }
