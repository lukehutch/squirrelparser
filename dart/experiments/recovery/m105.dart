// m105.dart -- m102 with I65: the SPAN of the damage, `echo - doubt`.
// Ordering under test: span ranks ABOVE doubt and echo.
//
// `doubt` and `echo` are the two ends of the damaged window; neither alone
// says how WIDE it is. Measured on m102, the eight cases it still loses are
// all ones where `doubt` prefers the later first repair and is wrong, while
// the span is right. Span needs no new field -- both ends are already
// carried, and `echo - doubt` is 0 exactly when there is no repair.
//
// NOT prefix-optimal, and that is deliberate: under a repairing suffix the
// span reduces to `-doubt`, under a repair-free one it is the prefix span,
// and no single scalar is right in both branches. `site`-as-runs had the
// same defect. Whether the better approximation is worth it is measured,
// not argued -- see I65 in LESSONS_LEARNED.md.
// Ordering under test: echo ranks BELOW doubt.
//
