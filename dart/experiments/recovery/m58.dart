// m58 -- recovery is the parser over a wider value, SCHEDULED BY COST CLASS:
// the worklist processes whole edit-count strata in order, each stratum ONCE,
// with m53's batched chaotic relaxation inside a stratum -- so the deepening
// ladder, the budget, and the search ceiling stay deleted (I14), and the
// per-fact event grain that made m57 pay on every clean cell is gone too.
//
//   I15 THE CLASS IS THE ROUND, RUN ONCE. m57 scheduled at regret grain: every
//       fact settled at its exact Delta, and every settlement was an event --
//       a heap entry, an announcement, a combine per edge. Exactness only needs
//       CLASSES ordered (cost first, A3); within a class, order is free, and
//       chaotic batch relaxation (m53's) has the smaller constant. So the key
//       is `(costClass << rankBits) | (rankMask - rank)`: Dijkstra across
//       strata, m53's furthest-position-first inside them, one heap. The
//       ladder's rounds are back as buckets -- but each bucket runs once,
//       which is the whole of what I12/I13 proved the ladder could not do.
//
...
class _Cell {
  _Cell(this.node, this.pos, this.c, this.g, this.rank);
  ...
  /// I8's within-class order, `(pos, node.id)` as one integer: furthest position
  /// first, then furthest from the grammar root. Measured in m50: bucketed by
  /// position alone costs 2.63 relaxations per cell against 2.43 with the
  /// tie-break.
  final int rank;

  /// The stratum this cell was last queued at, so an in-class re-dirty is
  /// deduped while queued and re-armed the moment the cell pops. -1 = not queued.
  int lastQueued = -1;
  ...
  /// the exact set of cells that read this one, dirtied at the stratum of each
  /// improvement this cell publishes.
...
class SuperDot3 {
  ...
  /// The distinct strata this relaxation improved -- almost always one or two,
  /// but the set must be EXACT: a stratum dropped here is a reader never
  /// dirtied there, which is a fact the search silently never consumes.
  final List<int> _impStrata = [];

  /// I15's PRODUCTION CUT, which is what remains of m53's `total < limit`: a
  /// combination priced beyond the current stratum is not written -- it is
  /// deferred, and the cell re-dirties ITSELF at the nearest deferred stratum,
  /// so production resumes exactly when the price becomes payable. Without
  /// this the junk chains manufacture the whole O(n^2) skip triangle that no
  /// pop will ever consume (measured: 78M combinations on latency case 8,
  /// 3.6s; with it, production stops at the answer the way popping does).
  int _deferred = 1 << 30;

  void _put(int key, int delta) {
    if (_keepBest(_out, key, delta)) {
      final cls = (_cur.g + delta) >> _costShift;
      if (!_impStrata.contains(cls)) _impStrata.add(cls);
    }
  }
  ...
  /// The key is `(costClass << _rankBits) | (_rankMask - rank)`: strata in cost
  /// order, and m53's furthest-position-first inside a stratum, one comparison.
  final List<int> _qp = [];
  final List<_Cell> _qc = [];

  /// The current stratum: every fact whose total price has this edit count or
  /// less is settled. Advanced only at a stratum boundary.
  int _wClass = 0;

  /// The packing of the two-level key, sized per input at `recoverCost`.
  int _rankBits = 0, _rankMask = 0;

  void _push(int key, _Cell cell) {
    var i = _qp.length;
    _qp.add(key);
    _qc.add(cell);
    while (i > 0) {
  ...
  /// Dirty a cell into stratum `cls`. Deduped while queued for that exact
  /// stratum; popping re-arms it, which is what in-class chaotic iteration
  /// needs. A cell can hold entries in several strata at once -- each is one
  /// relaxation, and a relaxation is idempotent.
  void _dirty(_Cell cell, int cls) {
    if (cell.lastQueued == cls) return;
    cell.lastQueued = cls;
    _push((cls << _rankBits) | (_rankMask - cell.rank), cell);
  }
  ...
  _Cell _cellAt(_Node node, int pos, int c, int g) =>
      _cells.putIfAbsent(_addr(node, pos, c), () {
        final cell = _Cell(node, pos, c, g, pos * (_nodeCount + 1) + node.id);
        if (node.noLook) {
        ...
        // NOT deferred to stratum `class(g) + 1`, though m53's `budget == 0`
        // walk-only leaf argues a seeded cell could be (LESSONS 5i: during its
        // own stratum every derivation through it has an edit-free
        // continuation, so the seed alone is exact there). MEASURED: the defer
        // collapses the small-K cone to m53's (latency case 10: 23.6ms ->
        // 6.5ms) and costs nothing on any reported COST -- but the routes it
        // suppresses are exactly the Delta-TIED witnesses the fully-settled
        // table prefers, so 9 of 252 smoke WITNESS SHAPES diverge, and shape
        // parity is the line's proof standard. The defer is recorded, not
        // shipped.
        _dirty(cell, g >> _costShift);
        return cell;
      });

  /// One relaxation: recompute the cell from its dependencies, writing into the
  /// value already there (I9), then dirty the readers at each stratum this
  /// relaxation improved -- once per (relaxation, stratum), which is the
  /// batching m57's per-fact announcements lacked.
  void _relax(_Cell cell) {
    ...
    for (var s = 0; s < _impStrata.length; s++) {
      final cls = _impStrata[s];
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        if (alts.length == 1) {
          ...
          if (only?.value case final v?) {
            for (var i = 0; i < v.length; i += 2) {
              final cls = (cell.g + v[i + 1]) >> _costShift;
              if (cls > _wClass) {
                if (cls < _deferred) _deferred = cls;
                continue;
              }
              _put(v[i], v[i + 1]);
            }
        ...
        for (var slot = 0; slot < alts.length; slot++) {
          ...
          for (var i = 0; i < ends.length; i += 2) {
            ...
            final cls = (cell.g + delta) >> _costShift;
            if (cls > _wClass) {
              if (cls < _deferred) _deferred = cls;
              continue;
            }
  ...
  void _chain(_Cell cell, _Cons node) {
    ...
    for (var i = 0; i < heads.length; i += 2) {
      ...
      // I14/I15: ONLY A SETTLED HEAD FACT SPAWNS A TAIL -- settled now meaning
      // "in a stratum that has been reached". The budget used to refuse a tail
      // whose context had overspent; the stratum refuses one whose context has
      // not yet been paid for. When the head fact's stratum arrives, its writer
      // dirties this cell there, and the tail is spawned then.
      final headCls = (head!.g + headDelta) >> _costShift;
      if (headCls > _wClass) {
        if (headCls < _deferred) _deferred = headCls;
        continue;
      }
      ...
      for (var j = 0; j < rest.length; j += 2) {
        ...
        final cls = (cell.g + total) >> _costShift;
        if (cls > _wClass) {
          if (cls < _deferred) _deferred = cls;
          continue;
        }
  ...
  /// The minimum settled goal fact that consumes the whole input and owes
  /// nothing the end of the string cannot pay (I7's `_permitsEnd`).
  int _goalScan(_Cell goalCell, int maxClass) {
    final v = goalCell.value;
    if (v == null) return _impossible;
    var best = _impossible;
    for (var i = 0; i < v.length; i += 2) {
      if (v[i + 1] >> _costShift <= maxClass &&
  ...
  int recoverCost(String input) {
    ...
    _rankBits = ((_inputLen + 2) * (_nodeCount + 1)).bitLength;
    _rankMask = (1 << _rankBits) - 1;
    _cells.clear();
    _qp.clear();
    _qc.clear();
    _cleanRegrets.clear();
    ...
    while (_qp.isNotEmpty) {
      // A STRATUM BOUNDARY: everything priced in the strata behind is final,
      // so the goal is read exactly here. The first stratum holding a
      // satisfying fact holds the minimum cost -- and because the whole
      // stratum has settled, every Delta-tied rival has too, which is what
      // makes the witness descent's tie-breaks reproducible.
      if ((_qp.first >>> _rankBits) > _wClass) {
        best = _goalScan(goalCell, _wClass);
        if (best < _impossible) break;
        _wClass = _qp.first >>> _rankBits;
      }
      final cell = _popMin();
      cell.lastQueued = -1;
      _relax(cell);
    }
    // The queue drained with the last stratum unread (or the language answered
    // every stratum): one final scan.
    if (best == _impossible) best = _goalScan(goalCell, 1 << 30);
    if (best < _impossible) {
      _bestGoalDelta = best;
