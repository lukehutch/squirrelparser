// Copy of _isect69.dart with the engine import switched to m70.
...
void main() {
  ...
  print('case                       truth  wit   m65   m68 m69  verdict');
  for (final (name, g, x) in cases) {
    ...
    final c5 = t5.SuperDot3(rules: r, topRuleName: 'S').recoverCost(x);
    final ok = c5 == tr;
    print('${name.padRight(24)} ${tr.toString().padLeft(5)} '
        '${('"$wit"').padLeft(5)} ${c65.toString().padLeft(5)} '
        '${c68.toString().padLeft(5)} ${c5.toString().padLeft(5)}  ${ok ? 'ok' : 'WRONG'} '
