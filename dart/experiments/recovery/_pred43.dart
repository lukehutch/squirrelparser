// Scratch: does m43's "a gap attaches to the next terminal" rule lose repairs
...
void main() {
  ...
  for (final (title, grammar, top, alphabet, inputs) in cases) {
    ...
    print('\n$title   $grammar'.trim());
    print('${'input'.padRight(10)}${'true'.padLeft(6)}${'m42'.padLeft(6)}'
        '${'m43'.padLeft(6)}');
    for (final s in inputs) {
      ...
      print('${(s.isEmpty ? '<empty>' : s).padRight(10)}${want.padLeft(6)}'
          ...
          '  ${c42 == want && c43 == want ? '' : 'MISMATCH'}');
    }
  }
  print('\nwrong: m42=$bad42/$n  m43=$bad43/$n');
}
