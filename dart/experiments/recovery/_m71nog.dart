...
class SuperDot3 {
    ...
    if (budget == 0) {
      if (f.pc == 0) {
        ...
        if (!m.isMismatch) {
          ...
          final loop =
              node is _Cons && identical(node.tail, node) ? node : null;
          if (m.len == 0) {
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        while (f.pc < alts.length) {
          final owe = _meet(c, guards[f.pc]); // I27
          if (_unmeetable(owe)) {
      ...
      case _Cons():
        ...
        if (f.pc == 0) {
          if (loops) {
            final owed = _notFirst(node.head.orig, c); // I27
  ...
  MatchResult? _build(
    ...
    final selfLoop = node is _Cons && identical(node.tail, node) ? node : null;
    if (!pure.isMismatch &&
        pos + pure.len == end &&
        cost == 0 &&
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        final guards = _guardsOf(node); // I27, as in the search
        for (var i = 0; i < alts.length; i++) {
          final alt = alts[i];
          final owe = _meet(c, guards[i]);
  ...
  List<MatchResult>? _row(
    ...
    return loops &&
            key == _key(pos, _notFirst(node.head.orig, c)) && // I27
            cost == 0 &&
            reg == 0
        ? const []
        : null;
