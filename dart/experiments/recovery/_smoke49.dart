// Scratch: m49 against m46 on JSON -- a grammar with NO lookahead anywhere.
//
...
void main() {
  ...
  for (final s in cases) {
    ...
    final e47 = g49.SuperDot3(rules: rules, topRuleName: 'JSON');
    ...
    try {
      ...
      print('m49 THREW on "$s": $e');
      rethrow;
    }
    if (cost46 != cost47) {
      costDiff++;
      if (costDiff <= 8) print('COST  "$s"  m46=$cost46 m49=$cost47');
    }
    if (spans46 != spans47) {
      spanDiff++;
      if (spanDiff <= 8) print('SPANS "$s"  m46=$spans46 m49=$spans47');
    }
    // The strongest form of the zero-cost claim: not merely the same answer, the
    // same number of `_compute` calls to reach it.
    if (steps46 != steps47) {
      stepDiff++;
      if (stepDiff <= 8) print('STEPS "$s"  m46=$steps46 m49=$steps47');
    }
  ...
  print('cases=${cases.length} costDiff=$costDiff shapeDiff=$shapeDiff '
      ...
      'shapeOk46=$shapeOk46 shapeOk49=$shapeOk47');
  print('hist46=$hist46');
  print('hist49=$hist47');
}
