// m117.dart -- I70: a blind fill pays one unit for the character it invents, and the `blind` tie-break level is deleted as redundant.
// Generated by mk117.py from m113.dart.
...
class SuperDot3 {
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    // I70: A FILL PAYS FOR THE CHARACTERS IT INVENTS, NOT ONLY FOR THE ONES IT
    // ASSERTS. A skip's characters are in the input and a DETERMINED fill's are
    // forced by the grammar, so in both cases naming the position names the
    // content. An UNDETERMINED fill's are recoverable from neither: the clause
    // admits many texts and the repair picks one. Under I33 -- cost is the width
    // of the description -- that choice is part of the description, so it is
    // part of the cost. One unit, the same unit every other term is in.
    //
    // This says once, in the field that already means it, what the engine used
    // to say twice and in the wrong place. I54 pruned the fill wherever a
    // strictly shorter skip existed at the same site, which is a LOCAL stand-in
    // for a GLOBAL comparison: measured, the site that wants the fill and the
    // site that wants the skip present the identical triple (Ref, need 1,
    // minSkip 1, no witness), so no local gate can separate them. Raising
    // `blind` in the key instead reorders ways that were never tied on what
    // they assert. Repricing needs neither.
    final fee = need == null ? 0 : (w != null ? need : need + 1);
