...
// -- instrumentation (this copy only) --
int _nLook = 0, _nComb = 0, _nPruneIn = 0, _nRecomp = 0;
...
class Squirrel {
  ...
  // -- instrumentation (this copy only) --
  int nExpand = 0, nWay = 0, nRound = 0, maxCell = 0;
  int get nLook => _nLook;
  int get nComb => _nComb;
  int get nPruneIn => _nPruneIn;
  int get nRecomp => _nRecomp;
  List<int> roundUs = [], roundExpand = [];

  /// Oracle knob: start deepening here instead of at 0. Setting it to the cost
  /// the deepening loop would have reached yields the SAME answer (round `b`
  /// admits every way of cost <= b, so the winning set is identical) while
  /// paying for no round below it -- a lower bound on any best-first design.
  int floor = 0;
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    while (true) {
      ...
      nWay += got.length;
      if (got.length > maxCell) maxCell = got.length;
  ...
  List<_Way> _seq(Seq c, int pos) {
      ...
      for (final w in cur) {
        ...
        if (w.end == _in.length && w.end > pos) {
          ...
          if (owed < _never && w.del + w.gap + owed <= _budget) {
            stops.add(_Way(w.end, w.del, w.gap + owed, w.net, false, null, w,
    ...
    return _prune([
      ...
      for (final w in stops)
        _Way(w.end, w.del, w.gap, w.net, false, _close(c, pos, w)),
    ]);
  }
  ...
  MatchResult recover(String s) {
    ...
    nRound = 0;
    maxCell = 0;
    _nLook = _nComb = _nPruneIn = _nRecomp = 0;
    roundUs = [];
    roundExpand = [];
    final clock = Stopwatch()..start();
    for (_budget = floor; _budget <= ceiling; _budget++) {
      nRound++;
      ...
      for (final w in _ways(rules[topRuleName]!, 0)) {
        ...
        final a = _Way(w.end, w.del + tail, w.gap, w.net, w.peg && tail == 0, w.node);
        if (best == null || _rank(a, best) < 0) best = a;
      }
      roundUs.add(clock.elapsedMicroseconds - t0);
      roundExpand.add(nExpand - e0);
      if (best != null) break;
    }
