...
class SuperDot3 {
  ...
  /// One memo, keyed by the ITEM ITSELF -- (clause, dot, position) -- holding
  /// the budget each entry was computed at alongside the entry. A3 makes the
  /// budget a filter rather than part of the key, so an entry computed at a
  /// larger budget still answers a smaller one.
  final Map<(Clause, int, int), (int, Map<int, int>)> _memo = {};
  MatchResult? _clean;
  ...
  Map<int, int> _ends(Clause c, int dot, int pos, int b) {
    ...
    final hit = _memo[key];
    if (hit != null) {
      if (hit.$1 == b) return hit.$2;
      if (hit.$1 > b) return _filter(hit.$2, b);
    }
    _memo[key] = (b, const {}); // reentrancy guard
    final answer = _compute(c, dot, pos, b);
    _memo[key] = (b, answer);
    return answer;
  }
  ...
  MatchResult _build(Clause c, int pos, int end, int d, int b) {
    ...
    if (c is Ref) {
      return Match(c, pos, end - pos,
          subClauseMatches: [_build(_rules[c.ruleName]!, pos, end, d, b)]);
    }
    ...
    if (c is First || c is Optional) {
      for (final sub in _alts(c)) {
        if (_ends(sub, 0, pos, b)[end] == d) {
          return Match(c, pos, end - pos,
              subClauseMatches: [_build(sub, pos, end, d, b)]);
        }
      }
      if (c is Optional && pos == end && d == 0) return Match(c, pos, 0);
      throw StateError('no alternative of $c reaches $end at $d');
    }
  ...
  List<MatchResult> _descend(
    ...
    if (_more(c, dot)) {
      ...
      for (final he in order) {
        ...
        return [
          _build(sub, pos, he, hv, b),
          ..._descend(c, to, he, end, rest, b - _cost(hv)),
        ];
      }
  ...
  SkipResult recover(String input, {int maxCost = 40}) {
    ...
    if (_bestLead > 0) children.add(SyntaxError(pos: 0, len: _bestLead));
    children
        .add(_build(top, _bestLead, _bestEnd, _bestInner, cost - _bestLead));
    if (_bestEnd < _n) {
  ...
  int recoverCost(String input, {int maxCost = 40}) {
    ...
    for (var k = 0; k <= maxCost; k++) {
      ...
      for (var lead = 0; lead <= k && lead <= _n; lead++) {
        ...
        for (final e in _ends(top, 0, lead, k - lead).entries) {
