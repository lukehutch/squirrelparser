// r7.dart -- r1's architecture, with the half of the alignment its widening
// loop could not reach: the same loop, started one step earlier.
//
...
class Squirrel {
  ...
  /// A candidate repair site: the clause that is stuck, where it is stuck, and
  /// the price of GIVING IT UP -- the fewest characters any derivation of it
  /// could have consumed. Every site offers both sides of the alignment.
  final List<(Clause, int, int)> _frontier = [];

  /// Total price of the repairs committed so far. Every commit pays exactly the
  /// `k` it was offered at -- `k` characters denied, or `k` characters the input
  /// never supplied -- so this is exact, not an estimate, and it is what makes
  /// the floor in [_round] a sound bound.
  int _paid = 0;
  ...
  /// Memo for [_minFill]: a property of the grammar, so it outlives a parse.
  ...
  bool _add(Clause c, int pos) {
    ...
    if (!(_tried[c]?.contains(pos) ?? false) && (_seen[c] ??= {}).add(pos)) {
      _frontier.add((c, pos, _minFill(c)));
    }
    return true;
  }
  ...
  bool _leaf(Clause c, int pos) {
    final deep = _walk(c, pos);
    // THE SHALLOW SIDE. Even when something deeper is stuck, this WHOLE clause
    // can be given up. A partial recovery deep in the tree and a fuller one at
    // a shallower node are the same operation at two depths; only the price
    // separates them. The cost-ordered loop is what makes offering both safe --
    // giving up a large subtree costs its entire [_minFill], so it is never
    // reached while anything cheaper still works.
    final here = _add(c, pos);
    return deep || here;
  }
  ...
  /// THE TWO COSTS ADD, because they are the two sides of one alignment and
  /// both are charged per character. r1 ORDERED them -- every deletion ahead of
  /// every gap, on the argument that the input is primal so no number of gaps
  /// justifies destroying one real character. That argument is sound while a
  /// gap is something the emit falls back to, and wrong once a gap is a repair
  /// the search can CHOOSE: under it a give-up has `del = 0` and beats every
  /// deletion outright, whatever it costs.
  ///
  /// The tie is what the ordering was really protecting, and it survives here as
  /// the second key. `{ a=1; b=2;` reads either as a `Block` whose `'}'` never
  /// arrived, two gaps, or as the statements standing alone with the `{` denied,
  /// two deletions. They cost the same, and the healthy prefix the input
  /// actually supplied should not be thrown away to level the score.
  static bool _cheaper((int, int) a, (int, int) b) =>
      a.$1 + a.$2 != b.$1 + b.$2 ? a.$1 + a.$2 < b.$1 + b.$2 : a.$1 < b.$1;
  ...
  MatchResult? _round(MatchResult root, (int, int) base) {
    ...
    // ONE LOOP OVER THE EDIT COST, OFFERING BOTH SIDES AT EACH COST.
    //
    // `k` is not a span width, it is a PRICE. At `k` the input side denies
    // exactly `k` characters the grammar did not want, and the grammar side
    // gives up exactly one clause that would have wanted `k` characters the
    // input never supplied. Those are the two operations of an edit distance,
    // charged in the same unit, so widening `k` is a uniform-cost search over
    // the whole alignment rather than over its deletion half.
    //
    // Ordering by the price and not by the span is what keeps the two sides
    // comparable. Ordered by span, every give-up is reached at `l = 0` and
    // commits before any deletion is tried at all -- measured, that alone cost
    // junk-insert 0.919 -> 0.878 and delim-insert 0.911 -> 0.853.
    for (var k = 1; k <= _in.length; k++) {
      Clause? bestClause;
      Repaired? bestRepair;
      var bestPos = -1;
      var best = base;

      /// Install [repair], see what the parse then costs, and take it back.
      ///
      /// Taking it back has to take back everything computed with it: the memo
      /// still holds results that read it, and the walk and the emit that
      /// follow would read those instead of the parse as it really is.
      bool offer(Clause c, int p, Repaired repair) {
        (_repairs[c] ??= {})[p] = repair;
        ...
        // THE FLOOR. Both sides pay exactly `k`, so no repair offered at this
        // price can bring the total below what is already paid plus `k`, and
        // reaching it means the whole input parsed with nothing left over. A
        // site that hits the floor is optimal outright, so the rest of the
        // scan cannot improve on it and is not run.
        //
        // This is a bound, not a heuristic, which is the whole difference. The
        // brief's own rule -- commit on the FIRST new match rather than the
        // best -- is the same short-circuit without the bound, and measured it
        // costs 0.8977 -> 0.8394 battery to save 6% latency.
        return got.$1 + got.$2 == _paid + k;
      }

      for (final (c, p, fill) in _frontier) {
        // THE GRAMMAR SIDE. Give the clause up: it contributes nothing here, and
        // the price is one mark per character it would have needed. Nothing is
        // invented and no character is denied, so this is the only repair that
        // is safe at the very end of the input -- which is what makes truncation
        // recoverable at all.
        //
        // It needs no enumeration of the LATER slots of a sequence, which is
        // what the brief asks for and r1 refused as "inventing structure by
        // deleting". Giving slot `i` up lets the ordinary parse reach slot `i+1`
        // at the same position by itself, and a slot that then fails becomes a
        // frontier site of the next round. The brief's rule is this one applied
        // repeatedly -- and this one also reaches the END of the sequence, which
        // the brief's rule cannot.
        if (fill == k) {
          if (offer(
              c,
              p,
              Repaired(p, 0, const [], [
                for (var j = 0; j < k; j++) SyntaxError(pos: p, len: 0),
              ]))) break;
        }
        // THE INPUT SIDE. Deny the `k` characters standing in front of it.
        if (p + k > _in.length) continue;
        ...
        // already refused every clause that matches at `p`.
        final r = _match(c, p + k);
        if (r.isMismatch) continue;
        if (offer(c, p,
            Repaired(p, k + r.len, [r], [SyntaxError(pos: p, len: k)]))) break;
      }
      final win = bestClause;
      if (win == null) continue; // No site helped at this price; pay one more.
      _paid += k;
      _repairs[win]![bestPos] = bestRepair!;
      (_tried[win] ??= {}).add(bestPos);
      return _parse();
    }
    return null;
  }
