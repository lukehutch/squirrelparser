// m145.dart -- m141's chart plus m143's I81, built to separate the two changes
// that the first three rows of the I79-I81 table confounded.
//
// It settles the question: holding I81 constant, adding the chart COSTS 0.0025
// AST-diff and 1.5 perfect points (0.9668/70.6 against m143's 0.9693/72.1) at
// 18x the latency. So the chart's extra reach is negative at zero latency, not
// merely unprofitable at 18x. Like m141 it carries I78 and escapes it -- see
// that file's header and `_recommit.dart` for why a descent-phrased guard cannot
// bind a chart.
...
void _emit(_Way? w, List<MatchResult> out, bool Function(Clause, int) hollow) {
  ...
  if (w.leaf != null) {
    ...
    // I81: PAST THE END OF THE INPUT, THE SLOT NEVER EXISTED.
...
class SuperDot3 {
  ...
  // -------------------------------------------------------------------------
  // I79 (UNDER TEST): THE REPAIR LAYER AS A RIGHT-TO-LEFT SWEEP.
  //
  // PEG defines a FUNCTION, not a relation: every dependency of the meaning of
  // clause `c` at position `p` is either at the same position on a structurally
  // smaller subexpression, or at a position strictly greater than `p`. So
  // evaluating positions from the end of the input backwards, and iterating to
  // a least fixed point WITHIN each position, computes exactly what recursive
  // descent computes -- with no prediction, no origin sets, and no left
  // recursion cycle detection, because a same-position cycle is just the inner
  // fixed point. That is why an Earley recogniser for a PEG is not cubic:
  // ordered choice leaves nothing to be ambiguous about, and the O(n^3) of
  // Earley is entirely the price of ambiguity.
  //
  // The cost-0 layer is deliberately NOT swept. It is the frozen parser, it is
  // already in `_pc`, and it stays true for the whole run because the input is
  // never modified -- which is the payoff of the never-modify rule, and the
  // reason nothing ever needs evicting.

  /// Every distinct clause node: the height of one chart column.
  late final List<Clause> _all = () {
    final seen = HashSet<Clause>.identity(), out = <Clause>[];
    void walk(Clause c) {
  ...
  /// Merge [add] into [into]. Merging, never replacing, is what makes the
  /// iteration monotone and therefore terminating -- see [_fix].
  ...
  /// Fill both repair families for budget [b] over the whole input, bottom-up.
  void _sweep(int b) {
    final mc = _mc[b], me = _me[b];
    for (final c in _all) {
      mc[c] ??= List.filled(_in.length + 1, null);
      me[c] ??= List.filled(_in.length + 1, null);
    }
    for (var pos = _in.length; pos >= 0; pos--) {
      ...
      while (moved) {
        ...
        for (final c in _all) {
          if (c is! Terminal) {
            final cell = mc[c]![pos] ??= _Cell();
            final r = _expand(c, pos);
          ...
          final ec = me[c]![pos] ??= _Cell();
          final r = _repair(c, pos);
          if (_grows(r, ec.ways)) {
            ec.ways = _merge(ec.ways, r);
            moved = true;
          }
          ec.done = true;
        }
  ...
  /// I79: above budget 0 this is a TABLE READ, not a descent. The sweep has
  /// already settled every position greater than [pos], and a same-position
  /// read returns the current iterate, which is exactly chaotic iteration on a
  /// monotone operator.
  _Ways _clause(Clause c, int pos) => c is Terminal
      ? _expand(c, pos)
      : _budget == 0
          ? _fix(_pc, c, pos, _expand)
          : pos > _in.length
              ? _none
              : _mc[_budget][c]?[pos]?.ways;

  /// A clause's ways WITH A LEADING REPAIR allowed -- what a sequence element,
  /// a repetition iteration or an optional body needs.
  _Ways _element(Clause c, int pos) => _budget == 0
      ? _clause(c, pos)
      : pos > _in.length
          ? _none
          : _me[_budget][c]?[pos]?.ways;
