// m90 -- I57: A TIE THE ENGINE CANNOT BREAK IS BROKEN BY THE LINKED LIST.
//
// m89 rewrote `_put` and was expected to be behaviour-identical. It was not:
// `_div` reports 15 diverging cases of 1824, and the cause is worth more than
// the speedup that found it.
//
// It is NOT in `_put`'s arithmetic. One way per ending is still the invariant,
// so which way is the incumbent never depended on list order. It is that
// `_better` is a PARTIAL order. Two ways can tie on all three of cost, net and
// got, and then neither beats the other and whichever arrived FIRST stays --
// arrival order follows list order, and list order is exactly what m89 changed.
// So 0.8% of the battery was being decided by a data-structure artefact, which
// is the standing objection to arbitrary heuristics, now measured rather than
// argued.
//
// The fix is a fourth key that the objective already implies, not a canonical
// list order -- which would only make the arbitrary choice repeatable.
//
// I33 SUPPLIES IT: DESTRUCTION IS THE WIDEST DESCRIPTION. Cost counts
// CHARACTERS, and two readings that discard the same number of characters can
// still make claims of different width: one run of k characters is one position
// and one length, where k scattered single characters are k positions and k
// lengths. Under description-in-bits the scattered reading is strictly the
// wider claim. So at equal cost, equal evidence and equal consumption, the
// engine prefers FEWER REPAIR SITES -- a count, with no parameter in it, and a
// quantity `cost` provably cannot see.
//
// Two fields carry it: `site`, the number of maximal repair RUNS, and `tail`,
// the mirror of `synth` -- whether a way closes with a repair. A run closing
// one way and a run opening the next are the SAME run, which is the only
// arithmetic involved (`_runs`).
//
// It sits LAST among the tie-breaks deliberately, so it decides exactly the
// cases the linked list was deciding and nothing that a stated principle
// already decided.
//
