// m141.dart -- I79: the requested two-mode engine. Top-down until the parse
// fails, then a bottom-up right-to-left chart over every clause x every
// position, relaxed to a within-position fixed point per budget. Derived from
// m132.dart (I78).
//
// REFUTED, and structurally rather than only on latency. The chart is 17.8x
// slower AND scores lower (0.9641 vs m132's 0.9648), because the readings it
// uniquely reaches are worse. `_recommit.dart` names the mechanism: I78 admits a
// repair-opened alternative only where the input witnesses it, which is a
// condition on the DESCENT -- on which alternative may be opened. A chart
// materializes cells with no descent to test, so on `[1,[2,` this file finds the
// cost-2 reading "the whole document is a damaged JSON string" that m132 cannot
// reach at all. It carries I78 and escapes it.
