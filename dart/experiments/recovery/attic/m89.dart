// m89 -- I56: A WAY THAT IS ALREADY NEW DOES NOT NEED TO BE COPIED.
//
// m89 is m88 with `_put` rewritten and nothing else. Every number it produces
// must be identical; only the time moves.
//
...
//
// The list is a SET keyed by ending, so the copied prefix may be reversed --
// nothing reads the order. And `_put` still never mutates a way that exists:
// the only field it writes is `next`, on the way it was just handed. That is
// what makes it safe at the two call sites whose head is a memo cell's own list.
//
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    for (var k = 1; k <= _budget && pos + k <= _in.length; k++) {
      ...
      for (var e = _clause(sub, pos + k); e != null; e = e.next) {
        ...
        out = _put(
            out,
            _Way(e.end, k + e.cost, e.got, e.net, true, a: skip, b: e));
      }
    }
    ...
    if (need != null && need <= _budget && (_witness(sub) != null || need < minSkip)) {
      ...
      out = _put(
          out,
          w != null
              ? _Way(pos, need, 0, 0, true, leaf: Filled(sub, pos, w))
