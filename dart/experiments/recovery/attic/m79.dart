...
class SuperDot3 {
  ...
  _Out? _rule(Clause body, int pos, int b) {
    ...
    while (true) {
      ...
      final better = cell.win == null ||
          r.got > cell.got ||
          (r.got == cell.got && r.cost < cell.cost) ||
          (r.got == cell.got && r.cost == cell.cost && r.end > cell.end);
      if (!better) break;
      cell
        ..win = r.m
        ..cost = r.cost
  ...
  _Out? _first(First f, int pos, int b) {
    ...
    for (final a in f.subClauses) {
      ...
      if (best == null ||
          r.got > best.got ||
          (r.got == best.got && r.cost < best.cost)) {
        best = r;
      }
    }
  ...
  bool _beats(_El a, _El? b) =>
      b == null ||
      a.got > b.got ||
      (a.got == b.got && a.cost < b.cost) ||
      (a.got == b.got && a.cost == b.cost && a.end > b.end);
}
