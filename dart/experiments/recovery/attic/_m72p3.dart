...
class SuperDot3 {
  ...
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    assert(why + 4 >= 0 && why + 4 < (1 << 21));
    final rw = (reg << 21) | (why + 4);
    // Same bisect as m72's, over a 3-stride list: split order is a total order
    // computable from the key alone, so the search for the key bisects and a
    // miss stops exactly where the new key belongs. The stride costs one
    // multiply per probe where m72 shifts, which is the whole question this
    // build exists to answer -- 25% less memory in every answer list, against a
    // slightly dearer probe.
    final end = _endOf(key);
    var lo = 0, hi = out.length ~/ 3;
    while (lo < hi) {
      final mid = (lo + hi) >> 1;
      final k = out[mid * 3];
      final e = _endOf(k);
    ...
    switch (node) {
      ...
      case _Cons():
        ...
        while ((f.pc - 1) * 3 < heads.length) {
          final i = (f.pc - 1) * 3;
          final headKey = heads[i], hCost = heads[i + 1];
          final hReg = heads[i + 2] >> 21;
          ...
          if (rv != null) {
            for (var j = 0; j < rv.length; j += 3) {
              ...
              if (total <= budget) {
                ...
                _put(f, rv[j], total, hReg + (rv[j + 2] >> 21), headKey);
              }
            }
          }
          f.pc++;
  ...
  void _mergeAlt(_Frame f, int altCount, _Entry branch, int ai) {
    ...
    for (var i = 0; i < v.length; i += 3) {
      ...
      _put(f, key, cost, v[i + 2] >> 21, ai); // I29
    }
  }
  ...
  (int, int, int)? _reasonAt(_Node node, int pos, int c, int key) {
    ...
    for (var i = 0; i < v.length; i += 3) {
      if (v[i] == key) {
        return (v[i + 1], v[i + 2] >> 21, (v[i + 2] & ((1 << 21) - 1)) - 4);
      }
    }
    return null;
  }
  ...
  int _pass(String input) {
    ...
    for (var k = 0; k <= maxCost; k++) {
      ...
      for (var i = 0; i < v.length; i += 3) {
        ...
        final r = v[i + 2] >> 21;
        if (v[i + 1] < bestC || (v[i + 1] == bestC && r < bestR)) {
          bestC = v[i + 1];
          bestR = r;
          _goalKey = v[i];
        }
