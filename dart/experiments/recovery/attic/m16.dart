  ...
  Map<int, int> _machine(Clause c, int pos, int b, [int dot0 = 0]) {
    ...
    for (var state = dot0 * width; state < val.length; state++) {
      ...
      for (final sub in _arcs(c, dot)) {
        // Every arc may be preceded by a SPAN of j discarded characters. That is
        // the only place SPAN appears anywhere in the engine.
        for (var j = 0; j <= rem && p + j <= _n; j++) {
          final span = j * _M + 2 * _lost(p, p + j);
          for (final e in _ends(sub, p + j, rem - j).entries) {
            final total = d + span + e.value;
