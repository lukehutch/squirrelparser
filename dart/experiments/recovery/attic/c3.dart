// c3.dart -- c1's judgment on c2's laws, with the rewrite HOISTED (I103):
// no grammar rewriting -- the behaviors the c2 normalization bought are the
// engine's own, so every tree follows the original grammar. The repetition
// closure, the option's two readings, the literal's char-fold and the root
// protocol are behavioral (c1's proven forms); what survives from c2 is
// what ubiquitous growth PROVED: Warth's involved-set instead of the
// per-position version bump (which cold-started whole positions), and the
// vouch symmetry of frozen spans (span - net, what the enumerated judges
// inside would have vouched).
//
// THE WAY-FRONT unifies the sort and the improvement test: one champion
// per ending, decided by rank at insertion; an accepted insert IS the
// improvement signal the grow-loop waits on; ties replace silently (the
// freshest reading holds the bucket, but a tie must never read as
// improvement or rank-equal rivals spin forever); the farthest-PEG claim
// and the budget filter are read-time views. The sort -- and the
// unstable-tie livelock class it carried -- ceases to exist. Two porting
// truths the battery taught: the SEED IS READ RAW (a budget filter at the
// seed hides the repair-carrying ways growth must build on), and re-entry
// IS left recursion (any inPath hit sets foundLR; gating it on a
// null-front misses the first hit once fronts are pre-created).
//
// PARSING MODE IS BUDGET ZERO for EVERY memoized clause, not only rules:
// the library answers composites too, cached in the cell it would have
// occupied. Measured out (2,440 vs 1,535 ms, no accuracy change): growing
// the repetition through its own cell -- the general loop re-expands the
// whole rule per growth step, so the closure keeps its one-pass scheduler,
// the same verdict the c2 rewrite earned.
...
/// The frozen parser's memo entry: the way-front for a result, the budget
/// it was computed under, and Warth's involved set -- the rules that read
...
/// THE WAY-FRONT: one champion per ending, decided by rank at insertion.
/// Insertion IS the improvement test (the grow-loop stops when no add
/// improves), the order is the map's own deterministic iteration (no sort,
/// so no unstable-tie livelock), and the farthest PEG claim and the budget
/// filter are applied at read. One structure serves the memo cells and
/// every transient prune.
class _Front {
...
class Squirrel {
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    if (e.inPath) {
      ...
      e.foundLR = true;
      return e.front?.ways() ?? const [];
    }
    if (e.front != null && e.at >= _budget) {
      final h = _heads[pos];
      if (h == null || !h.contains(c) || e.tick == _tick) {
        return e.front!.ways(_budget);
      }
    }
    if (_budget == 0 && e.front == null) {
      ...
      final f = e.front = _Front();
      e.at = 0;
      final m = c.match(_ref, pos);
      if (!m.isMismatch) {
        final net = _nets[m] ??= _netOf(m);
        f.add(_Way(pos + m.len, 0, 0, net, _peg, vouch: m.len - net, leaf: m));
      }
      return f.ways();
    }
    e.inPath = true;
