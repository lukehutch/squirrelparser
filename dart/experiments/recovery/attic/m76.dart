...
enum _Pk { fail, term, accept, not, seq, choice }
...
class SuperDot3 {
  ...
  final List<_P> _ps = [_P(_Pk.fail)];
  final Map<String, int> _pIndex = {'z': 0};
  ...
  int _norm(int g, int mark, [Set<int>? left]) {
    ...
    switch (q.k) {
      ...
      case _Gk.ref:
        if (!path.add(g)) {
          throw UnsupportedError('left-recursive obligation at grammar state $g');
        }
      ...
      case _Gk.star:
        if (_lambda![q.a]) {
          throw UnsupportedError('zero-width repetition in an obligation');
        }
        final a = _norm(q.a, mark, path);
