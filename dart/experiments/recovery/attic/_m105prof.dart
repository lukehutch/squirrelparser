// _m105prof.dart -- m105 with the search instrumented. Generated by
// mkprof.py. NOT an engine: the counters are global, so it measures one
// process-wide run and nothing else.
//
...
/// Counters, read by _prof105.dart. `work` is the unit of search effort --
/// one offered way -- so `wasted` and `total` are in the same unit and their
/// ratio is the fraction of the search thrown away by re-deriving.
int putCalls = 0, putReplace = 0, consInPut = 0, putScan = 0;
int wastedWork = 0, totalWork = 0, roundsRun = 0, casesRun = 0;
int extendCalls = 0;
List<int> budgetHist = List.filled(28, 0);
List<int> setSize = List.filled(64, 0);
...
_Way _extend(_Way w, _Way x) {
  extendCalls++;
  return _extendReal(w, x);
}

_Way _extendReal(_Way w, _Way x) => _Way(
    x.end,
    w.cost + x.cost,
    w.got + x.got,
    w.net + x.net,
...
class SuperDot3 {
  ...
  MatchResult recover(String s) {
    ...
    final caseStart = putCalls;
    var roundStart = putCalls;
    casesRun++;
    for (_budget = 0; _budget <= cap; _budget = _budget == 0 ? 1 : _budget * 2) {
      ...
      _Way? best;
      final topWays = _element(top, 0);
      var n = 0;
      for (var p = topWays; p != null; p = p.next) {
        n++;
      }
      setSize[n < 63 ? n : 63]++;
      for (var e = topWays; e != null; e = e.next) {
      ...
      if (best != null) {
        // Everything before this round's start was built and discarded.
        wastedWork += roundStart - caseStart;
        totalWork += putCalls - caseStart;
        var b = 0;
        while ((1 << b) <= _budget && b < 27) {
          b++;
        }
        budgetHist[b]++;
        lastCost = best.cost;
