// m60 -- THE MEMO-RESIDENT COROUTINE: m49's recurrence, ladder, budget and
// budget-zero walk -- all three of the budget's measured amortizations -- with
// the native recursion deleted. The continuation lives IN the memo entry.
//
//   I16 THE CONTINUATION IS A MEMO FIELD. m49 paid for right recursion with
//       the native stack (a frame per input position) and m50-m53 bought stack
//       safety with a worklist that re-relaxes cells 1.97 times and pays a heap
//       operation per wake. Neither is necessary. An entry that needs an
//       unsettled child PARKS: it stores itself as the child's parent and the
//       driver runs the child; when the child settles, control returns and the
//       parent's step function re-derives the child it was waiting on from its
//       own cursor, finds it settled, consumes it, and advances. One parent
//       pointer suffices because exactly one chain of computation exists.
//
//       THE CYCLE PROTOCOL IS `MemoEntry`'S, VERBATIM IN A NEW COAT. A request
//       that reaches a RUNNING entry can only be reaching an ancestor of the
//       single chain -- that is what `inRecPath` detected -- so it marks
//       `foundCycle`, takes the provisional value (the left-recursion seed),
//       and continues. When the owner's pass completes with `foundCycle` set
//       and something improved, it bumps the position's memo version and runs
//       another pass, exactly `MemoEntry.match`'s widening loop with the frame
//       replaced by a cursor reset. `running`, `parent`, `foundCycle` are
//       `inRecPath` and `foundLeftRec` generalized from cycles to all waits:
//       the same O(1) descendant-to-ancestor message through the memo, applied
//       to "your operand is ready" as well as "you are a cycle".
//
//       Resuming at the cursor over I9's append-only values is what the
//       fact-grain schedule (m57) lacked: a wide value is consumed once, not
//       rescanned once per fact. A pass that must see lowered prices re-runs
//       whole -- which is the widening loop's job, not the cursor's.
//
// Everything else is the line's settled core, unchanged from m59/m53: the
// curried normal form; I2's lies with deletion as SUB on Nothing; I3's oracle
// veto; I6/I7's obligation channel (down the argument, across the value, up
// the memo); I5's verified witness; A1/A2 pricing with Delta as the pair
// (cost, regret) compared lexicographically; the budget as A3's filter, the
// deepening ladder, the budget-zero oracle walk, and the derived ceiling
// `n + fabricate(goal)` with its predicate tiers (restored from m53 -- the
// bucket drain's ceiling-free termination is m59's luxury, and the fifteenth
// occasion priced it).
//
...
/// One memo entry, and the whole coroutine: value, budget, cursor, and the
/// parked chain. `MemoEntry` held a result and two bits; this holds a frontier
/// and three, plus the program counter the native stack used to be.
class _Entry {
  ...
  /// The budget of the pass currently running.
  int budget = -1;
  ...
  /// `inRecPath`: this entry is on the one live chain.
  bool running = false;

  /// `foundLeftRec`, generalized: a descendant re-entered this entry, so its
  ...
  /// The one parked reader waiting on this entry, or null. One suffices:
  /// exactly one chain runs.
  _Entry? parent;
}
...
class SuperDot3 {
  ...
  void _prepare(_Entry e, int budget) {
    e.running = true;
    e.budget = budget;
    e.pc = 0;
    e.headEntry = null;
  ...
  /// Run `e` to settlement at `budget`, iteratively: the chain of parked
  /// parents lives in the entries, and the native stack never deepens.
  ...
  /// Advance `entry` until it parks on a child (returns the child) or settles
  /// (returns its parked parent, resumed). The parent needs no resume record:
  /// its next step re-derives the awaited child from `pc`, finds it settled,
  /// consumes it, and moves on.
  _Entry? _step(_Entry entry) {
    _steps++;
    ...
    if (budget == 0) {
      if (entry.pc == 0) {
        ...
        if (!m.isMismatch) {
          if (m.len == 0) {
            _put(entry, _key(pos, c), 0, _cleanRegret(m));
          } else if (_permitsFirst(c, pos)) {
            _put(entry, _key(pos + m.len, _free), 0, _cleanRegret(m));
          }
        }
      }
    ...
    switch (node) {
      case _Term(:final editable, :final demands):
        if (demands != _free) {
          _put(entry, _key(pos, _meet(c, demands)), 0, 0);
          return _finish(entry);
        }
        final m = node.orig.match(_parser, pos);
        if (!m.isMismatch) {
        ...
        if (editable) {
          ...
          if (silent || _permits(c, emits)) {
            ...
            if (pos < _inputLen) {
              _put(entry, _key(pos + 1, owed), 1,
                  2 * _skipRegret(pos, pos + 1)); // SUB
            }
            _put(entry, _key(pos, owed), 1, _widestClass); // FAB
          }
        }
        return _finish(entry);
      case _Alt(:final alts):
        while (entry.pc < alts.length) {
          final child = _entryAt(alts[entry.pc], pos, c);
          if (child.running) {
            child.foundCycle = true; // the LR seed: use what it has now
          } else if (!_settled(child, budget)) {
            child.parent = entry;
            _prepare(child, budget);
            return child; // park
          }
          _mergeAlt(entry, alts.length, child);
          entry.pc++;
        }
        return _finish(entry);
      case _Cons():
        ...
        if (entry.pc == 0) {
          if (loops) _put(entry, _key(pos, c), 0, 0);
          final head = _entryAt(node.head, pos, c);
          if (head.running) {
            head.foundCycle = true;
          } else if (!_settled(head, budget)) {
            head.parent = entry;
            _prepare(head, budget);
            return head; // park
          }
          entry.headEntry = head;
          entry.pc = 1;
        }
        final heads = entry.headEntry!.value ?? const <int>[];
        while ((entry.pc - 1) * 3 < heads.length) {
          final i = (entry.pc - 1) * 3;
          ...
          if (rv != null) {
            for (var j = 0; j < rv.length; j += 3) {
              ...
              if (total <= budget) {
                _put(entry, rv[j], total, hReg + rv[j + 2]);
              }
            }
          }
          entry.pc++;
  ...
  void _put(_Entry e, int key, int cost, int reg) {
    if (_keepBest(e.value!, key, cost, reg)) e.improved = true;
  }
  ...
  void _mergeAlt(_Entry entry, int altCount, _Entry branch) {
    ...
    for (var i = 0; i < v.length; i += 3) {
      ...
      if (cost == 0 && altCount > 1) {
        if (committed == -2) {
          final oracle = _parser.match(entry.node.orig, entry.pos);
          committed = oracle.isMismatch ? -1 : entry.pos + oracle.len;
        }
  ...
  /// memos and run another pass; otherwise settle and resume the parked parent.
  _Entry? _finish(_Entry entry) {
    if (entry.foundCycle && entry.improved && entry.budget > 0) {
      _versionAtPos[entry.pos]++;
      entry.pc = 0;
      entry.headEntry = null;
      entry.improved = false;
      return entry; // another widening pass
    }
    entry.running = false;
    entry.settledBudget = entry.budget;
