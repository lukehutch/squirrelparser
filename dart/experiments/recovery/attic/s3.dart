// s3.dart -- s2 collapsed to its pure core. Same cost model, same gates,
// roughly half the mechanism, because everything the engine used to SPECIAL-
// CASE is generated by the machinery it already had:
//
//   * A FILL IS NOT A MECHANISM. A failing terminal already returns its
//     obligation as a way; a sequence composes its slots' obligations; an
//     ordered choice offers its cheapest arm by the rank it already has; a
//     Ref cap names the spine. So the fill of `Value` -- node structure and
//     all -- EMERGES from the ordinary descent, and the explicit give-up
//     branch, the minFill pricing of it, the telescoped stop, and the whole
//     spine builder (I96) are deleted. What kept them alive in r9/s1/s2 was
//     one refusal: [_lift] dropped zero-width ways. One added disjunct admits
//     them, and one build-time rule replaces the spine machinery: a
//     zero-width cap at the end of the input loses its name (I81 -- the
//     construct the writer never reached), and keeps it mid-document (I96).
//   * A LITERAL IS A SEQUENCE (m41's currying, finally applied). A
//     mismatched `Str` folds over its characters through the SAME slot fold
//     as every Seq, so interior denial, partial prefixes and completion --
//     the whole 60-line alignment table of s1/s2 -- emerge from the fold.
//   * A MOVE IS A RESYNC AT SLOT ZERO. One skip rule: where a slot cannot be
//     read as it stands, deny up to the first place it reads freely.
//   * D8'S FEE stays what s2 made it (I72 scoped by I36): an unspellable
//     fill pays one rank-visible toll where a no-dearer denial offered to
//     read instead -- attached in the fold, where both candidates are born.
//
// The memo cell is the frozen parser's entry, unchanged in role: result
// (ways), inRecPath, foundLeftRec, version -- plus the budget it was computed
// under, which is the one recovery-specific fact. The LR loop IS the recovery
// loop (I100): re-descend while the reading improves.
...
class Squirrel {
      ...
      for (final w in cur) {
        ...
        if (!clean) {
          ...
          for (var j = w.end + 1; j <= w.end + room && j <= _in.length; j++) {
            final keep = _budget;
            _budget = 0; // the skip is the whole price
            final at = _ways(sub, j);
            _budget = keep;
            if (at.isEmpty) continue;
            final past = w.then(_Way.skip(w.end, j));
            for (final v in at) {
              if (!v.free) continue;
  ...
  /// only remaining client, since fills now price themselves.
    ...
    if (_fill.isEmpty) {
      ...
      for (var moved = true; moved;) {
        ...
        for (final k in all) {
          int v;
          if (k is Ref) {
            v = _fill[rules[k.ruleName]!]!;
          } else if (k is Seq) {
            v = 0;
            for (final s in k.subClauses) {
              final f = _fill[s]!;
            ...
            v = _never;
            for (final s in k.subClauses) {
              if (_fill[s]! < v) v = _fill[s]!;
            }
          } else if (k is Repetition) {
            v = k.requireOne ? _fill[k.subClause]! : 0;
