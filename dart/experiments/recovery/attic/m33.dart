// m33 -- m32 with the budget COARSENED to powers of two.
//
// The budget an entry is asked with does not have to be exact. Passing a LARGER
// budget down is always sound: the caller filters every combination against its
// own `total < limit`, so a loose budget costs exploration, never correctness.
// m26 exploits none of this -- it passes the exact remaining budget, so a single
// (item, pos) is recomputed once per distinct budget it ever sees, up to K of them.
// That is the whole K^0.47 excess in the computation count (LESSONS_LEARNED 5d).
//
// Rounding the budget UP to the next power of two leaves at most log K distinct
// values per entry instead of K, while budget 0 stays exact -- and budget 0 is the
// one that matters, because it is what cuts the descent and keeps the search local
// to the damage. Locality is kept; the recomputation factor drops from K to log K.
//
// This is only sound because m32 made level 0 COMPLETE. With m26's greedy oracle
// base case, budget 0 is an underapproximation and coarsening would expose it.
//
...
class SuperDot3 {
  ...
  /// Round a budget UP to the next power of two, capped at the round budget. Sound
  /// for any monotone increase (see the header); the cap keeps it from exceeding
  /// what the round can spend, and 0 is left exact because it is the descent bound.
  int _up(int x) {
    if (x <= 0) return x;
    if (x >= _b) return _b;
    var p = 1;
  ...
  Map<int, int> _chain(Clause c, int dot, int pos, int b) {
    ...
    for (final h in _ends(sub, 0, pos, b).entries) {
      ...
      for (final t in _ends(c, to, h.key, _up(b - _cost(h.value))).entries) {
    ...
    if (pos < _n && b >= 1) {
      ...
      for (final t in _ends(c, dot, pos + 1, _up(b - 1)).entries) {
