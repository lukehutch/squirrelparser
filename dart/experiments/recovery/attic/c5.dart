// c5.dart -- THE STANDING ENGINE (I108): c4's judgment exactly -- zero
// differing cases over the whole battery -- with the swallow DERIVED, NEVER
// STORED. c4 accumulated the absorption toll into the way (toll), then
// needed vouch to stop it accumulating twice, a judge to levy it, symmetry
// rules for frozen spans, and a vouch-blind exception at the root: three
// fields and a bug class (the vouch asymmetry cost two full debugging
// sessions) in service of an accumulator. The inversion: a reading that
// absorbed more of its span than it pinned pays one -- computed at every
// comparison, from four scalars the way already carries (end, del, net,
// free), relative to the position of the front doing the comparing, which
// every memo cell and fold stage knows. Idempotent, so double-charging is
// unrepresentable; judged as often as compared and accumulated never.
// toll, vouch, _judge, the capped ate/vouch plumbing, frozen-vouch
// symmetry, and the root's separate swallow arithmetic all cease to exist.
//
// Everything else is c4: the six-key judgment (with the derived swallow in
// the price and vouch's tie-key no longer needed), the tie law, the seed
// exemption on the D8 fee, eof-is-not-spend, the literal replace edit, the
// way-front as the memo cell, Warth's involved set, budget-zero parsing.
...
class _Way {
  ...
  final int end, del, gap, net, key, fees, marks, from;
  ...
  /// more of its span than it pinned pays one -- computed HERE, from
  /// scalars the way already carries, relative to the position of the
  /// front doing the comparing. Nothing accumulates, so a span can never
  /// be charged twice however many levels compare it, and the whole
  /// toll/vouch/judged-once apparatus (three fields, the judge, the
  /// vouch-symmetry bug class) is unrepresentable.
  int ate(int pos) => !free && (end - pos) - del - net > net ? 1 : 0;
  ...
  _Way over(MatchResult n, [int k = _peg]) =>
      _Way(end, del, gap, net, _min(key, k),
          fees: fees, eof: eof, owing: owing, marks: marks, leaf: n);
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    if (c is Ref) {
      ...
      if (_budget == 0 && pos < _n) {
        ...
        return [_Way(pos + r.len, 0, 0, net, _peg, leaf: m)];
      }
    ...
    if (_budget == 0 && e.at < 0 && pos < _n) {
      ...
      if (!m.isMismatch) {
        ...
        e.add(_Way(pos + m.len, 0, 0, net, _peg, leaf: m));
      }
      return e.ways();
    }
    e.inPath = true;
  ...
  MatchResult recover(String s) {
    ...
    for (_round = 1; _round <= ceiling; _round++) {
      ...
      for (final w in _ways(rules[topRuleName]!, 0)) {
        ...
        // and loses its PEG claim with it; the swallow needs no charging
        // here -- the rank derives it (I108).
