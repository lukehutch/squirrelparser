// _m105bf.dart -- m105 counting the ceiling on a best-first schedule.
// Generated by mkprof2.py. NOT an engine: the counters are global.
//
...
/// Counters, read by _bf105.dart.
int allOffers = 0;
int underOpt = 0, overOpt = 0;
List<int> roundCost = List.filled(4096, 0);
...
_Way? _put(_Way? head, _Way v) {
  ...
  allOffers++;
  roundCost[v.cost < 4096 ? v.cost : 4095]++;
  var old = head;
...
class SuperDot3 {
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= cap; _budget = _budget == 0 ? 1 : _budget * 2) {
      ...
      if (best != null) {
        // Split THIS round's offers at the optimum the round returns. A
        // cost-ordered search would have settled everything at or under it and
        // never reached anything above.
        for (var i = 0; i < 4096; i++) {
          if (roundCost[i] == 0) continue;
          if (i <= best.cost) {
            underOpt += roundCost[i];
          } else {
            overOpt += roundCost[i];
          }
        }
