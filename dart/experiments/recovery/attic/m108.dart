// m108.dart -- m102 (seven prefix-optimal keys) with I65's `span`
// applied ONLY to completed readings. See _beatsFinal.
//
// Generated from m102.dart by mk108.py.
//
...
class _Way {
  _Way(this.end, this.cost, this.got, this.net, this.synth,
      {this.site = 0, this.doubt = _never, this.echo = _never,
      this.blind = 0,
...
_Way _cons(_Way w, _Way? next) => _Way(w.end, w.cost, w.got, w.net, w.synth,
    site: w.site, doubt: w.doubt, echo: w.echo, blind: w.blind,
...
bool _better(
        int cost, int net, int got, int blind, int site, int doubt, int echo,
        _Way? b) =>
    b == null ||
    cost < b.cost ||
    (cost == b.cost &&
...
/// I65 AT THE ONLY PLACE IT IS DETERMINED. `span` -- how wide the repaired
/// window is -- ranks above both of its endpoints, but ONLY between ways that
/// already span the whole input. On a prefix the answer is not settled yet: a
/// suffix that repairs replaces the last repair, so the final span is
/// `x.echo - w.doubt`, and the prefix that then wins is the one with the LARGER
/// doubt, not the smaller span. Measured on m105, that conflict is reached 459
/// times per battery and the discarded way had the larger doubt in all 459.
/// Here every prune is settled by [_better], whose seven keys are each
/// prefix-optimal, and `span` decides only among finished readings.
bool _beatsFinal(_Way a, _Way? b) =>
    b == null ||
    a.cost < b.cost ||
    (a.cost == b.cost &&
        (a.net > b.net ||
      ...
      for (var e = _element(top, 0); e != null; e = e.next) {
        ...
        final cand = _Way(s.length, e.cost + trail, e.got, e.net, e.synth,
            ...
            b: trail > 0
                ...
                : null);
        if (cand.cost <= _budget && _beatsFinal(cand, best)) best = cand;
      }
      if (best != null) {
        lastCost = best.cost;
        final kids = <MatchResult>[];
  ...
  _Ways _wrap(Clause c, int pos, _Ways w) {
    ...
    for (var e = w; e != null; e = e.next) {
      out = _Way(e.end, e.cost, e.got, e.net, e.synth,
          site: e.site, doubt: e.doubt, echo: e.echo, blind: e.blind,
