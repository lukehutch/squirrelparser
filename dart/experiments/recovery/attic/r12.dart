// r12.dart -- the brief's own architecture, on a core that keeps its mismatches.
//
...
//
// THE GRAMMAR SIDE (from r11). r10 is the brief taken literally, and it is
...
// supplied. Same unit, one loop, no tuning constant. The give-up is offered at
// the frontier LEAVES only, which is where a missing delimiter lives.
//
// r12 = r11 WITH THE COMMIT RULE REPLACED BY A SCORE. 0.8661 against r11's
// 0.3143. r11 shows the two operations cannot share the brief's first-match-wins
// rule: a give-up supplies what the clause wanted, so it can never fail, and the
// earliest frontier leaf priced l is committed before any other site at that
// price is tried. So the price loop no longer commits on the first success. It
// evaluates EVERY candidate at price l -- install, re-descend, score the whole
// tree, uninstall -- and commits the cheapest, or moves to l+1 if none beats
// doing nothing. Affordable only because stage 4 is a memo-hit continuation:
// 18 us per trial, against a cold re-parse.
//
// The score is (characters denied, obligations given up), lexicographic. The
// three ways that is still wrong -- a partial tree that reaches the end reads as
// free, an ABANDONED obligation is not charged while a GIVEN-UP one is, and a
// `First` salvages by its first arm rather than its furthest -- are r13.
...
class Squirrel {
  ...
  (int, int) _try(Clause c, int p, Repair fix, int n) {
  ...
  /// What a tree costs: characters denied, then obligations left open. Denials
  /// dominate outright -- the input is primal, so no number of missing tokens
  /// justifies throwing away one character the document really contains.
  (int, int) _cost(MatchResult root, int n) {
    _salved.clear();
    final t = root.isMismatch ? _salvage(root) : root;
    if (t == null) return (n + 1, 0);
  ...
  bool _cheaper((int, int) a, (int, int) b) =>
