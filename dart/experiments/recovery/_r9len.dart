...
class Squirrel {
  ...
  List<_Way> _terminal(Clause c, int pos) {
    ...
    // supply the rest at one obligation each. THAT RULE HAS TO REACH `k == 0`
    // TOO. Supplying the literal entirely is an invention and supplying all but
    // one character is a repair, but they are priced in the same unit, and the
    // add-all case used to sit outside this loop at a flat cost of 1 -- so
    // inventing the two characters of `if` cost what restoring one of them
    // cost, and the engine charged the budget for an obligation it then
    // declined to say. `k == 0` is the loop's own last iteration and says both.
    final out = <_Way>[];
    if (c is Str) {
  ...
  int _fillOf(Clause c) {
    ...
    // AND A LITERAL COSTS WHAT ITS CHARACTERS COST, which is the price
    // [_terminal] actually charges to supply one. Left flat, giving up a slot
    // holding `"else"` was quoted at 1 here and billed 4 there.
