// s2.dart -- s1 with the give-up EXCLUSION deleted and D8's reason made
// arithmetic. The standing engine: 0.9841 / 77.0% / ~1,530 ms, every gate
// green, score identical to s1 with the structure honest where s1's was
// lucky.
//
// WHAT CHANGED, AND WHY IT IS THE PRINCIPLED FORM. r9 and s1 offered a
// slot's give-up only where move and resync REACHED nothing -- and "reached"
// is not "useful": on `...2e],t":...` the resync probe finds the quote after
// `t`, goes nowhere downstream, and the honest fill it suppressed (owe the
// OPENING quote, a determined character) was the reading a human wants. The
// exclusion is gone: fills always compete. What keeps b2 honest in its place
// is I72 SCOPED BY I36 -- m143's fee formula, ported to this algebra at
// last: a fill the grammar SPELLS (a quote, a brace, a comma) invents no
// content and rides free; only an UNSPELLABLE fill pays one rank-visible
// toll where a denial no dearer than it offered to read instead. That is
// D8-b2's own sentence ("inventing a character is ridiculous, it could be
// anything; deleting the comma immediately yields a valid list") as
// arithmetic rather than as a suppressed candidate list.
//
// MEASURED against s1 per case: +0.18 / -0.24 case-equivalents -- a wash on
// the battery (0.9841 both), which is itself the finding: the battery
// rarely puts a useless resync in front of a useful fill, but the defect
// class was real (the quote-key swallows) and is now closed by pricing
// rather than by luck. The I97 refinement "only a judged cap vouches" was
// tried here and REVERTED (-0.67): it re-tolled the escape-conjure swallow
// but un-shielded the honest `{"alpha"` completion, and no expressible rule
// separates a free CONSTITUENT from free leaf caps inside a swallow's own
// run. The escape-conjure swallow (an owed `\` reclassifying a real quote,
// cost 1) therefore stands as the named open class -- fee, vouch-refinement
// and prohibition all measured against it, all refuted.
//
// ---------------------------------------------------------------------------
// s1's own account follows, inherited unchanged.
//
// s1 -- r9, rebuilt where the residual said r9 could not say what the
// damage plainly was. The first engine of the residual-driven series: every
// change below was found by itemizing the 474 imperfect battery cases and
// reading them, not by proposing a mechanism and scoring it.
//
// THE PRINCIPLE THE FOUR CHANGES SHARE. A failing parse already writes the
// honest description of the damage into the mismatch tree -- a Str records how
// much of its literal the input supplied, a Seq records the slot that broke,
// a First records every arm -- and the engine's job is to PRICE that
// description (I33: cost is the width of a description) and never trade it for
// a cheaper reading that says less. Each change below is one place r9 could
// not afford, or could not express, the description the failure had already
// written down:
//
// S1-A -- "THE DOCUMENT STOPPED" IS ONE CLAIM (truncations). Obligations at
// the very end of the input are charged ONCE, however many marks they stand
// for ([_Way.eof]): the grammar itself enumerates what is missing, so per-
// character charges said nothing more -- they only made "keep the construct
// the writer started and owe its tail" lose to "close the construct empty and
// DENY what the writer wrote" on every short truncation (`{"a`, `if (`, both
// engines 0.500). Truncate 0.954 -> 0.986, and truncations now answer in
// round 1 instead of round minFill. The marks are still all emitted; the
// claim is priced as one, not silenced. Ranking the mark count as a tiebreak
// was tried in both directions and REFUTED (0.9762 fewer-first / 0.9760
// sign-only vs 0.9770 unranked): how much a stopped reading says was under
// way is a bet on a continuation the evidence cannot decide.
//
// S1-B -- A LITERAL'S INTERIOR CAN BE DENIED (damaged keywords). [_align] is
// now a full three-op alignment (match / owe text / deny input) computed by
// one edit-distance table per Str mismatch, so `fa"se`, `falzse`, `i,f`
// read as their keywords with the junk denied at cost 1-2, where the
// subsequence-only alignment had to let the enclosing sequence destroy the
// whole keyword. The literal's text is grammar-determined, so reading through
// interior junk completes a shape the input witnesses (I36/I78) and no
// character of an absent class is invented (D7). Predicates still see only
// the frozen parse, so conformance is untouched (`0 1 1 0 2 3`, 0 free).
//
// S1-C -- AN OWED SLOT IS STILL A PLACE IN THE TREE (deleted constructs).
// Giving up a mandatory slot mid-document now emits the named spine the
// grammar FORCES for it ([_owedNode]): through Refs, one-or-more bodies, the
// single unfillable-for-nothing element of a sequence, and an ordered choice
// only where a UNIQUE arm is cheapest to satisfy -- one step further would be
// choosing, which is I36's line. `if ()` reads as If(Cond(err)); `[1,[,`
// keeps Value(Number). At the end of the input the node is withheld (I81):
// there the slot is not a construct the damage broke but one the writer never
// reached. Prices are unchanged, so every ranking decision is r9's.
//
// S1-D -- THE TOLL IS A JUDGMENT, AND A SPAN IS JUDGED ONCE ([_judge]). r9's
// swallow toll re-judged the same absorbed characters at every Ref between
// them and the root, so the honest completion of `{"alpha"` -- whose five
// string-content characters sit inside a FREE String subtree the frozen
// parser itself vouches for -- carried two tolls and lost to denying the `{`.
// [_Way.vouch] records absorption already accounted for, by a free subtree or
// by a toll below, and a cap judges only what is fresh. The swallow still
// tolls exactly once (recommit 16/16 stands).
//
...
// chart is never opened (I89 by construction, D1's memo table literally).
// The chart's rounds then start at 1, where round 0 by definition finds
// nothing on damaged input, and the round-0 branch of [_prune] is deleted.
//
// REFUTED HERE, so nobody retries them: budget DOUBLING (1.39x slower -- this
// chart's accumulate-and-merge cells make unit rounds cheap, unlike the
// m-line's budget-indexed tables); the earliest-first-repair key flip
// (0.9799/73.7 -- r4's later-wins rule holds); I59's finality bit (a cell
// that never consulted the cap skips higher rounds -- bought 2% and leaked a
// staleness bug through a missed cut site; the recomputation it deletes was
// never the cost); deleting the markless stop (-0.0002 -- with S1-C emitting
// spine nodes, the stop is the only way left to say "it just ended").
//
// MEASURED (2026-08-05, same battery/session as the baselines): 0.9841 /
// 77.0% perfect / ~1,620 ms battery, against r9's 0.9748 / 74.0 / ~2,040 and
// m143's 0.9693 / 72.1 / ~1,180. Every gate: accept cx2=1 b1=1 b2=1, freespan
// 3 3 4 4 1, recommit 16/16, conf1 0 1 1 0 2 3 with 0 free passes.
//
// STILL OPEN, found by the same residual read and deliberately not knobbed:
// the deleted-opener placement class (~7 case-equivalents: both readings are
// pure-gap cost-1 and [key] puts the owed opener as late as possible --
// `(c)*4` where the document meant `(3+c)*4`, a trailing empty Block where
// the document meant the brace wrapped the statements; no principled
// discriminator found, and the global key flip loses more than it wins).
//
// ---------------------------------------------------------------------------
// r9's own account follows, inherited unchanged.
//
// r9 -- r8 with the one thing a reading may not say: that the document
...
class _Way {
  ...
  /// not silenced.
  _Way.owe(int p, int n, {bool atEof = false, int fee = 0})
...
class Squirrel {
  ...
  ///
  /// The refinement "only a JUDGED cap vouches" was measured and reverted
  /// (-0.67 c-e): it restored the toll on the escape-conjure swallow but took
  /// it from the honest completion of `{"alpha"`, whose free String must
  /// shield its content from the Object cap above. No expressible rule
  /// separates a free CONSTITUENT (the String under Object) from free
  /// per-`Character` caps inside a swallowing String's own run -- that, with
  /// the fee and the prohibition both refuted on the same class, is why the
  /// escape-conjure swallow stands recorded as open.
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        if (i == 0) {
          ...
          // arrived would assert a part that was never reached. `w.end > pos`
          // keeps it from inventing a production that was never entered. This
          // is a shortcut for the give-up chain over the same slots -- and
          // since the chain now emits the grammar-forced spine NODES
          // mid-document, the markless stop is also the only way left to say
          // "it just ended": deleting it was measured at -0.0002.
          final owed = w.end > pos ? _owed(c, i) : 0;
          final atEofS = w.end == _in.length;
          final chgS = atEofS ? (w.eof > 0 ? 0 : 1) : owed;
          if (owed > 0 && owed < _never && w.edits + chgS <= _budget) {
            var x = w;
            for (var j = 0; j < owed; j++) {
              x = x.then(_Way.owe(w.end, 1, atEof: atEofS));
            }
            stops.add(x);
          }
          ...
          for (var k = w.end + 1; k <= w.end + room && k <= _in.length; k++) {
            ...
            for (final v in at) {
              ...
              reached = true;
              denyK = denyK < 0 ? k - w.end : denyK;
            }
            break;
          }
        }
        // THE GIVE-UP COMPETES; IT IS NOT EXCLUDED. r9 offered it only where
        // move and resync reached nothing, and "reached" is not "useful": on
        // `...2e],t":...` the resync probe finds the quote after `t` and goes
        // nowhere, while the fill it suppressed -- owe the OPENING quote, a
        // determined character -- was the honest reading, so the engine fell
        // back to a swallow. The exclusion is replaced by I72 SCOPED BY I36,
        // which is m143's fee ported to this algebra at last: a fill the
        // grammar SPELLS (a quote, a brace) invents no content and rides
        // free; only an UNSPELLABLE fill pays one toll where a denial no
        // dearer than it offered to read instead -- which is exactly D8's b2
        // ("inventing a character is ridiculous, it could be anything") made
        // arithmetic, and it is rank-visible, not budget-spent, like every
        // other toll.
        if (reached && _determined(sub)) denyK = -1;
        ...
        final fee =
            !atEof && denyK > 0 && denyK <= fill && !_determined(sub) ? 1 : 0;
        if (fill > 0 && fill < _never && w.edits + chg <= _budget) {
          ...
          if (spine != null) {
            next.add(w.then(_Way(w.end, 0, fill, 0, w.end,
                toll: fee, leaf: spine, owing: true)));
          } else {
            var x = w;
            for (var j = 0; j < fill; j++) {
              x = x.then(_Way.owe(w.end, 1, atEof: atEof, fee: j == 0 ? fee : 0));
            }
            next.add(x);
          }
        }
