  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    while (frontier.isNotEmpty) {
      ...
      for (final w in frontier) {
        ...
        // THE SAME RESYNCHRONIZATION A SEQUENCE SLOT GETS. Junk between two
        // occurrences is the commonest damage a list suffers, and without this
        // the only way past it is to stop the repetition early -- which is why
        // a stray quote could cost a whole block. Offered only where an
        // occurrence cannot be read where it stands, and only as far as the
        // nearest position where one reads cleanly.
        if (here.any((v) => v.free)) continue;
        final room = _budget - w.del - w.gap;
        for (var k = w.end + 1; k <= w.end + room && k <= _in.length; k++) {
          final full = _budget;
