  ...
  MatchResult _partial(Clause c, int pos) {
    ...
    if (c is Seq) {
      ...
      for (var i = 0; i < c.subClauses.length; i++) {
        ...
        // as a complete-looking parse at cost 0.
        //
        // Emit the required clause itself, empty, holding one mark. That names
        // WHICH obligation is open rather than only how many, and it keeps the
        // children lined up with the grammar's subclauses so index i is still
        // subclause i. No terminal is invented: the node covers no characters
        // and its only content is the mark saying none were supplied.
        if (kids.isNotEmpty) {
          for (var j = open; j < c.subClauses.length; j++) {
            kids.add(Match(c.subClauses[j], curr, 0,
                subClauseMatches: [SyntaxError(pos: curr, len: 0)]));
          }
        }
        break;
      }
