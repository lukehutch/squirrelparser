// c14.dart -- a recovery engine over the library's own grammar, in one file.
//
...
//
// WHAT c14 CHANGES FROM c13. Nothing in the judgment (the battery is
// bit-identical: 0.9899, 85.8% perfect, every gate); two changes to the
// machine, both about how the search behaves on LONG input.
//
//   1. The repetition sweep hands each step only the budget its prefix
//      has not spent -- exactly what the sequence fold already did for
//      its slots. c13 asked every step at the round's whole budget, so a
//      step whose prefix had already spent an edit was explored one rung
//      deeper than any reading through it could afford, and that
//      exploration nested along the document: the recursion depth grew
//      with the input (1,159 frames at 24k characters, a stack overflow
//      at 6k characters with two errors from a cold start), and the
//      wasted fills grew faster than the input. Now the depth is the
//      grammar's nesting (30 frames at every length measured) and a
//      100k-character document with one error is repaired in seconds.
//   2. Best-per-end lists are read through [_Front]: a scan while the
//      list is short (most are one entry), a map from end to index once
//      it is not. On long input a construct with m members holds a
//      reading per member boundary, and the scan was quadratic in m.
//
// The scaling numbers are in LESSONS_LEARNED.md (the c14 round).
//
// WHAT c14 TAKES FROM ITS THREE PARENTS. c9, c10 and c11 each won one axis
// and lost the others; c12 (and this engine) is the synthesis the
// measurements force:
//
...
/// A best-per-end list: readings in first-offer order (the order the
/// tie law depends on), looked up by each reading's own `end`. A short
/// list is scanned -- measured over the battery, 71% of consults see at
/// most one reading and 90% at most three -- and past [scanLimit]
/// entries a map indexes it, because on long input a construct with many
/// members holds a reading per member boundary and the scan was
/// quadratic.
class _Front {
  final List<_Reading> list = [];
  Map<int, int>? _index;
  static const scanLimit = 32;

  int indexOf(int end) {
    final index = _index;
    if (index != null) return index[end] ?? -1;
    for (var i = 0; i < list.length; i++) {
      if (list[i].end == end) return i;
    }
    return -1;
  }
  ...
  void add(_Reading r) {
    ...
    final index = _index;
    if (index != null) {
      index[r.end] = list.length - 1;
    } else if (list.length > scanLimit) {
      _index = {for (var i = 0; i < list.length; i++) list[i].end: i};
    }
  }
}
...
class _RepairCell {
  ...
  /// Best reading per end position, in first-offer order.
  final _best = _Front();
  ...
  bool add(_Reading r) {
    final i = _best.indexOf(r.end);
    var cmp = -1;
    if (i < 0) {
      _best.add(r);
    } else {
      cmp = Squirrel._compare(r, _best.list[i], pos);
  ...
  List<_Reading> _buildView(int budget) {
    ...
    final farthest = Squirrel._farthest(best);
    final out = <_Reading>[];
  ...
  List<_Reading> proposeReadings(Squirrel e, int pos) {
    ...
    while (moved.isNotEmpty) {
      ...
      for (var i = 0; i < moved.length; i++) {
        final r = moved[i];
        // Hand the step only the budget this reading has not spent (as
        // the sequence fold hands its slots): a step is explored at the
        // depth a reading through it can still afford, not the round's
        // whole budget.
        final whole = e._budget;
        e._budget = whole - r.spent;
        ...
        for (var j = 0; j < steps.length; j++) {
          ...
          final slot = best.indexOf(longer.end);
          if (slot < 0) {
            best.add(longer);
            changed.add(best.list.length - 1); // a fresh index, never seen
          } else if (Squirrel._compare(longer, best.list[slot], pos) < 0) {
            best.list[slot] = longer;
            if (!changed.contains(slot)) changed.add(slot);
          }
        }
      }
      moved = [for (var k = 0; k < changed.length; k++) best.list[changed[k]]];
    }
    if (best.list.isEmpty) {
    ...
    return e._bestPerEnd(best.list, pos);
  }
}
...
class Squirrel {
  ...
  static void _keepBest(_Front best, _Reading r, int pos) {
    final i = best.indexOf(r.end);
    if (i < 0) {
      best.add(r);
    } else if (_compare(r, best.list[i], pos) <= 0) {
      best.list[i] = r;
    }
  }
  ...
  List<_Reading> _readSlots(List<Clause> slots, lib.Clause? label, int pos) {
    ...
    for (var s = 0; s < slots.length; s++) {
      ...
      for (var i = 0; i < cur.length; i++) {
        ...
        // The rule prices "something unknowable was missing" against a
        // demonstrated deletion. When the slot is the sequence's LAST, the
        // question of what stood there belongs to the enclosing context --
        // the parent's next slot may want exactly the character the
        // deletion would consume -- and this fold cannot see that far.
        final lastSlot = s == slots.length - 1;
        final penalize =
            deletedAhead > 0 && !lastSlot && !seed && !slot.hasOneShape();
      ...
      if (next.list.isEmpty) return const [];
      cur = next.list;
    }
    final farthest = _farthest(cur);
    final out = <_Reading>[];
