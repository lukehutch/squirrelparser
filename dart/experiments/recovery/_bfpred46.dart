// Scratch: I4 (m46) against BRUTE-FORCE ground truth, next to m45.
//
// `_bfpred45.dart` established that PRED is not "reports a cost that is too
...
//     engine that prices them differently is pricing the spelling. m45 does.
...
void main() {
  ...
  for (final (title, grammar, top, alphabet, inputs) in cases) {
    ...
    for (final s in inputs) {
      ...
      if (!agrees(c46)) totalBad46++;
      var kind = '';
      final n46 = int.tryParse(c46);
      if (!agrees(c46) && n46 != null) {
        kind = n46 < 0
            ? '  LOST'
            : (t == null || n46 < t)
                ? '  UNDER-REPORT'
                : '  too high';
        worse.add('$title / "$s": true $want, m46 $c46 --$kind');
      }
      final mark = agrees(c46)
          ? (agrees(c45) ? 'ok' : 'FIXED by I4')
          : (agrees(c45) ? 'REGRESSED' : 'still wrong');
      print('${(s.isEmpty ? '<empty>' : s).padRight(9)}${want.padLeft(6)}'
          '${c45.padLeft(6)}${c46.padLeft(6)}   $mark$kind');
    }
  }
  print('\nm45: ${total - totalBad45}/$total     m46: ${total - totalBad46}/$total');
  if (worse.isEmpty) {
    print('m46 matches brute force everywhere above');
  } else {
    print('m46 residual (${worse.length}):');
...
void spelling(String title, String asClass, String asPredicate, String alphabet,
  ...
  print('${'input'.padRight(9)}${'true'.padLeft(6)}${'m45[^]'.padLeft(8)}'
      '${'m45!'.padLeft(6)}${'m46[^]'.padLeft(8)}${'m46!'.padLeft(6)}   verdict');
  var bad = 0;
  for (final s in inputs) {
    ...
    final c =
        cell(g46.SuperDot3(rules: classSpelling, topRuleName: 'S').recoverCost, s);
    final d =
        cell(g46.SuperDot3(rules: predSpelling, topRuleName: 'S').recoverCost, s);
    final ok = c == d && agreesWith(t, c);
    if (!ok) bad++;
    print('${(s.isEmpty ? '<empty>' : s).padRight(9)}${want.padLeft(6)}'
        '${a.padLeft(8)}${b.padLeft(6)}${c.padLeft(8)}${d.padLeft(6)}'
        '   ${!ok ? "MISMATCH" : a != b ? "ok -- m45 PRICES THE SPELLING" : "ok"}');
  }
  print(bad == 0
      ? 'm46: both spellings agree, and agree with brute force'
