...
class Squirrel {
  ...
  bool _leaf(Clause c, int pos) {
    ...
    // separates them, and the cost-ordered loop is what makes offering both
    // safe -- giving up a large subtree costs its whole [_minFill], so it is
    // never reached while anything cheaper still works.
  ...
  int nParse=0,nRound=0,nSite=0,nMatch=0,nK=0,maxK=0,nFront=0;
  void _forget() {
    _memo.clear();
    _salvaged.clear();
  }
  ...
  MatchResult? _round(MatchResult root, (int, int) base) {
    ...
    nRound++; nFront += _frontier.length;
    for (var k = 1; k <= _in.length; k++) {
      ...
      void offer(Clause c, int p, Repaired repair) {
        (_repairs[c] ??= {})[p] = repair;
        nParse++;
      ...
      for (final (c, p, fill) in _frontier) { nSite++;
        ...
        offer(c, p,
            Repaired(p, k + r.len, [r], [SyntaxError(pos: p, len: k)]));
      }
