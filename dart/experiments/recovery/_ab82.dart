// _ab81.dart -- m82 vs m82 on the same battery, under the OFFICIAL protocol
// (one engine built once and reused, exactly as final_table.dart does). Shape,
// coverage, failures and the COST SUM must be identical: m82 changes the data
...
void main() {
  ...
  print('=== clean input ===');
  print('  m81 cost ${e0.recoverCost(base)}  shape ok '
      '${treeShape(e0.recover(base)) == origShape}');
  print('  m82 cost ${e1.recoverCost(base)}  shape ok '
  ...
  for (final s in accept) {
    ...
    print('  ${'"$s"'.padRight(14)} m81 $ac ${ra.padRight(20)} '
        'm82 $bc ${rb.padRight(20)} ${ra == rb && ac == bc ? "" : "  <-- DIFFER"}');
  }
  ...
  for (final pass in [0, 1, 0, 1]) {
    ...
    sw.stop();
    print('\n=== ${pass == 0 ? "m81" : "m82"} battery (n=${battery.length}) ===');
