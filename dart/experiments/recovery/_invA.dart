...
class _Way {
  ...
  _Way over(MatchResult n) => _Way(end, del, gap, net, key, n);
...
class Squirrel {
      ...
      for (final w in cur) {
        ...
        if (w.end == _in.length && w.end > pos) {
          ...
          if (owed < _never && w.del + w.gap + owed <= _budget) {
            stops.add(w.then(_Way.owe(w.end, owed)));
          }
        }
        ...
        for (var k = w.end + 1; k <= w.end + room && k <= _in.length; k++) {
          ...
          for (final v in at) {
            if (!v.free) continue;
            next.add(w.then(_Way.skip(w.end, k)).then(v));
          }
          break;
        }
      }
  ...
  List<_Way> _first(First c, int pos) {
    ...
    for (final s in c.subClauses) {
      ...
      for (final w in ws) {
        ...
        out.add((settled ? w.demoted : w)
            .over(_wrap(c, pos, w.end, [w.node!], const [])));
      }
  ...
  List<_Way> _opt(Optional c, int pos) {
    ...
    final zero = _Way.unit(pos).over(Match(c, pos, 0));
    return [
      ws.any((w) => w.peg) ? zero.demoted : zero,
