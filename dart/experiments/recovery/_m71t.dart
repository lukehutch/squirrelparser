...
class SuperDot3 {
  ...
  MatchResult? _build(
    ...
    final r = _reconstruct(0, node, pos, key, cost, reg, budget, c) as MatchResult?;
    ...
    while (st.isNotEmpty) {
      ...
      if (f.kind == 0) {
        ...
        if (f.pc == 0) {
          ...
          final pure =
              f.pos > _inputLen ? mismatch : orig.match(_parser, f.pos);
          // I27: the search's budget-zero walk hands a self-loop's stop
          // forward with `!A` attached, so the key this shortcut has to
          // recognise is the guarded one. A reconstruction that looks for the
          // free key finds nothing.
          final selfLoop =
              _guarded && node is _Cons && identical(node.tail, node)
                  ? node
                  : null;
          if (!pure.isMismatch &&
              f.pos + pure.len == end &&
              ...
              (pure.len == 0
                  ? f.key ==
                      _key(
                          ...
                          selfLoop == null
                              ? f.c
                              : _notFirst(selfLoop.head.orig, f.c))
                  : f.key ==
                          _key(
                              end,
                              selfLoop == null
          ...
          switch (node) {
            case _Term():
              final accepts = f.c == _free ? null : _oneCharClass(orig);
              ret = Match(
                  accepts == null
                      ? orig
            ...
            case _Cons():
              f.pc = 3;
              st.add(
                  _RFrame(2, node, f.pos, f.key, f.cost, f.reg, f.budget, f.c));
          }
          continue;
        }
        if (f.pc == 1) {
          // Alternative `f.ai`, one per visit.
          final alt0 = node as _Alt;
          final alts = alt0.alts;
          if (f.ai >= alts.length) {
          ...
          final guards = _guarded ? _guardsOf(alt0) : null; // I27, as in search
          final alt = alts[f.ai];
          final owe = guards == null ? f.c : _meet(f.c, guards[f.ai]);
          if (_unmeetable(owe) ||
              _deltaOf(_ends(alt, f.pos, f.budget, owe), f.key) !=
                  (f.cost, f.reg)) {
            f.ai++;
            continue;
          }
          f.pc = 2;
          st.add(_RFrame(1, alt, f.pos, f.key, f.cost, f.reg, f.budget, owe));
          continue;
        }
      ...
      if (f.pc == 2) {
        ...
        f.pc = 3;
        st.add(
            _RFrame(1, cons.head, f.pos, headKey, hCost, hReg, f.budget, f.c));
        continue;
      }
      if (f.pc == 3) {
        if (ret == null) {
