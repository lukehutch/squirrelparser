// Scratch: I6 (m47) against BRUTE-FORCE ground truth, next to m46.
//
...
int run(List<(String, String, String, String, List<String>)> block,
  ...
  for (final (title, grammar, top, alphabet, inputs) in block) {
    ...
    for (final s in inputs) {
      ...
      final c47 =
          cell(g47.SuperDot3(rules: rules, topRuleName: top).recoverCost, s);
        ...
        kind = n == null
            ...
            : n < 0
                ...
                : (t == null || n < t)
                    ? '  UNDER-REPORT'
                    : '  too high';
        worse.add('$title / "$s": true $want, m47 $c47 --$kind');
      }
  ...
  print('  block total: m46 ${total - bad46}/$total, m47 ${total - bad47}/$total');
  return total - bad47;
}
...
void main() {
  ...
  if (worse.isEmpty) {
    print('m47 matches brute force everywhere above');
  } else {
    print('m47 residual (${worse.length}):');
...
void spelling(String title, String asClass, String asPredicate, String alphabet,
  ...
  print('${'input'.padRight(9)}${'true'.padLeft(6)}${'m46[^]'.padLeft(8)}'
      '${'m46!'.padLeft(6)}${'m47[^]'.padLeft(8)}${'m47!'.padLeft(6)}   verdict');
  var bad = 0;
  for (final s in inputs) {
    ...
    final c = cell(
        g47.SuperDot3(rules: classSpelling, topRuleName: 'S').recoverCost, s);
    final d = cell(
        g47.SuperDot3(rules: predSpelling, topRuleName: 'S').recoverCost, s);
  ...
  print(bad == 0
      ? 'm47: both spellings agree, and agree with brute force'
