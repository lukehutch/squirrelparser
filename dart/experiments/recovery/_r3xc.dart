...
class Squirrel {
  ...
  /// How many rule nodes the tree derives. A repair that LOWERS this has not
  /// repaired the parse, it has thrown a derivation away -- the dual of
  /// [_invents], and the reason `[1,[2,` must not be re-read as one String.
  int _refs(MatchResult m) {
    var n = m.clause is Ref ? 1 : 0;
    for (final k in m.subClauseMatches) {
      n += _refs(k);
    }
    return n;
  ...
  MatchResult? _round(MatchResult root, (int, int) base) {
    ...
    final held = tree.isMismatch
        ...
        : _explained(tree);
    final baseRefs = tree.isMismatch ? 0 : _refs(tree);
    _frontier.clear();
    _seen.clear();
    _walked.clear();
    if (root.isMismatch) {
    ...
    for (var l = 1; l <= _in.length; l++) {
      ...
      void consider(Clause c, int p, Repaired repair, bool fill) {
        ...
        final shown = got.isMismatch ? _salvage(rules[topRuleName]!, 0) : got;
        final invents = fill && _invents(shown);
        // A repair may extend or embed what the parse already derived; it may
        // not erase it.
        final destroys = !shown.isMismatch && _refs(shown) < baseRefs;
        ...
        _repairs[c]!.remove(p);
        _forget();
        if (!invents && !destroys && _cheaper(cost, best)) {
          best = cost;
          bestClause = c;
          bestPos = p;
          bestRepair = repair;
