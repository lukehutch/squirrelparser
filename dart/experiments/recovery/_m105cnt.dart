// _m105cnt.dart -- m105 with _put instrumented. Generated by mkcnt.py;
// see the note on _beatsNoSpan. NOT an engine: the counters are global,
// so it measures one process-wide run and nothing else.
//
...
/// m102's order: the same eight fields with the `span` level removed. Every
/// key in it is prefix-optimal on its own, so a prune it agrees with is sound.
bool _beatsNoSpan(_Way a, _Way? b) =>
    b == null ||
    a.cost < b.cost ||
    (a.cost == b.cost &&
        (a.net > b.net ||
...
/// Counters, read by _spanprobe.dart. [cmp] is every contested _put, [flip] is
/// the subset where span reversed the verdict, and [lost] is the subset of
/// those where the way span DISCARDED had the strictly larger doubt -- the
/// case that a repairing suffix would have preferred.
int cmpPut = 0, flipPut = 0, lostDoubt = 0;
...
_Way? _put(_Way? head, _Way v) {
  ...
  final full = _beats(v, old), plain = _beatsNoSpan(v, old);
  cmpPut++;
  if (full != plain) {
    flipPut++;
    // Which way did the full order throw away, and did it have more doubt?
    final dropped = full ? old : v, kept = full ? v : old;
    if (dropped.doubt > kept.doubt) lostDoubt++;
  }
  if (!full) return head;
