// m44 -- recovery is the parser over a wider value, and terminals that may lie.
//
...
class SuperDot3 {
  ...
  _Node _node(Clause clause) {
    ...
    if (clause is Seq) {
      ...
      // string, and only the string is being repaired.
      node = _wrap(_term(clause, clause is Terminal), clause);
    }
  ...
  /// and the second pass trusts them -- the same envelope PRED already describes,
  /// and the only assumption in this file. Measured, it is what separates
  /// "reports a cost that is too high" from "reports no repair at all":
  /// `S <- &'x' 'x' / 'y' 'y' 'y' 'y'` on `""` is repairable at 4 (the predicate
  /// can never pass, so the cheap branch is not available at any price), and
  /// trusting the predicate prices the ceiling at 1 and loses it.
  ///
        ...
        for (final node in all) {
          final now = switch (node) {
            _Term(:final editable, :final orig) => editable
                ...
                : orig is Nothing || trustPredicates
