...
//
// ===========================================================================
// m77 / I33 -- DESTRUCTION IS THE WIDEST DESCRIPTION, SO IT IS NOT AN AXIS.
//
// m75's secondary key was the pair (invention, loss): bits asserted, then
// characters destroyed. Two units, ordered because no exchange rate between
// them could be derived. But `loss` was never a second quantity -- it was the
// LIMIT of the first one, measured from the other side.
//
// Read a repair as a description of the input. Every input character the
// repair gives a role to is described by the class that consumed it, and
// costs log2|C| bits: a singleton `'{'` costs nothing (the grammar already
// said which character it was), `[0-9]` costs 3.32, and a class that admits
// nearly everything costs nearly log2|Sigma| -- it barely narrows the
// character down, so it barely describes it. A character the repair DELETES
// gets no class at all, so it costs the most any character can: log2|Sigma|.
// Deletion is not a different kind of harm; it is the widest possible
// reading, the case where the description says nothing whatsoever.
//
//   (edits, invention, description)  lexicographic
//
// This is what fixes B021. m75 repaired `2[,33,true]` at cost 2 by inserting
// two quotes and swallowing eleven characters into a JSON String -- invention
// 0 (quotes are singletons) and loss 0 (nothing destroyed), the best possible
// pair the old key could give, so it was not the tie-break failing to prevent
// that reading, it was the tie-break preferring it. But `Character <- [^"\\]`
// admits 65534 of the 65536 code units, which is 16000 millibits -- the SAME
// number, after rounding, as leaving the character with no role at all.
// Absorbing a character into a JSON string says nothing about it that deleting
// it would not also say. Eleven of them therefore cost eleven deletions, and
// the structural reading, which spends singletons and one `[0-9]` per digit,
// wins by roughly an order of magnitude.
//
// INVENTION STAYS STRICTLY AHEAD, and the reason sharpens: a description can
// be CHECKED, because the caller still holds the input it describes; an
// invention can be checked against nothing. Summing the two is refuted by the
// user's own case: with one total, `[,2,` would be repaired by fabricating a
// digit (3.32 bits) rather than by deleting the stray comma (20.087) -- the
// repair the brief rules out by name. So they stay lexicographic.
//
// The cost is untouched: a secondary key can only choose AMONG repairs that
// already tie on edit count, so no number of edits can move (measured: 0 cost
// differences from m75 across battery, controls and latency cases).
// ===========================================================================
...
/// The alphabet a repair actually draws from. `CharSet.match` reads
/// `input.codeUnitAt(pos)` and returns a match of length 1
/// (`lib/src/parser/terminals.dart:97-113`), so the parser's symbol is one
/// UTF-16 CODE UNIT and there are 2^16 of them. m75 priced an inverted class
/// over `0x110000` code points instead; a supplementary code point never
/// reaches a class as a single symbol, it arrives as two units matched
/// separately, so that count was of things the parser cannot see. The error
/// was invisible while width priced only fabrication -- under I33 it prices
/// every character too, so it is corrected here.
const _alphabetSize = 0x10000;

/// log2 of the alphabet, in millibits: what an unconstrained FAB asserts, and
/// what an input character left with no role at all costs. Derived through the
/// same formula `_width` uses, so the two ends cannot drift apart.
final _widestClass = (math.log(_alphabetSize) / math.ln2 * 1000).round();

// ---- the tie-break, replacing regret --------------------------------------
//
// Among repairs that already tie on EDIT COUNT, two quantities can still
// separate them, and they are asserted in opposite directions:
//
...
//   DESCRIPTION -- bits the repair spends saying what the input already is.
//     A character consumed by class C costs log2|C|, because that is how much
//     narrowing the class did. A character given NO role costs log2|Sigma|:
//     the widest reading there is, which is why deletion needs no axis of its
//     own -- it is the extreme of this one, not a different kind of thing.
//
// They stay ordered rather than summed, and the order is forced rather than
// chosen: a description can be checked against the input the caller still
// holds, while an invention can be checked against nothing. Summing them is
// refuted by measurement, not taste -- with one total, `[,2,` repairs by
// fabricating a digit (3322 millibits) instead of deleting the stray comma
// (16000), which is the repair the brief excludes by name.
//
//   (edits, invention, description)  lexicographic
//
// Packed into the one secondary slot as `invention * descSpan + description`,
// where `descSpan` is one more than the largest description any repair can
// accrue: every input character priced at the widest class.
...
int _width(Clause? clause) {
  ...
  size = clause.inverted ? _alphabetSize - size : size;
...
class SuperDot3 {
  ...
  /// Radix for packing (invention, description): one more than the largest
  /// description, which is every input character priced at the widest class.
  int get _descSpan => _inputLen * _widestClass + 1;

  /// What the oracle's own reading of a subtree costs to describe. The
  /// budget-zero walk settles an entire subtree in ONE step precisely so it
  /// does not have to look inside; pricing it per character would undo that,
  /// so the fold is memoised on the result object the packrat table already
  /// hands back, and each distinct subtree is walked once per input.
  final Map<MatchResult, int> _descCache = Map.identity();

  int _descOf(MatchResult m) {
    final known = _descCache[m];
    if (known != null) return known;
    final kids = m.subClauseMatches;
    if (kids.isEmpty) {
      // A leaf spends its class's width once per character it read: a `Str` is
      // a run of singletons and so is free, a `CharSet` costs its log.
      final c = m.clause;
      return _descCache[m] = c == null ? 0 : _widthOf(c) * m.len;
    }
    var sum = 0;
    for (final k in kids) {
      sum += _descOf(k);
    }
    return _descCache[m] = sum;
  }
        ...
        if (!m.isMismatch) {
          ...
          if (empty || _has(c, _input.codeUnitAt(pos))) {
            ...
            if (owed == base || !_unmeetable(owed)) {
              _put(f, _key(pos + m.len, owed), 0, _descOf(m), _wPure);
            }
          }
        }
      }
    ...
    switch (node) {
      case _Term(:final editable, :final demands):
        ...
        if (!m.isMismatch && (m.len == 0 || _has(c, _input.codeUnitAt(pos)))) {
          _put(f, _key(pos + m.len, m.len == 0 ? c : _free), 0,
              _widthOf(node.orig) * m.len, _wPure);
        }
        ...
        if (silent || _permits(c, emits)) {
          ...
          if (pos < _inputLen) {
            // SUB invents |class| bits, and the input character it drops keeps
            // no role, so it costs the widest description there is. DELETE is
            // this same edge on `_junk`, whose class is empty: it invents 0
            // bits and pays that one widest description.
            _put(f, _key(pos + 1, owed), 1,
                _widthOf(node.orig) * _descSpan + _widestClass, _wSub);
          }
          // FAB invents |class| bits and destroys nothing to describe.
          _put(f, _key(pos, owed), 1, _widthOf(node.orig) * _descSpan, _wFab);
        }
        return _finish(f);
  ...
  int _pass(String input) {
    ...
    _descCache.clear(); // keyed on this parse's results, not on the grammar
    _cells.clear();
    _stack.clear();
    _depth = -1;
    _steps = 0;
