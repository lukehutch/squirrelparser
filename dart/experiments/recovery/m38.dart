...
class _Entry {
  ...
  /// The budget-0 value of this item: what the pure parser alone says here. No
  /// round can change it, because no round can spend an edit inside it. Without
  /// this field the first larger-budget request overwrites `ends_`, and every
  /// later round pays to recover level 0 by filtering a wide map -- which is both
  /// slower AND wider than the one oracle call the level-0 fast path makes.
  Map<int, int>? zero;
...
class SuperDot3 {
  ...
  Map<int, int> _compute(Clause c, int dot, int pos, int b) {
    ...
    // so the pure parser decides this item outright -- at EVERY dot, not just at
    // dot 0. The oracle cannot match a clause's tail, but a tail at budget 0 is
    // only a deterministic left-to-right walk of what remains, one memoised
    // oracle call per element, so `_walk` supplies what the oracle cannot.
  ...
  /// The budget-0 value of an item: a single end, or none. PEG is deterministic
  /// and no edit is affordable here, so there is nothing to search -- the walk
  /// below IS the parser, and its singleton result is also the narrowest possible
  /// operand for every head-by-tail product in `_chain`.
  Map<int, int> _walk(Clause c, int dot, int pos) {
    ...
    var p = pos, s = 0;
    if (c is Seq) {
      for (var d = dot; d < c.subClauses.length; d++) {
        final m = c.subClauses[d].match(_parser, p);
        if (m.isMismatch) return const {};
        p += m.len;
        s += _score(m);
      }
    } else {
      // A Repetition past its first element: zero or more further iterations,
      // taken greedily and cut at zero width, exactly as the parser takes them.
      final sub = (c as HasOneSubClause).subClause;
      while (true) {
        final m = sub.match(_parser, p);
