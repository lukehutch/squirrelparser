// m28 -- m26 with the budget deepened by doubling instead of by one.
// The answer is identical; the repeated work is not. See recoverCost.
//
...
class SuperDot3 {
  ...
  int recoverCost(String input, {int maxCost = 40}) {
    ...
    _scores.clear();
    _steps = 0;
    // DEEPENING BY DOUBLING, not by one.
    //
    // A3 says the budget is a FILTER on Delta, never a memo key. Filtering can
    // only remove repairs costing more than the budget, never add a cheaper one,
    // so the minimum over {cost <= b} is the SAME integer for every b >= the true
    // optimum. Any budget that reaches the optimum therefore reports it exactly,
    // and nothing forces the schedule to visit the budgets in between.
    //
    // Unit deepening visits all of them anyway, so it pays for budgets
    // 0, 1, ... K to learn an answer only the last one produces: a whole factor
    // of K of repeated work, and the reason the measured exponent in damage was
    // 2.4 when a single round is only about 1.4. Doubling makes the total a
    // geometric series dominated by its last term, which is the standard schedule
    // for searching an unbounded range with no knowledge of where the answer is.
    // The factor 2 is not a tuning parameter: any base above 1 gives the same
    // asymptotics, and no value of it can change the answer -- only how much work
    // is done before reaching it.
    // The last rung is clamped to maxCost rather than skipped, so doubling past
    // the cap cannot make a repair that unit deepening would have found look
    // unreachable.
    for (var k = 0;; k = k == 0 ? 1 : k * 2) {
      final b = math.min(k, maxCost);
      final best = _ends(_goal, 0, 0, b)[_n];
      if (best != null) {
        _bestInner = best;
        lastCost = _cost(best);
