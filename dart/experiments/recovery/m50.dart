// m50 -- recovery is the parser over a wider value, and terminals that may lie,
// relaxed by a worklist over the memo instead of a recursive descent through it.
//
...
//       the SEARCH's stack ceiling goes from k=541 to k=2292, a 4.2x gain, which
...
class _Cell {
  ...
  /// so `_dirty` is idempotent), and a re-relaxation re-adds the same readers, so
  /// deduplicating here buys nothing and costs a hash per edge traversed. Measured
  /// on the battery: 713ms as a Set, 650ms as a list, identical step counts.
...
class SuperDot3 {
  ...
  /// `(pos, node.id)` as one integer, so one comparison orders both.
  int _rank(_Cell cell) => cell.pos * (_nodeCount + 1) + cell.node.id;

  void _dirty(_Cell cell) {
    if (cell.queued) return;
    cell.queued = true;
    final rank = _rank(cell);
    var i = _heap.length;
    _heap.add(cell);
    while (i > 0) {
      final parent = (i - 1) >> 1;
      if (_rank(_heap[parent]) >= rank) break;
  ...
  _Cell _pop() {
    ...
    if (_heap.isNotEmpty) {
      final rank = _rank(last);
      var i = 0;
      while (true) {
        var child = 2 * i + 1;
        if (child >= _heap.length) break;
        if (child + 1 < _heap.length &&
            _rank(_heap[child + 1]) > _rank(_heap[child])) child++;
        if (_rank(_heap[child]) <= rank) break;
        _heap[i] = _heap[child];
        i = child;
      }
      _heap[i] = last;
  ...
  _Cell _cell(_Node node, int pos, int c) => _cells.putIfAbsent(
      (c + 1) * _keyStride + node.id * (_inputLen + 2) + pos,
      () => _Cell(node, pos, c));
  ...
  void _relax(_Cell cell) {
    ...
    final known = cell.value;
    if (known != null) {
      var improves = false;
      for (final e in fresh.entries) {
        final was = known[e.key];
        if (was == null || e.value < was) {
          improves = true;
          break;
        }
      }
  ...
  Map<int, int> _read(_Cell from, _Node node, int pos, int budget, int c) {
  ...
  /// READ A CELL FROM OUTSIDE, which the deepening loop and the witness descent
  /// do: demand it, then drain, so the answer is settled rather than partial.
  ...
  Map<int, int> _compute(_Cell cell) {
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        if (alts.length == 1) return _read(cell, alts[0], pos, budget, c);
        ...
        for (final alternative in alts) {
          for (final e in _read(cell, alternative, pos, budget, c).entries) {
  ...
  Map<int, int> _chain(_Cell cell, _Cons node) {
    final pos = cell.pos;
    final budget = cell.budget;
    final c = cell.c;
    final out = <int, int>{};
    ...
    for (final head in _read(cell, node.head, pos, budget, c).entries) {
      ...
      final rest = _read(cell, node.tail, headEnd,
          budget - _editCount(head.value), _oweOf(head.key));
