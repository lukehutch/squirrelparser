...
class _Way {
  const _Way(this.end, this.del, this.gap, this.net, this.key, this.node,
      [this.prev, this.mark]);

  /// Read nothing at [p], perfectly: the identity of the chain product.
  const _Way.unit(int p) : this(p, 0, 0, 0, _peg, null);

  /// Deny the characters in `[from, to)`.
  _Way.skip(int from, int to)
      : this(to, to - from, 0, 0, from, null, null,
            SyntaxError(pos: from, len: to - from));

  /// Owe [n] obligations at [p], having matched nothing for them.
  _Way.owe(int p, int n)
      : this(p, 0, n, 0, p, null, null, SyntaxError(pos: p, len: 0));
  ...
  _Way then(_Way v) => _Way(v.end, del + v.del, gap + v.gap, net + v.net,
      _min(key, v.key), v.node, this, v.mark);
  ...
  _Way over(MatchResult n, [int k = _peg]) =>
      _Way(end, del, gap, net, _min(key, k), n);

  /// No longer PEG's reading -- but still whatever else it was.
  _Way get demoted =>
      _Way(end, del, gap, net, _min(key, _far), node, prev, mark);
}
...
class Squirrel {
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    // rule body's, which is where the cycle is detected and the fixed point
    // actually taken. Between them these are most of the lookups the engine
    // makes, and each was paying for a loop that could run only once.
    if (c is Ref) {
      final r = _prune(_lift(c, pos, _ways(rules[c.ruleName]!, pos)));
      _check(c, pos, r);
      return r;
    }
    if (c is! HasOneSubClause && c is! HasMultipleSubClauses) {
      final r = _prune(_terminal(c, pos));
      _check(c, pos, r);
      return r;
    }
  ...
  static final Map<String, int> viol = {};
  static void _note(String k) => viol[k] = (viol[k] ?? 0) + 1;

  /// Every claim the merged [_Way.key] rests on, checked on every way the chart
  /// ever stores -- not on a sample.
  void _check(Clause c, int pos, List<_Way> ws) {
    for (final w in ws) {
      _note('ways');
      // The merge is only lossless if "costs nothing" and "takes the whole
      // document at face value" really are the same fact.
      if ((w.del == 0 && w.gap == 0) != (w.key >= _far)) {
        _note('BROKEN cost0-iff-key>=far');
      }
      final n = w.node;
      if (n == null) continue;
      final (d, g) = _edits(n);
      if (d != w.del) _note('BROKEN del-is-tree-aggregate');
      if (g != w.gap) _note('BROKEN gap-is-tree-aggregate ${w.gap - g}');
      var first = _far;
      void walk(MatchResult k) {
        if (k is SyntaxError && k.pos < first) first = k.pos;
        k.subClauseMatches.forEach(walk);
      }

      walk(n);
      if (w.key < _far && w.key != first) _note('BROKEN key-is-earliest-error');
      if (w.key >= _far && first < _far) _note('BROKEN clean-but-tree-repaired');
      if ((w.end - pos) - w.del - w.net < 0) _note('BROKEN negative-raw');
    }
  }
  ...
  List<_Way> _lift(Ref c, int pos, List<_Way> ways) => [
        for (final w in ways)
          if (w.net > 0 || w.free || _determined(c))
            w.over(_wrap(c, pos, w.end, [w.node!], const []))
      ];
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        // negative and every slot is looked up to be thrown away.
        if (w.del + w.gap > _budget) continue;
    ...
    return _prune([
      for (final w in [if (whole) ...cur, ...stops, ...moved])
        w.over(_close(c, pos, w))
    ]);
  }
  ...
  List<_Way> _first(First c, int pos) {
    ...
    for (final s in c.subClauses) {
      ...
      for (final w in ws) {
        ...
        out.add(w.over(
            _wrap(c, pos, w.end, [w.node!], const []), settled ? _far : _peg));
      }
  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    while (frontier.isNotEmpty) {
      ...
      for (final w in frontier) {
        // PEG'S OWN READING OF A REPETITION IS THE RUN THAT COULD NOT GO ON.
        // The frozen loop takes another occurrence whenever the subclause has
        // a reading to give it, so a way that stopped while one was still on
        // offer is a shorter run than PEG would have taken -- however clean
        // each of its own occurrences was. Without this the chart keeps every
        // run length at no charge, and a possessive `*` silently backs off.
        var more = false;
        ...
        // Every way reaching here is one the loop expanded: `zero` first, then
        // each pruned frontier -- the same set the closing loop used to collect.
        if (!c.requireOne || w.end > pos) all.add(more ? w.demoted : w);
      }
    ...
    return [for (final w in all) w.over(_close(c, pos, w))];
  }
  ...
  List<_Way> _opt(Optional c, int pos) {
    ...
    return [
      _Way.unit(pos)
          .over(Match(c, pos, 0), ws.any((w) => w.peg) ? _far : _peg),
      for (final w in ws) w.over(_wrap(c, pos, w.end, [w.node!], const []))
    ];
  }
  ...
  List<_Way> _terminal(Clause c, int pos) {
    ...
    if (len >= 0) {
      ...
      return [_Way(pos + len, 0, 0, n, _peg, Match(c, pos, len))];
    }
  ...
  ///
  /// The round at budget 0 IS the frozen parser -- nothing can be filled,
  /// nothing skipped, and no reading accepted that PEG would not itself have
  /// taken -- so an undamaged document is answered by one pure parse and never
  /// opens the chart at all. Each further round raises the ceiling by
  ...
  MatchResult recover(String s) {
    ...
    // Where the top rule cannot be filled to anything however much is
    // supplied it has no finite derivation at all -- a fill tree and a match
    // tree have the same shape -- so there is nothing to deepen TOWARDS and the
    // ceiling sits below the floor, leaving the whole-input error below.
    ...
    for (_budget = 0; _budget <= ceiling; _budget++) {
      for (final w in _ways(rules[topRuleName]!, 0)) {
        ...
        final a = _Way(w.end, w.del + tail, w.gap, w.net,
            tail == 0 ? w.key : _min(w.key, w.end), w.node);
