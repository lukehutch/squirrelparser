...
class SuperDot3 {
  ...
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    // The LINEAR form, kept deliberately. m72 bisects this list, which is only
    // sound because split order is a total order computable from the key alone;
    // under I30 the order of the offers IS the tie-break rule, so a bisect that
    // disagreed with this walk would change the LANGUAGE, not the speed. This
    // scans the WHOLE list rather than stopping where the key would sort, so it
    // does not assume the ordering it is here to check. `_bseq` runs the two
    // against each other over the battery and the latency corpus.
    ...
    for (var i = 0; i < out.length; i += 4) {
      ...
      final e = _endOf(k);
      if (at == out.length && (e > end || (e == end && k > key))) at = i;
    }
    if (at == out.length) {
      out
        ..add(key)
