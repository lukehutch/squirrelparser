  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        _budget = full;
        final free = here.any((v) => v.free);
        if (!free && w.end == _in.length && w.end > pos) {
          final owed = _owed(c, i);
          if (owed < _never && w.del + w.gap + owed <= _budget) {
            var stop = w;
            for (var n = 0; n < owed; n++) {
              stop = _Way(w.end, w.del, w.gap + n + 1, w.net,
                  _min(w.fix, w.end), false, null, stop,
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= ceiling; _budget++) {
      for (final w in _ways(rules[topRuleName]!, 0)) {
        if (w.end == 0 && !w.free && !_determined(rules[topRuleName]!)) {
          continue;
        }
