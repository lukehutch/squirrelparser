// _starwide77.dart -- Codex's counterexample, re-checked against m75 AND m77.
//
// D-A rests on this measurement, and the recorded one was taken on m74, an
// ancestor. Re-taken here on the engines that actually stand, because a defect
// argued from a superseded engine is not evidence about the current one.
//
...
void main() {
  ...
  for (final (g, alpha, inputs, what) in cases) {
    ...
    for (final s in inputs) {
      final c75 = e75.SuperDot3(rules: r, topRuleName: 'S').recoverCost(s);
      final e = e77.SuperDot3(rules: r, topRuleName: 'S');
      final c77 = e.recoverCost(s);
      checked++;
      final mark = empty
          ? (c77 == -1 ? 'ok' : '<-- WRONG (truth -1)')
          : '';
      if (empty && c77 != -1) wrong++;
      stdout.writeln('  ${'"$s"'.padRight(10)}'
          ' m75 ${c75.toString().padLeft(3)}'
          '  m77 ${c77.toString().padLeft(3)}'
          '  verified=${e.lastVerified}  $mark');
    }
  }
  stdout.writeln('\nm77 wrong on $wrong of $checked '
