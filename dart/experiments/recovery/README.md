...
- **`c8.dart` — the engine.** Self-contained: the full squirrel parser is
  ...
  84.0% perfect, ~1.1 s battery, all gates. `c7.dart` is its pre-audit
  twin, kept until c8's latency parity is confirmed on a quiet machine.
  See `LESSONS_LEARNED.md` at the repository root for the yardstick, the
  critical lessons, and the refutation ledger.
