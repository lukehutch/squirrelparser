// m47 -- recovery is the parser over a wider value, and terminals that may lie.
//
...
//
//   I6  A LOOKAHEAD IS A CONSTRAINT ON THE NEXT CHARACTER EMITTED, and it is
//       carried to whoever emits it. This is the flaw I5 reports, repaired.
//
...
//       that emits the character honours it. One number travelling DOWN the
//       recursion, where every other value in this file travels up.
//
//       The channel is the memo key, widened by one field. `_ends(node, pos,
//       budget, c)` is the set of repairs of `node` at `pos` WHOSE FIRST EMITTED
//       CHARACTER LIES IN CLASS c, and the only new rule is how a cons cell splits
//       it: either the head emits that character, or the head emits nothing at all
//       and the tail must -- so
//
//           Cons(h, t) @ c   =   h @ c  x  t @ free    union    h @ silent  x  t @ c
//
//       and everything else is unchanged. THE EMPTY CLASS IS THE SILENT
//       CONSTRAINT: "the first character emitted lies in {}" is exactly "nothing is
//       emitted", so `silent` needs no representation of its own, no second value,
//       and no bit anywhere. It falls out of the same intersection lattice the
//       classes already live in -- and a lookahead whose class intersects to empty
//       is likewise not a cheap repair but a promise of silence.
//
//       That single equation is the run-time fact `_nullseq45` says a static
//       rewrite cannot have. For `S <- !'x' A B; A <- 'a'?; B <- 'b' / 'x';` the
//       left branch prices A emitting the constrained character and B free; the
//       right prices A emitting nothing and B constrained; the minimum over both
//       IS the answer the two static placements each got half right. Measured: "x"
//       costs 1 here and 2 in m46, "xx" costs 1 here and 3 in m46, and the truth
//       (brute force over the pure parser) is 1 for both.
//
//       WHAT PAYS FOR IT: a memo dimension the size of the intersection lattice of
//       the grammar's one-character lookaheads, plus `free`. A grammar without
//       lookahead has a lattice of one and every key below is m46's, bit for bit --
//       which is why the JSON battery does not move by a single tree.
//
//       I6 STOPS WHERE THE CHAIN DOES. A constraint still pending when a sequence
//       ends has escaped past the last element, and the value has no room to carry
//       it into the caller's frame -- that would be a constraint travelling UP, and
//       up is where the Pareto front starts. `_end` therefore discharges whatever
//       reaches it, which is m46's answer for exactly the case m46 had: a trailing
//       lookahead (`Kw <- "if" !Alpha`) and a tail that emits nothing. Optimistic,
//       never lost, and still reported by I5.
//
//       A lookahead WIDER than one character is untouched for the same reason it
//       always was: its class is not a class. The derivative of `!"*/"` after one
//       emitted character is `!"/"`, a constraint that changes as it is discharged,
//       and this channel carries a set, not a state machine.
//
...
class _Cons extends _Node {
  ...
  /// I6: the class a lookahead in this cell's head position demands of the next
  /// character anyone emits. `_free` when there is none, which is every cell in a
  /// grammar without lookahead.
  int post = SuperDot3._free;
}
...
class _Entry {
  ...
  Map<int, int> ends(
      SuperDot3 engine, _Node node, int pos, int budgetWanted, int c) {
    if (inRecPath) {
    ...
    var first = true;
    while (true) {
      final fresh = engine._compute(node, pos, budgetWanted, c);
...
class SuperDot3 {
  ...
  ///
  /// I6: IT ENFORCES A PENDING CONSTRAINT RATHER THAN DISCHARGING IT. Arriving
  /// here means the derivation emitted nothing, and `_permits` reads a nonempty
  /// class as "something has to be emitted", so a nullable run cannot satisfy a
  /// lookahead by matching nothing. m47 kept a SECOND node here that discharged
  /// optimistically -- on the argument that a constraint reaching the end of a
  /// sequence belongs to the parent, whose continuation this value does not
  /// carry -- and `_leak47` measured what that costs: 0 where the truth is 1, an
  /// UNDER-REPORT, the one direction that is not safe. Enforcing loses nothing,
  /// because a head that emits nothing is already counted in the OTHER branch of
  /// `_split`; and a constraint that could arrive here at all is one `_cons`
  /// refuses to post in the first place.
  ...
  // ---- I6: the constraint lattice ------------------------------------------
  //
  // A constraint is a class of code units, and it means "the first character
  // this derivation emits is one of these". Two of them are special and NEITHER
  // is a special case in the code:
  //
  //   `_free`    -- no constraint. The top of the lattice, and the only value a
  //                 grammar without lookahead ever uses.
  //   `_silent`  -- the EMPTY class, index 0. "The first character emitted is a
  //                 member of {}" has exactly one reading: nothing is emitted.
  //                 The bottom of the lattice, reached by intersection like any
  //                 other element, which is why a lookahead that contradicts one
  //                 already pending demands silence rather than failing.
  //
  // Meet is intersection. Classes are interned so the memo key can be an integer
  ...
  static const int _silent = 0;

  /// Interned classes, and the empty one is seeded because it IS `_silent`: index
  /// 0 has to be the empty class rather than whichever class the grammar happens
  /// to mention first.
  final List<List<(int, int)>> _classes = [const []];
  final Map<String, int> _classIndex = {'': _silent};
  ...
  /// May a move that emits `emits` -- null or empty meaning it emits nothing --
  /// happen under constraint `c`? Emitting nothing satisfies only the empty
  /// class, and that is the whole of the silent case.
  bool _permits(int c, List<(int, int)>? emits) => c == _free ||
      (emits == null || emits.isEmpty
          ? _classes[c].isEmpty
          : _intersect(emits, _classes[c]).isNotEmpty);

  /// The same question for a MATCH, which emits the input it consumed -- so the
  /// character in question is the one already there.
  bool _permitsMatch(int c, int pos, int len) => c == _free ||
      _permits(
          c,
          len == 0
              ? null
              : [(_input.codeUnitAt(pos), _input.codeUnitAt(pos))]);

  /// I6's ONE new rule, and the whole of the channel. The cell's constraint binds
  /// the first character the CELL emits; the head either emits it, or emits
  /// nothing and hands the constraint to the tail. `_free` is both at once, and
  /// `_silent` demands silence of both halves.
  ///
  /// The first branch does NOT have to say "and the head must emit something":
  /// under a nonempty `c` a silent head fails `_permits` wherever it bottoms out,
  /// so the branch admits only heads that emit -- provided nothing discharges a
  /// pending constraint for free, which is exactly the invariant `_eps` and
  /// `_cons` keep between them.
  static List<(int, int)> _split(int c) => c == _free
      ? const [(_free, _free)]
      : c == _silent
          ? const [(_silent, _silent)]
          : [(c, _free), (_silent, c)];
  ...
  /// reader beside it when there is one; I6 carries it as a constraint when there
  ...
  /// CAN THIS CLAUSE EMIT NOTHING? Not "can the parser match it empty" by
  /// accident of phrasing -- the two coincide, and that is the point. Both of
  /// I2's lies emit what the terminal accepts (SUB in place of a character, FAB
  /// in place of none), and a skipped run is `Nothing` repeated, which accepts
  /// the empty string. So no repair can make a clause that must read a character
  /// emit none, and the ordinary nullability of the grammar answers the question
  /// exactly.
  ///
  /// `_cons` needs this in one direction only: it posts a constraint when the
  /// answer is NO. Saying "yes" when the truth is no merely keeps a predicate
  /// node -- m46's behaviour, safe. The `_ => true` fallback is therefore the
  /// side to be wrong on, and every case above it is exact.
  bool _nullable(Clause clause) => _emitsNothing(clause,_nullableRules);

  /// The rules that can match empty: the least fixed point, which is the only
  /// reading that makes a rule referring to itself non-nullable through that
  /// reference. Sweeping to a fixed point rather than recursing with a `seen`
  /// set also keeps this LINEAR -- `_oneCharClass`'s copied guard set would
  /// re-expand a shared subtree once per path to it.
  late final Set<String> _nullableRules = () {
    final out = <String>{};
    for (var improved = true; improved;) {
      improved = false;
      for (final e in _rules.entries) {
        if (!out.contains(e.key) && _emitsNothing(e.value, out)) {
          out.add(e.key);
          improved = true;
        }
      }
  ...
  bool _emitsNothing(Clause clause, Set<String> nullableRules) =>
      switch (clause) {
        // A lookahead consumes nothing and emits nothing, always.
        Nothing() || FollowedBy() || NotFollowedBy() || Optional() => true,
        Str(:final text) => text.isEmpty,
        Terminal() => false, // Char, CharSet, AnyChar: one character, or no match
        Seq(:final subClauses) =>
          subClauses.every((s) => _emitsNothing(s, nullableRules)),
        First(:final subClauses) =>
          subClauses.any((s) => _emitsNothing(s, nullableRules)),
        Repetition(:final requireOne, :final subClause) =>
          !requireOne || _emitsNothing(subClause, nullableRules),
        Ref(:final ruleName) => nullableRules.contains(ruleName),
        _ => true,
      };
  ...
  ///
  /// I6: a one-character lookahead I4 could not fuse becomes a CONSTRAINT on the
  /// cell rather than a leaf in it. The leaf it would have become reads the input
  /// through the oracle -- the ORIGINAL input, at a position whose character the
  /// repair may be about to change -- so it decides the wrong string. The
  /// constraint decides the right one, and the head it leaves behind is the empty
  /// match, because a lookahead consumes nothing and appears in no tree.
  ///
  /// A CONSTRAINT IS POSTED ONLY IN FRONT OF A READER -- exactly A4's rule for
  /// gaps, applied to constraints, and for the same reason. A4 attaches a gap in
  /// front of whatever reads the input next because a gap is only meaningful
  /// where reading happens; a lookahead constrains the next character EMITTED,
  /// so it is only meaningful where something must emit one. When the rest of
  /// the chain can be silent the constraint has no reader to bind, and would
  /// fall off the end of the chain to a parent whose continuation this value does
  /// not carry -- which is precisely m47's leak. Such a lookahead keeps its node
  /// and is decided by the oracle, as in m46. So I6 applies exactly where the
  /// constrained character is guaranteed to be emitted inside the same sequence,
  /// and that boundary is decided statically, once, at build time.
  _Node _cons(List<Clause> parts, Clause orig) {
    parts = _fuseLookaheads(parts);
    var node = _eps;
    var mustRead = false; // does anything after this point have to emit?
    for (var i = parts.length - 1; i >= 0; i--) {
      final looks = _looks(parts[i]);
      final post = looks != null && mustRead ? _intern(looks) : _free;
      node = _Cons(_nodeCount++, i == 0 ? orig : Seq(parts.sublist(i)))
        ..head = post == _free ? _node(parts[i]) : _eps
        ..post = post
        ..tail = node;
      mustRead = mustRead || !_nullable(parts[i]);
    }
    return node;
  }
  ...
  late final int _goalFromNothing = () {
    ...
    int cheapest(bool trustPredicates) {
      // I6 MAKES THE CEILING A TABLE PER CONSTRAINT, and it has to: a constraint
      // can close the cheap branch of a choice and force a dearer one, so a price
      // computed without them is not an upper bound at all. `S <- !'x' A;
      // A <- 'x' / "yy";` on the empty input is the whole argument -- one
      // fabrication unconstrained, two under `!'x'`, and a ceiling of one would
      // make the engine DECLINE an input it can repair. Rows are allocated when
      // something asks for one, so a grammar without lookahead has exactly one and
      // this is m44's table, value for value.
      final cost = <int, List<int>>{};
      var improved = true;
      List<int> row(int c) => cost.putIfAbsent(c, () {
            improved = true;
            return List.filled(_nodeCount, _impossible);
          });
      // A cons cell is `_chain` with the input taken away: the head emits the
      // constrained character, or the head is silent and the tail owes it.
      int chain(_Cons node, int c) {
        // A cons cell whose tail is itself is a repetition: zero iterations, and
        // zero iterations emit nothing.
        var best = identical(node.tail, node) && _permits(c, null) ? 0 : _impossible;
        for (final (headC, tailC) in _split(c)) {
          final sum = row(headC)[node.head.id] +
              row(_meet(tailC, node.post))[node.tail.id];
        ...
        for (final c in cost.keys.toList()) {
          for (final node in all) {
            final now = switch (node) {
              ...
              // EVERYTHING HERE THAT EMITS NOTHING ASKS THE CONSTRAINT whether
              // nothing is enough, with no exception -- m47 exempted its chain
              // terminator and that is the same leak `_eps` closes below.
              _Term(:final editable, :final orig) => editable
                  // fabricate it: I2's FAB, price 1 -- if what it accepts is
                  // something the constraint still allows it to emit.
                  ? (_permits(c, _oneCharClass(orig)) ? 1 : _impossible)
                  : (orig is Nothing ||
                              (trustPredicates && orig is! Terminal)) &&
                          _permits(c, null)
                      ? 0 // already there, or assumed to be
                      : _impossible,
              _Cons() => chain(node, c),
              _Alt() => node.alts.fold(
                  _impossible, (best, alt) => math.min(best, row(c)[alt.id])),
            };
            if (now < row(c)[node.id]) {
              row(c)[node.id] = now;
              improved = true;
            }
          }
  ...
  /// The stride that makes I6's constraint a memo DIMENSION rather than a second
  /// map: `(c, node, pos)` is one integer. A grammar with no lookahead only ever
  /// asks under `_free`, so it uses one slab of the key space and the memo is
  /// m46's, entry for entry.
  late int _keyStride;

  /// Every end position reachable from `pos` by matching `node` -- whose first
  /// EMITTED character lies in class `c` (I6) -- each mapped to its minimum Delta,
  /// given that at most `budget` edits may be spent. This is `Parser.match`:
  /// bounds check, find the entry, ask it.
  Map<int, int> _ends(_Node node, int pos, int budget, int c) {
    if (pos > _inputLen || budget < 0) return const {};
    return _entries
        .putIfAbsent(
            (c + 1) * _keyStride + node.id * (_inputLen + 2) + pos, _Entry.new)
        .ends(this, node, pos, budget, c);
  }
  ...
  Map<int, int> _compute(_Node node, int pos, int budget, int c) {
    _steps++;
    ...
    // operand for every product in `_chain`. It stays exact under a constraint:
    // spending nothing emits the input verbatim, so the character the constraint
    // asks about is the one already there, and the lookaheads INSIDE the clause are
    // decided by the oracle on the same unedited text they will be read on.
    if (budget == 0) {
      final m = node.orig.match(_parser, pos);
      return m.isMismatch || !_permitsMatch(c, pos, m.len)
          ? const {}
          : {pos + m.len: _cleanRegret(m)};
    }
    switch (node) {
      case _Cons():
        return _chain(node, pos, budget, c);
      case _Alt(:final alts):
        ...
        if (alts.length == 1) return _ends(alts[0], pos, budget, c);
        ...
        for (final alternative in alts) {
          for (final e in _ends(alternative, pos, budget, c).entries) {
            ...
            // here. A constraint therefore permits both or neither, and when it
      ...
      case _Term(:final editable):
        final out = <int, int>{};
        final m = node.orig.match(_parser, pos);
        // MATCH -- and matching emits the input it consumed, so what I6 asks
        // about is the character already there.
        if (!m.isMismatch && _permitsMatch(c, pos, m.len)) {
          out[pos + m.len] = _cleanRegret(m);
        }
        if (!editable) return out;
        // I6: BOTH LIES EMIT WHAT THE TERMINAL ACCEPTS -- SUB in place of the
        // character it consumed, FAB in place of nothing -- so one class decides
        // both. A terminal that accepts no character at all emits nothing when it
        // lies (`Nothing`, which is deletion), and `_permits` reads that as the
        // silent case rather than as a special one.
        if (!_permits(c, _oneCharClass(node.orig))) return out;
  ...
  Map<int, int> _chain(_Cons node, int pos, int budget, int c) {
    final out = <int, int>{};
    final limit = (budget + 1) * _costUnit;
    ...
    // emits nothing, which under I6 is a claim the constraint has to allow.
    final loops = identical(node.tail, node);
    if (loops && _permits(c, null)) out[pos] = 0;
    // I6, AND THE WHOLE OF IT. The cell's constraint binds the first character the
    // CELL emits, and the cell is a head and a tail: either the head emits it, or
    // the head emits nothing and the tail inherits the obligation. Both, always --
    // which of the two happens is a run-time fact (`_nullseq45`: no static
    // placement of the constraint is the right language), and a union over a
    // semiring is exactly the shape that answers a question at run time. The
    // cell's own lookahead, if it has one, meets whatever the tail already owes.
    for (final (headC, tailC) in _split(c)) {
      final tailConstraint = _meet(tailC, node.post);
      for (final head in _ends(node.head, pos, budget, headC).entries) {
        ...
        final rest = _ends(node.tail, head.key,
            budget - _editCount(head.value), tailConstraint);
  ...
  MatchResult? _build(_Node node, int pos, int end, int delta, int budget,
    ...
    if (!pure.isMismatch &&
        ...
        _permitsMatch(c, pos, pure.len)) {
      return pure;
    }
    switch (node) {
      case _Term():
        // Reached here the leaf is lying, and under I6 it may not lie freely: the
        // character it invents has to be one the constraint allows. The class it
        // accepts, narrowed to the constraint, is a class the parser understands,
        // so the narrowing is expressible in the witness itself -- and `_spelling`
        // then picks a member that verifies instead of one that does not.
      ...
      case _Alt(:final alts):
        final state = (node, pos, end, delta, c);
        if (!_path.add(state)) return null;
        for (final alternative in alts) {
          if (_ends(alternative, pos, budget, c)[end] != delta) continue;
          final m = _child(alternative, pos, end, delta, budget, c);
          if (m != null) {
            _path.remove(state);
        ...
        return null;
      case _Cons():
        final children = _row(node, pos, end, delta, budget, c);
  ...
  List<MatchResult>? _child(
      _Node node, int pos, int end, int delta, int budget, int c) {
    if (identical(node, _junk)) {
      return end == pos ? const [] : [SyntaxError(pos: pos, len: end - pos)];
    }
    if (node is _Cons && identical(node.head, _junk)) {
      return _row(node, pos, end, delta, budget, c);
    }
    final m = _build(node, pos, end, delta, budget, c);
  ...
  List<MatchResult>? _row(
    ...
    if (node is! _Cons) {
      ...
      return identical(node, _eps)
          ...
          : _child(node, pos, end, delta, budget, c);
    }
    final loops = identical(node.tail, node);
    // I6, replayed exactly as `_chain` decided it: the head either emits the
    // constrained character or emits nothing and passes the obligation on.
    for (final (headC, tailC) in _split(c)) {
      final tailConstraint = _meet(tailC, node.post);
      final heads = _ends(node.head, pos, budget, headC);
      ...
      for (final headEnd in order) {
        ...
        final rest = _ends(node.tail, headEnd, restBudget, tailConstraint)[end];
        if (rest == null || headDelta + rest != delta) continue;
        final head = _child(node.head, pos, headEnd, headDelta, budget, headC);
        if (head == null) continue;
        final tail =
            _row(node.tail, headEnd, end, rest, restBudget, tailConstraint);
  ...
  SkipResult recover(String input) {
    ...
    final root = cost < 0
        ? null
        : _build(_goal, 0, _inputLen, _bestGoalDelta, cost, _free);
    if (root == null) {
  ...
  int recoverCost(String input) {
    ...
    _keyStride = (_nodeCount + 1) * (_inputLen + 2);
    ...
    for (var k = 0; k <= maxCost; k++) {
      final best = _ends(goal, 0, k, _free)[_inputLen];
      if (best != null) {
        _bestGoalDelta = best;
