...
// PROFILING COPY OF m87 -- counters only, no behaviour change.
int pClause = 0, pExpand = 0, pRepair = 0, pSkip = 0, pRound = 0, pFirstFull = 0;
int pNil = 0, pCons = 0, pExtend = 0, pCand = 0, pExp = 0, pWrap = 0;
int pSkipWay = 0, pFillWay = 0, pPutNew = 0, pPutRebuild = 0, pPutCall = 0;
int pN = 0, pTleaf = 0, pTnode = 0, pCat = 0;
void pReset() {
  pClause = pExpand = pRepair = pSkip = pRound = pFirstFull = 0;
  pNil = pCons = pExtend = pCand = pExp = pWrap = 0;
  pSkipWay = pFillWay = pPutNew = pPutRebuild = pPutCall = 0;
  pN = pTleaf = pTnode = pCat = 0;
}
...
class _T {
  ...
  _T.node(this.c, this.pos, this.kids) : leaf = null {
    pTnode++;
  }
...
_N? _cat(_N? a, _N? b) {
  pCat++;
  return b == null ? a : _N(b.head, _cat(a, b.tail));
}
...
_Way _nil(int pos) {
  pNil++;
  return _Way(pos, 0, 0, 0, false, null);
}
...
_Way _cons(_Way w, _Way? next) {
  pCons++;
  return _Way(w.end, w.cost, w.got, w.net, w.synth, w.nodes, next);
}
...
_Way _extend(_Way w, _Way x, _N? nodes) {
  pExtend++;
  return _Way(
    x.end,
    w.cost + x.cost,
...
class SuperDot3 {
  ...
  _Ways _clause(Clause c, int pos) {
    pClause++;
    return _clause0(c, pos);
  }

  _Ways _clause0(Clause c, int pos) => c is Terminal
      ? _expand(c, pos)
