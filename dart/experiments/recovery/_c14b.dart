// c13.dart -- a recovery engine over the library's own grammar, in one file.
//
...
//
// WHAT c13 CHANGES FROM c12. One rule of the sequence fold gains a
// jurisdiction exemption: the noise-vs-absence penalty ("something was
// missing here, and a deletion stood nearby -- price the absence")
// applies only while the question still belongs to THIS fold. When the
// slot is the sequence's LAST, what stood there is the enclosing
// context's question -- the parent's next slot may want exactly the
// character the deletion would consume -- so the fold stops guessing.
// Measured: +0.511 points over 14 cases (10 fixed, 4 broken), battery
// 0.9899/85.8 with every other column unchanged.
//
// WHAT c13 TAKES FROM ITS THREE PARENTS. c9, c10 and c11 each won one axis
...
class _Reading {
  ...
  int get cost =>
      deleted * Squirrel.delWeight + missing + (missingAtEnd > 0 ? 1 : 0);
  ...
  /// through [cost].
  int get spent => deleted * Squirrel.delWeight + missing;
...
class _RepairCell {
  ...
  bool add(_Reading r) {
    ...
    if (i < 0) {
      ...
      // PROBE: with retention on, tied rivals are KEPT BESIDE the holder
      // (as further entries with the same end), capped per end.
      if (cmp == 0 && Squirrel.retainTies) {
        var n = 0;
        for (var j = 0; j < _best.length; j++) {
          if (_best[j].end == r.end) n++;
        }
        if (n < Squirrel.tieCap) {
          _best.add(r);
          _view = null;
...
class Squirrel {
  /// PROBE knobs: how many rungs past first admission to keep exploring;
  /// whether exact-tie readings are retained beside their cell holders;
  /// and the per-end cap on retained ties. All complete coherent root
  /// admissions land in [rootCands] for offline judging.
  static int extraHold = 0;
  static bool retainTies = false;
  static int tieCap = 3;

  /// PROBE knob: how many charge points one deleted input character
  /// costs (1 = the standing symmetric pricing).
  static int delWeight = 1;
  final List<_Reading> rootCands = [];
  ...
  static void _keepBest(List<_Reading> best, _Reading r, int pos) {
    ...
    if (i < 0) {
      best.add(r);
    } else if (retainTies && _compare(r, best[i], pos) == 0) {
      // PROBE: keep exact ties beside the holder, capped per end.
      var n = 0;
      for (var j = 0; j < best.length; j++) {
        if (best[j].end == r.end) n++;
      }
      if (n < tieCap) best.add(r);
  ...
  lib.MatchResult recover(String s) {
    ...
    var admittedAt = -1;
    final extraRungs = extraHold;
    rootCands.clear();
    for (_budget = 1;
        _budget <= ceiling && (admittedAt < 0 || _budget < admittedAt + extraRungs);
        _budget++) {
      ...
      for (var ri = 0; ri < roots.length; ri++) {
        ...
        // PROBE: every complete coherent admission is collected.
        rootCands.add(whole);
      ...
      if (best != null) {
        if (admittedAt < 0) {
          admittedAt = _budget;
          if (extraRungs > 0) continue; // PROBE: hold the ladder open.
        }
        final b = best;
  ...
  /// PROBE: materialize one collected candidate reading as a tree.
  lib.MatchResult candidateTree(_Reading r) => _buildTree(r);
}
