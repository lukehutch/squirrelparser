...
int nWays = 0, nWaysRef = 0, nWaysTerm = 0, nWaysCell = 0;
int nHit = 0, nHitFiltered = 0, nLoop = 0;
int nAff = 0, nAffIn = 0, nAffOut = 0, nAffNoop = 0;
int nPr = 0, nPrIn = 0, nPrOut = 0, nPrNoop = 0;
int nRefPr = 0, nRefPrNoop = 0;
int nClose = 0, nWrap = 0, nTerm = 0;
int nRepAll = 0, nRepEnds = 0;
int nSeqPre = 0, nSeqOver = 0, nSkipK = 0, nMoveK = 0, nMoveChain = 0;
int nLiftIn = 0, nLiftOut = 0;
int nSeqUnsorted = 0, nAffUnsorted = 0, nPrOutUnsorted = 0;
final List<int> prHist = List.filled(12, 0);
bool sortedOK(List<_Way> ws) {
  for (var i = 1; i < ws.length; i++) {
    if (ws[i - 1].del + ws[i - 1].gap > ws[i].del + ws[i].gap) return false;
  }
  return true;
}
...
class Squirrel {
  ...
  static List<_Way> _prune(List<_Way> ws) {
    nPr++;
    nPrIn += ws.length;
    prHist[ws.length < 11 ? ws.length : 11]++;
    if (ws.length <= 1) {
      nPrOut += ws.length;
      nPrNoop++;
      return ws;
    ...
    nPrOut += out.length;
    if (!sortedOK(out)) nPrOutUnsorted++;
    var same = out.length == ws.length;
    if (same) {
      for (var i = 0; i < out.length; i++) {
        if (!identical(out[i], ws[i])) {
          same = false;
          break;
        }
      }
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    if (c is Ref) {
      nWaysRef++;
      final inp = _lift(c, pos, _ways(rules[c.ruleName]!, pos));
      final pr = _prune(inp);
      nRefPr++;
      var same = pr.length == inp.length;
      if (same) {
        for (var i = 0; i < pr.length; i++) {
          if (!identical(pr[i], inp[i])) {
            same = false;
            break;
          }
        }
  ...
  List<_Way> _afford(List<_Way> ws) {
    nAff++;
    nAffIn += ws.length;
    if (!sortedOK(ws)) nAffUnsorted++;
    final out = [for (final w in ws) if (w.del + w.gap <= _budget) w];
    nAffOut += out.length;
    if (out.length == ws.length) nAffNoop++;
    return out;
  }
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        nSeqPre++;
        if (w.del + w.gap > _budget) {
          nSeqOver++;
          if (!sortedOK(cur)) nSeqUnsorted++;
          continue;
        }
  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    nRepAll += all.length;
    nRepEnds += {for (final w in all) w.end}.length;
