// m27 -- m26 plus possessiveness: a Repetition may finish only where its body
// fails, as PEG semantics require. See _mayFinish.
//
...
class SuperDot3 {
  ...
  // POSSESSIVENESS. A Seq that has run off the end of its element list has no
  // choice: it is finished. A Repetition does have a choice, and PEG semantics
  // do not give it one -- `*` is greedy and possessive, so it stops exactly where
  // its body FAILS and never gives a character back. Offering a free stop at
  // every iteration boundary is what let every previous engine repair toward the
  // CFG reading of the grammar instead of the PEG one, and it is also what made
  // the end-maps O(n) wide, since a list of k items could finish at all k
  // boundaries. Under this rule a k-item list has ONE finish point, plus one more
  // per edit that breaks the body.
  //
  // A zero-width body match counts as a failure here for the same reason the pure
  // parser treats it as one: it cannot make progress.
  bool _mayFinish(Clause c, int pos) {
    if (c is! Repetition) return true;
    final m = c.subClause.match(_parser, pos);
  ...
  Map<int, int> _chain(Clause c, int dot, int pos, int b) {
    final out = <int, int>{};
    final limit = (b + 1) * _M;
    if (_done(c, dot) && _mayFinish(c, pos)) out[pos] = 0;
