...
class _Way {
  ...
  /// I54: A GUESS IS A COST YOU CANNOT SEE. How many of this way's asserted
  /// characters the grammar does NOT determine.
  ///
  /// I52 lets a fill be priced when its spelling is unknown, and that is right --
  /// but it makes two repairs cost the same that are not equally believable.
  /// Given `[,2,33,true]` the leading comma can be DISCARDED for one character,
  /// or a Value can be ASSERTED before it for one character; both cost 1, and
  /// [net] and [got] then both prefer the assertion, because it leaves every
  /// character of the input in a role. The brief says the opposite, and gives
  /// the reason: the asserted value "could be anything, so why pick one?" --
  /// while the comma is simply one character too many.
  ///
  /// The line the brief draws is DETERMINACY, and I36 already computes it. A
  /// fill whose text the grammar fixes -- a `,`, a `}` -- picks nothing and is
  /// not a guess. A fill whose text it does not fix is a guess for each
  /// character it stands for, and among readings that cost the same, the one
  /// that guesses less wins. It is a tie-break only: it can never make a more
  /// expensive repair beat a cheaper one, so `{ ="de"; }` still admits its
  /// missing Name for one character rather than paying three to avoid it.
  final int guess;
...
_Way _cons(_Way w, _Way? next) => _Way(
    w.end, w.cost, w.got, w.net, w.synth, w.nodes,
    guess: w.guess, next: next);
...
/// I54 inserts GUESS directly under cost: fewer unforced assertions wins before
/// "explains more" gets a say, because "explains more" is exactly what an
/// invented node buys and it must not be able to buy it.
bool _better(int cost, int guess, int net, int got, _Way? b) =>
    b == null ||
    cost < b.cost ||
    (cost == b.cost &&
        (guess < b.guess ||
            (guess == b.guess &&
                (net > b.net || (net == b.net && got > b.got)))));

bool _beats(_Way a, _Way? b) => _better(a.cost, a.guess, a.net, a.got, b);
  ...
  _Ways _wrap(Clause c, int pos, _Ways w) {
    ...
    for (var e = w; e != null; e = e.next) {
      out = _Way(e.end, e.cost, e.got, e.net, e.synth,
          _N(_T.node(c, pos, e.nodes), null),
          guess: e.guess, next: out);
    }
    return out;
  }
    ...
    for (final sub in subs) {
      ...
      for (var w = ways; w != null; w = w.next) {
        for (var x = _element(sub, w.end); x != null; x = x.next) {
          ...
          if (!_better(cost, w.guess + x.guess, w.net + x.net, w.got + x.got,_at(next, x.end))) {
            continue;
          }
    ...
    while (frontier.isNotEmpty) {
      ...
      for (final e in frontier) {
        ...
        for (var x = _element(r.subClause, e); x != null; x = x.next) {
          ...
          final cost = w.cost + x.cost;
          if (cost > _budget) continue;
          if (!_better(cost, w.guess + x.guess, w.net + x.net, w.got + x.got,reach[x.end])) {
            continue;
          }
      ...
      out = _put(
          ...
          _Way(
              ...
              _N(
                  ...
                  null),
              // I54: a determined spelling picks nothing, so it is not a guess.
              guess: w != null ? 0 : need));
    }
    return out;
  }
