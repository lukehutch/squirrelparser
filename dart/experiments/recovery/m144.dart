// m144.dart -- PROBE, not a candidate engine. m143 plus a SEEDED right-to-left
// re-relaxation: after the top-down pass at each budget has demanded its cells,
// those same cells (and only those) are swept right-to-left to a global fixed
// point. This is the strongest surviving form of the two-mode brief -- a chart
// restricted to the demanded subgraph, so it never pays the |G| x n fill that
// sank m141.
//
// It exists to answer one question: does the demand-driven top-down search
// ALREADY reach the fixed point a chart would? `sweepGrew` counts cells whose
// way-list the sweep improves, and `sweepChangedBest` counts rounds where that
// changed the winning tree. If both are zero across the battery, the chart is
// not merely slower -- it is provably redundant on this workload.
//
// Below is m143's header, unchanged.
//
// m143.dart -- I81: past the end of the input, the slot never existed. A named
// node covering zero characters at or past `input.length`, asserting no
// character of its own, stands for a construct the human never wrote, so it is
// dropped at emit -- unless the frozen parser genuinely admits a zero-length
// reading there, which the cost-0 memo is asked. Derived from m132.dart (I78).
//
// Mid-input the same node is KEPT: there the slot is real and only its evidence
// was destroyed ([1,x] still has two Number slots), which is why the unrestricted
// form, m142, lost. See LESSONS_LEARNED occasion 58 and _i81probe.dart.
...
/// Whether anything in [m] asserts a character the input does not carry.
bool _asserts(MatchResult m) {
  if (m is Filled) return true;
  for (final k in m.subClauseMatches) {
    if (_asserts(k)) return true;
  }
  return false;
}
...
void _emit(_Way? w, List<MatchResult> out, bool Function(Clause, int) hollow) {
  ...
  if (w.leaf != null) {
    ...
    _emit(w.a, kids, hollow);
    final node = Match(w.c, w.pos, 0, subClauseMatches: kids);
    // I80: A NODE THAT EXPLAINS NO CHARACTER AND ASSERTS NONE IS NOT A NODE.
    //
    // An empty `Number` where the document ran out covers nothing and invented
    // nothing. The brief forbids exactly this: never invent a terminal of a
    // class that is not there. It is not a judgement call, because the GRAMMAR
    // answers it -- keep the node iff the frozen parser admits a zero-length
    // reading of this clause here, true for `X <- 'a'*` and false for
    // `Number <- Integer ...`, whose `Integer` demands a digit.
    //
    // Length, not childlessness, is the test: `Number`'s child `Integer` is not
    // a named rule, so the node HAS a child and still covers nothing. And a
...
class SuperDot3 {
  ...
  /// Cells the seeded sweep improved, and rounds where that changed the winner.
  int sweepGrew = 0, sweepChangedBest = 0, sweepRounds = 0;
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0;
      ...
      _Way? best = _readTop(top, s);
      if (_budget > 0) {
        sweepRounds++;
        final grew = _reseed(_budget);
        if (grew > 0) {
          sweepGrew += grew;
          final again = _readTop(top, s);
          if (again != null && (best == null || _beats(again, best))) {
            sweepChangedBest++;
            best = again;
          }
        }
  ...
  /// The best whole-input way under the current budget, or null.
  _Way? _readTop(Clause top, String s) {
      _Way? best;
  ...
  /// I80: whether a childless wrap of [c] at [pos] must be dropped -- true when
  ...
  bool _hollow(Clause c, int pos) {
    ...
    // node sits between real characters, and the honest reading of it is a
    // construct the damage broke -- which the brief wants MARKED, as a syntax
    // error span on the node, not deleted along with the node.
    if (pos < _in.length) return false;
    for (var e = _pure(c, pos); e != null; e = e.next) {
      if (e.end == pos) return false;
    }
    return true;
  }
  ...
  /// Fold every way of [add] into [into], keeping the better at each ending.
  _Ways _merge(_Ways into, _Ways add) {
    if (into == null) return add;
    _Ways merged = into;
    for (var e = add; e != null; e = e.next) {
      merged = _put(merged, _cons(e, null));
    }
    return merged;
  }

  /// Sweep the ALREADY-MATERIALISED cells of budget [b] right-to-left until
  /// nothing improves, and report how many improved. Only cells the top-down
  /// search actually demanded are visited, so this costs the demanded subgraph
  /// rather than the whole chart.
  int _reseed(int b) {
    final mc = _mc[b], me = _me[b];
    var grew = 0;
    var moved = true;
    while (moved) {
      moved = false;
      for (var pos = _in.length; pos >= 0; pos--) {
        for (final entry in mc.entries) {
          final cell = entry.value[pos];
          if (cell == null || !cell.done || entry.key is Terminal) continue;
          final r = _expand(entry.key, pos);
          if (_grows(r, cell.ways)) {
            cell.ways = _merge(cell.ways, r);
            grew++;
            moved = true;
          }
        }
        for (final entry in me.entries) {
          final cell = entry.value[pos];
          if (cell == null || !cell.done) continue;
          final r = _repair(entry.key, pos);
          if (_grows(r, cell.ways)) {
            cell.ways = _merge(cell.ways, r);
            grew++;
            moved = true;
