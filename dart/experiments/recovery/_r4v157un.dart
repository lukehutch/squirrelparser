...
class Squirrel {
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        //
        // BUT NOT IN FRONT OF THE FIRST SLOT. Characters discarded before a
        // later slot lie BETWEEN two things this production claims, so the
        // production itself brackets them as junk. In front of the first slot
        // there is no left bracket: the production begins where its PARENT put
        // it, and discarding there does not resynchronize within this
        // production, it moves the whole production somewhere else. That is how
        // a `String` comes to start at the third character of the document,
        // reading everything after it through `[^"\\]`. The parent is the
        // clause that can honestly delete there, because for the parent the
        // same position IS between slots.
        if (i == 0) continue;
  ...
  List<_Way> _first(First c, int pos) {
    ...
    for (final s in c.subClauses) {
      ...
      for (final w in ws) {
        // Once an arm has read the input AS IT STANDS, the choice is decided by
        // the document, not by the budget: a later arm may still carry the
        // parse where it too reads cleanly, but it may not BUY the choice with
        // a repair.
        if (settled && !w.free) continue;
  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    while (frontier.isNotEmpty) {
      ...
      for (final w in frontier) {
        final here = _ways(c.subClause, w.end);
        for (final v in here) {
          if (v.end <= w.end) continue;
          grown.add(_Way(v.end, w.del + v.del, w.gap + v.gap, w.net + v.net,
              _min(w.fix, v.fix), w.peg && v.peg, v.node, w));
        }
        // JUNK BETWEEN ITEMS. Nothing brackets one occurrence from the next but
        // the input itself, so where no occurrence can be read AS IT STANDS at
        // the frontier, the characters there are junk between items rather than
        // the opening of a damaged one. This is the resynchronization `_seq`
        // already performs in front of a slot, offered on the same two terms:
        // only where the occurrence cannot be read where it stands, and only as
        // far as the nearest position where it reads cleanly.
        // AND ONLY OVER CHARACTERS NO OCCURRENCE COULD HAVE STARTED
        // WITH. Where some reading of the item takes the character at the
        // frontier AS WRITTEN -- `fix` past it -- the input is offering a
        // damaged item, and a damaged item is to be repaired, not thrown away.
        // That is the difference between `x=;`, which is an assignment missing
        // its condition, and a stray `"`, which no statement can begin with.
        if (here.any((v) => v.fix > w.end)) continue;
        // A REPETITION OF TERMINALS IS A TOKEN, AND A TOKEN IS CONTIGUOUS.
        // "Between two occurrences" is only a place where something else could
        // be once the occurrences on either side are constructs with their own
        // boundaries; between two characters of `[ \t\n\r]*` there is no such
        // place, and letting whitespace delete whatever stands between it and
        // the next space would throw away the whole document one word at a
        // time.
        final item = c.subClause;
        if (item is! Ref && item is! Seq && item is! First) continue;
        final room = _budget - w.del - w.gap;
        for (var k = w.end + 1; k <= w.end + room && k <= _in.length; k++) {
          final full = _budget;
          _budget = 0;
          final at = _ways(c.subClause, k);
          _budget = full;
          if (at.isEmpty) continue;
          for (final v in at) {
            if (!v.free || v.end <= k) continue;
            grown.add(_Way(v.end, w.del + v.del + k - w.end, w.gap + v.gap,
                w.net + v.net, _min(w.fix, w.end), false, v.node, w,
                SyntaxError(pos: w.end, len: k - w.end)));
          }
          break;
