// Scratch: bf_check.dart's ground-truth gate, run for m41 and m42.
...
void main() {
  ...
  for (final (title, grammar, top, alphabet, inputs) in cases) {
    ...
    print('${'input'.padRight(9)}${'true'.padLeft(6)}${'m41'.padLeft(6)}'
        '${'m42'.padLeft(6)}   verdict');
    var bad = 0;
    for (final s in inputs) {
      ...
      var tree = '';
      try {
        final root = r42.recover(s).root;
        if (!covers(root, s.length)) tree = ' m42-NOCOVER';
      } catch (e) {
        tree = ' m42-BUILD(${e.runtimeType})';
      }
      final ok = c41 == want && c42 == want && tree.isEmpty;
      if (!ok) {
        bad++;
        totalBad++;
      }
