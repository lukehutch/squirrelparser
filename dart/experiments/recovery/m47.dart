...
class SuperDot3 {
  ...
  /// The empty match: the last alternative of every Optional, and the node a
  /// `Nothing` in the grammar denotes. It is a terminal, so it needs no case.
  late final _Node _eps = _term(const Nothing(), false);

  /// I6: THE END OF A CHAIN, which is the empty match plus one thing -- it is
  /// where a constraint that nobody discharged goes. `_eps` may not do this:
  /// it is also Optional's empty branch, and letting THAT discharge would let a
  /// nullable prefix satisfy a lookahead by matching nothing, which is the
  /// under-report `_nullseq45` demonstrates. The two nodes are the same terminal
  /// and differ only in that one is the identity of sequencing and the other is
  /// a skipped alternative -- so they must be different nodes, and are.
  ///
  /// Discharging is OPTIMISTIC: a lookahead last in its sequence constrains the
  /// PARENT's continuation, which this value does not carry, so the constraint is
  /// dropped rather than enforced. That is m46's behaviour exactly, it is the one
  /// place I6 is not exact, and I5 reports it when it goes wrong.
  late final _Node _end = _term(const Nothing(), false);
  ...
  _Node _cons(List<Clause> parts, Clause orig) {
    ...
    for (var i = parts.length - 1; i >= 0; i--) {
      ...
      node = _Cons(_nodeCount++, i == 0 ? orig : Seq(parts.sublist(i)))
        ..head = looks == null ? _node(parts[i]) : _eps
        ..post = looks == null ? _free : _intern(looks)
        ..tail = node;
    }
    return node;
  }
        ...
        for (final c in cost.keys.toList()) {
          for (final node in all) {
            final now = switch (node) {
              ...
              // Everything here that emits nothing has to ask the constraint
              // whether nothing is enough, and `_end` is the one node that does
              // not, because discharging is what it is for.
              _Term(:final editable, :final orig) => identical(node, _end)
                  ? 0
                  : editable
  ...
  Map<int, int> _compute(_Node node, int pos, int budget, int c) {
    _steps++;
    // I6: the end of a chain is where an undischarged constraint stops. See `_end`.
    if (identical(node, _end)) return {pos: 0};
  ...
  List<MatchResult>? _row(
    ...
    if (node is! _Cons) {
      ...
      return identical(node, _eps) || identical(node, _end)
