// m93 -- I59: A CELL THAT NEVER CONSULTED THE CAP CANNOT CHANGE WHEN IT RISES.
//
// m93 is m92 with the deepening loop's round tables no longer thrown away. The
// numbers must be identical; only the time moves.
//
// Deepening re-parses the whole document once per budget, and 98.5% of all cell
// bodies run in round 1 or later -- 62.7% in round 1 alone, 35.7% after it. The
// tables are cleared each round because an answer computed under a cap of 1 may
// be wrong under a cap of 2. But that is only true of answers the cap actually
// touched.
//
// Costs only rise along a way, so the ways with cost <= b are COMPLETE at budget
// b: raising the cap can only ADD ways, all of them strictly dearer. `_better`
// orders by cost first, so a dearer way can never displace an incumbent. Hence a
// cell whose evaluation refused nothing for exceeding the cap -- and which read
// only cells with that same property -- holds the same answer at every larger
// budget. It is final, and the next round can read it as it stands.
//
// One flag carries this. `_bound` is cleared on entry to a cell body and set
// wherever the cap refuses something, wherever an in-cycle seed is read, and
// wherever a cell computed earlier in THIS round is read. On the way out the
// cell records `fin = !_bound` and restores the caller's flag with its own or-ed
// in, so the property propagates up the call tree exactly as the argument
// requires. Nothing else changes: the same ways, the same order, the same tie
// breaks.
//
// The budget-0 table is untouched by this. It answers the frozen parser's
// question, which has no cap, so its cost tests never set the flag and its cells
// are final the moment they complete -- which is why `_pure` needs no special
// handling despite running at budget 0 inside a capped cell.
//
// I58: A FREE WAY IS ALREADY THE PROOF, SO DO NOT GO AND FETCH IT.
//
// m92 is m90 with two probes removed and nothing else. Every number it produces
// must be identical; only the time moves. Both changes came out of comparing
// this engine's own profile against Codex's independent ablation of m81, which
// measured the same two as 9.4% and 7.5% there.
//
// `_repair` asked `_pure(sub, pos) != null` -- "does pure PEG match here?" --
// after it had ALREADY computed `_clause(sub, pos)`. But `cost == 0` means no
// skip and no fill, so a free way in that answer IS a pure reading, and the
// question was answered before it was asked.
//
// The implication runs ONE WAY and the fallback is not optional. `_pure` can be
// non-null where `direct` has no free way: inside an unfinished left-recursion
// cycle `_clause` hands back an empty seed while the global fact "this matches
// purely here" stays true, and `_first` depends on exactly that gap to know it
// is seeding. Collapsing the two tables is what once regressed `a+b*` to cost
// -1, so `_pure` stays as the fallback and only the common case short-circuits.
//
// The three memo tables and the witness cache become `HashMap.identity()`. This
// is not a change of key semantics: `Clause` overrides neither `==` nor
// `hashCode` anywhere in the frozen library, so the default map already keys by
// identity. It only removes the virtual dispatch that rediscovers that on every
// one of 33.3M probes -- which is the 60 ns/call term of the cost model, the
// one place that term can be attacked without cutting rounds, tables or
// positions.
//
// NOT TAKEN, AND WHY. Codex's third measured change replaces `_rep`'s
// `Map<int,_Way>` and per-wave `Set<int>` with input-length-indexed arrays,
// worth 9.3% in m81. Here it would allocate two `List.filled(len + 1)` per
// `_rep` CALL -- repetitions nest, so a single shared scratch buffer is unsound
// -- and `reach` holds one entry per iteration, not one per position. Trading a
// five-entry map for two arrays of length 50 is the wrong direction under a
// cost model whose unit is the field written, so it is left out until measured
// rather than adopted on another engine's number.
//
// I56: A WAY THAT IS ALREADY NEW DOES NOT NEED TO BE COPIED.
//
// Measured on the 1824-case battery (`_prof.dart`), `_cons` was 37.6M of the
// 79.2M ways the engine allocated -- 47% of all of them, and not one carried
// any information that was not already in hand:
//
//   * 17.6M were `_put` copying a way the CALLER had just built, purely to give
//     it a `next` pointer. A freshly built way is unlinked by definition, so it
//     can simply BE the new head.
//   * 15.8M were `_put` rebuilding an entire list to replace one ending. Only
//     the ways BEFORE the replaced one have to move: everything after it can be
//     shared, because the new way takes over the old way's tail.
//
// `_put` still never mutates a way that already exists: the only field it
// writes is `next`, on the way it was just handed. That is what makes it safe
// at the call sites whose head is a memo cell's own list. What it does NOT do
// is preserve order -- see I57 above, which is why that now costs nothing.
//
// I55: THE SEARCH DOES NOT NEED THE TREE.
//
// m88 is m87 with one representation changed and nothing else. Every number it
// produces must be identical; only the time and the object count move.
//
// `_beats` reads four integers. Nothing anywhere in the search reads a node --
// measured, the ONLY readers of the node list were `_cat`, which concatenates
// without looking, and `_list`, which runs once at the very end. The tree was
// write-only for the whole search, and the engine built one for EVERY candidate
// way at EVERY memo cell, of which exactly one is ever delivered.
//
// Measured on the 1824-case battery (`_prof.dart`): 155.0M objects, of which
// 75.8M were `_N`/`_T` tree nodes, at 36 ns each -- which is the whole of m87's
// 5398 ms. The four numbers cost 79.2M; the trees for the losers cost 75.8M.
//
// So a way now records HOW IT WAS MADE -- a leaf, a wrap, or the concatenation
// of two other ways -- and the tree is materialised once, from the way that
// wins. `_N`, `_T`, `_cat` and `_list` are all deleted; the provenance is five
// fields on a class that already existed, so the promise costs no allocation at
// all. This is the same argument `_T` already made for deferring `Match`, taken
// to its end: `_T` deferred the COPY but still allocated a node per way.
//
// I51: REACHING A LATER ALTERNATIVE CLAIMS THE EARLIER ONES FAILED, AND
...
bool _better(int cost, int net, int got, int site, _Way? b) =>
    b == null ||
    cost < b.cost ||
    (cost == b.cost &&
        (net > b.net ||
            (net == b.net &&
                (got > b.got || (got == b.got && site < b.site)))));

bool _beats(_Way a, _Way? b) => _better(a.cost, a.net, a.got, a.site, b);
...
/// I59: DID THIS EVALUATION DEPEND ON THE CAP?
///
/// Set wherever a candidate is refused for exceeding [_budget], and wherever a
/// provisional answer is read -- an in-cycle seed, or a cell computed earlier in
/// THIS round. Cleared on entry to every cell and restored on the way out, so a
/// cell learns whether anything beneath it could change when the cap rises.
bool _bound = false;

/// Which pass of the deepening loop is current. A cell computed in an earlier
/// pass and not marked final is stale; a final one never is.
int _roundNo = 0;
...
class _Cell {
  ...
  /// Budget-independent: nothing beneath this cell was refused for exceeding
  /// the cap, so no larger cap can change it. Survives every later round.
  bool fin = false;
  int gen = -1;
  int round = -1;
}
...
class SuperDot3 {
  ...
  MatchResult recover(String s) {
    ...
    _bound = false;
    // I59: THE ROUND TABLES ARE NO LONGER CLEARED BETWEEN ROUNDS. A cell that
    // never consulted the cap is reused as it stands; one that did is stale, and
    // the round stamp says so without a map to rebuild.
    _roundNo = 0;
    for (_budget = 0;
        _budget <= cap;
        _budget = _budget == 0 ? 1 : _budget * 2, _roundNo++) {
      // ABLATION: throw the round tables away exactly as m92 does, so the flag
      // can never save a cell. What is left is the flag's price on its own.
      _mc.clear();
      _me.clear();
  ...
  _Ways _fix(Map<Clause, List<_Cell?>> memo, Clause c, int pos,
    ...
    final cell = (memo[c] ??= List.filled(_in.length + 1, null))[pos] ??=
        _Cell();
    // I59. Nothing beneath it was ever refused for exceeding the cap, so no
    // later round can change it -- answer without touching [_bound], because a
    // reader of a final cell learns nothing about the cap from it.
    if (cell.fin) return cell.ways;
    final persist = _budget == 0;
    if (cell.inPath) {
      // Second visit with no result yet: the fixed point of a left recursive
      // cycle. Signal the ancestral frame and answer with the seed so far. The
      // seed is provisional whatever the budget is, so this always propagates.
      cell.foundLR = true;
      _bound = true;
      return cell.ways;
    }
    if (cell.done && (persist || cell.round == _roundNo) && cell.gen == gen[pos]) {
      // Computed already. In the budget-0 table that answer is the frozen
      // parser's and is good forever; in a round table it is good for this round
      // only, which is exactly what stops it being promoted.
      if (!persist) _bound = true;
      return cell.ways;
    }
    final outer = _bound;
    _bound = false;
  ...
  _Ways _expand(Clause c, int pos) {
    switch (c) {
      ...
      case FollowedBy f:
        ...
        if (w == null) return _none;
        if (w.length > _budget) {
          if (_budget > 0) _bound = true;
          return _none;
        }
        return _Way(pos, w.length, 0, 0, true, leaf: Filled(f, pos, w));
  ...
  _Ways _wrap(Clause c, int pos, _Ways w) {
    ...
    for (var e = w; e != null; e = e.next) {
      out = _Way(e.end, e.cost, e.got, e.net, e.synth,
          site: e.site, tail: e.tail, c: c, pos: pos, a: e, next: out);
    }
    return out;
  }
    ...
    for (final sub in subs) {
      ...
      for (var w = ways; w != null; w = w.next) {
        for (var x = _element(sub, w.end); x != null; x = x.next) {
          ...
          if (!_better(cost, w.net + x.net, w.got + x.got, _runs(w, x),
              _at(next, x.end))) {
            continue;
          }
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    // The loop above stopped on the CAP, not on the end of the input, so a
    // larger cap would try longer skips here.
    if (_budget < _in.length - pos) _bound = true;
    ...
    if (need != null && need > _budget) _bound = true;
      ...
      out = _put(
          ...
          w != null
              ? _Way(pos, need, 0, 0, true,
                  site: 1, tail: true, leaf: Filled(sub, pos, w))
              : _Way(pos, need, 0, 0, true,
                  site: 1,
                  tail: true,
                  c: sub,
