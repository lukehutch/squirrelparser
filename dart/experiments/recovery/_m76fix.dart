// m76 -- exact regular PEG obligations; one parser and one recovery memo.
//
...
// never become AST nodes.  Named recursion reached while left-expanding an
// obligation, and zero-width repetition inside an obligation, are rejected
// explicitly.  There are no tuning parameters.
...
class SuperDot3 {
  ...
  bool _chase(int cost, List<MatchResult> out) {
    ...
    while (st.isNotEmpty) {
      ...
      switch (why) {
        case _wNone:
          exit = 'wNone: no reason recorded at this cell';
          return false;
        case _wSub:
        ...
        case _wPure:
          final m = _parser.read(node.orig, pos);
          if (m.isMismatch || m.len != end - pos) {
            exit = 'wPure: oracle read differs (mismatch=${m.isMismatch} len=${m.len} want=${end - pos})';
            return false;
          }
        ...
        case _wDemand:
          ...
          if (demand == null) {
            exit = 'wDemand: clause is not a lookahead';
            return false;
          }
        ...
        default:
          ...
          switch (node) {
            ...
            case _Term():
              exit = 'term: a leaf never writes a structural reason';
              return false;
          }
      }
      if (proof == _dead) {
        exit = 'proof dead: an obligation became unsatisfiable mid-chase';
        return false;
      }
    }
    ...
    final okEdits = _edits.length == cost, okEnd = _atEnd(proof);
    if (!okEdits || !okEnd) {
      exit = 'end: edits=${_edits.length} cost=$cost atEnd=$okEnd';
    }
    return okEdits && okEnd;
  }
