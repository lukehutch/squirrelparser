...
class SuperDot3 {
  ...
  int lastCost = -1;
  final Set<int> probePos = {};
  int probeCalls = 0;
  int probeReach = 0;

  /// Distinct (clause, position, budget) cells whose repair layer was actually
  /// computed. A bottom-up chart evaluates each of these EXACTLY ONCE; the
  /// top-down engine recomputes one every time a different parent descends into
  /// it. probeCalls / probeCells is therefore the redundancy factor a chart
  /// removes -- the single number that decides whether the rewrite is worth it.
  final Set<String> probeCells = {};

  /// EVERY memo cell body the engine runs, at any budget and any cost layer --
  /// the true total work, of which [probeCells] is only the repair layer. This
  /// is the quantity a windowed chart must beat, so it is the one to compare
  /// against |window| x |grammar|.
  int probeBody = 0;
  final Set<String> probeAll = {};
  ...
  _Ways _fix(Map<Clause, List<_Cell?>> memo, Clause c, int pos,
    ...
    cell
      ...
      ..ways = _none
      ..foundLR = false;
    probeAll.add('${identityHashCode(memo)}\x00${identityHashCode(c)}\x00$pos');
    while (true) {
      probeBody++;
      final r = body(c, pos);
  ...
  Match? _term(Terminal t, int pos) {
    if (_budget == 0 && pos > probeReach) probeReach = pos;
    switch (t) {
      case Str s:
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    probePos.add(pos);
    probeCalls++;
    probeCells.add('${identityHashCode(sub)}\x00$pos\x00$_budget');
    var out = direct;
