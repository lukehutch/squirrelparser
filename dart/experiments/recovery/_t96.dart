// m96 -- I61 over I60: the fifth key, with the SWEPT repetition.
//
// m95's key on m94's `_rep`. m94 proved the sweep computes the same ways as the
// wave and differs only in the order it returns them, so if the fifth key has
// really replaced the list order, this file and m95 must agree on every case.
// That is the test; the sweep is the reason to want it.
//
// m92 with a fifth and final key. Four keys order the search -- cost, then
// evidence, then consumption, then the number of repair runs -- and when all
// four tie the engine took whichever way it happened to meet first in a linked
// list. That is the D2 violation that has been standing since I30: a linked
// list is not a principle, and the answer changed when an unrelated rewrite
// changed the list's order (m94 differs from m92 on 4 of 1824 cases while
// computing, provably, the same SET of ways -- 0 differences over 1,197,132
// `_rep` calls).
//
// The missing principle is in the machine already: A READER GOES LEFT TO RIGHT.
// Two readings that discard the same characters, at the same price, in the same
// number of runs, still differ in WHERE the first run falls. The one whose
// first repair is later agreed with the input for longer before it doubted, so
// everything it committed to before that point it committed to on evidence
// alone. Prefer the LATER first doubt.
//
// It is a sound key, not a preference. A tie on `cost` means either BOTH
// readings repair or NEITHER does, because cost is exactly the count of
// repaired characters. If neither repairs, both first doubts are `_never` and
// the key is silent. If both repair, then extending on the right cannot move a
// first doubt that is already on the left, so the winner of a tie stays the
// winner of the tie -- which is what `_put` needs to keep one way per ending.
//
// Derived from the reading order and nothing else: no parameter, no exchange
// rate, no appeal to which answer scored better.
...
_Way _cons(_Way w, _Way? next) => _Way(w.end, w.cost, w.got, w.net, w.synth,
    site: w.site, tail: w.tail, doubt: w.doubt,
...
bool _better(int cost, int net, int got, int site, int doubt, _Way? b) =>
    b == null ||
    cost < b.cost ||
    (cost == b.cost &&
        (net > b.net ||
            (net == b.net &&
                (got > b.got ||
                    (got == b.got &&
                        (site < b.site ||
                            (site == b.site && doubt > b.doubt)))))));

bool _beats(_Way a, _Way? b) =>
    _better(a.cost, a.net, a.got, a.site, a.doubt, b);
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= cap; _budget = _budget == 0 ? 1 : _budget * 2) {
      ...
      for (var e = _element(top, 0); e != null; e = e.next) {
        ...
        tieProbe.add('    cand cost=${cand.cost} net=${cand.net}'
            ' got=${cand.got} site=${cand.site} doubt=${cand.doubt}'
            ' tail=${cand.tail} synth=${cand.synth} end=${e.end}');
      }
      if (best != null) {
        tieProbe.add('  WON cost=${best.cost} net=${best.net} got=${best.got}'
            ' site=${best.site} doubt=${best.doubt} tail=${best.tail}'
            ' synth=${best.synth}');
        final ks = <MatchResult>[];
        _emit(best, ks);
        void walk(MatchResult m) {
          if (m is SyntaxError) {
            tieProbe.add('    SKIP  pos=${m.pos} len=${m.len}');
          } else if (m is Filled) {
            tieProbe.add('    FILL  pos=${m.pos} text=${m.text}');
          } else if (m is Match) {
  ...
  _Ways _wrap(Clause c, int pos, _Ways w) {
    ...
    for (var e = w; e != null; e = e.next) {
      out = _Way(e.end, e.cost, e.got, e.net, e.synth,
          site: e.site, tail: e.tail, doubt: e.doubt,
