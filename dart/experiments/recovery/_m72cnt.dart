...
//
//       ONE oracle call survives in the reconstruction, and it is not a
//       re-derivation. A reason is a SUFFICIENT explanation, not a canonical
//       one: the budget-zero walk settles a whole subtree with a single
//       oracle call and records `_wPure`, but a cell first reached at a
//       HIGHER budget never meets that shortcut -- the walk returns before
//       the decomposition -- so above zero only the split ever writes. Both
//       describe the same span at the same price, and the oracle's is the
//       tree this parser actually produces: `"ab"` is one match, not a chain
//       of two. `_wholesale` prefers it. It has no candidates and no
//       backtracking, and when it declines the recorded reason answers, so
//       it cannot bring back the doubt. `cost == 0` is what makes it safe --
//       it never CREATES a repair, only re-describes a cell the search
//       already priced at zero, which is why it does not leak past I27.
//
//       It belongs to `_build` and NOT to `_row`, which is the one asymmetry
//       in the file. `_row` walks a spine's SUFFIX nodes, and a suffix shares
//       its `orig` with the whole sequence, so asking the oracle about `orig`
//       at the suffix's position asks about a clause that node does not stand
//       for -- measured, it drops certificates and builds trees with gaps in
//       them. Reading the RECORDED `_wPure` there is safe for exactly the
//       reason re-deriving it is not: `_wPure` is only ever WRITTEN where
//       `orig` describes the node.
//
//       What the chase buys beyond the deletion is that it is READ-ONLY.
//       m71's `_ends` calls `_entryAt`, which is `putIfAbsent`, and then
//       `_run` -- so its reconstruction creates memo cells and relaxes them,
//       making the witness descent a search that extends the fixed point.
//       `_cellAt` is a plain lookup. Measured: m71 ends the 519-mutant
//       battery holding 520823 cells against m62's 511209, and m72 holds
//       511209 -- m62's count exactly, on an engine that answers the true
//       PEG language where m62 does not.
//
...
class _Frame {
  ...
  /// its answers without a lookup, and how many answers it had when this frame
  /// last looked (see the restart in `_Cons`).
  _Entry? headEntry;
  int headLen = -1;
...
class SuperDot3 {
  ...
  static int nTight = 0, nPass = 0;

  /// Diagnostic ONLY: skip I28's certificate demand, which on the cost
  /// path runs the whole reconstruction. Changes the answer; used to
  /// localise where the battery gap against m62 lives.
  static bool skipCert = false;

  /// Diagnostic ONLY: start the pass already guarded, so I28's relaxed
  /// first leg never runs. Prices the premise that relaxing is cheaper
  /// than the certificate it obliges you to buy.
  static bool forceGuard = false;

  /// How many times the RELAXED pass computed something the guarded pass
  /// would have computed differently. Zero means the two passes are the
  /// same DP, so the relaxed answer already IS the tight answer.
  int nRelax = 0;

  /// Diagnostic: keep the reconstruction, drop I5's re-parse of the
  /// emitted string. Asks whether _build's own failure is what catches
  /// the cases the tighten exists for, or whether the parse does the work.
  ...
  /// A new answer goes in at its place in SPLIT order -- by end, then by
  /// obligation -- and not at the back. The list is what a `_Cons` hands to
  /// `_keepBest` as head candidates, so keeping it ordered here is what makes
  /// the shortest split arrive first there, at no cost in the loop: the search
  /// for the key already walks the list, so the insertion point comes for free
  /// alongside it, and a longer end is the common case, which still appends.
  ///
  /// A `_Cons` may be parked on a tail while this runs, holding an index into
  /// this list. An insertion before that index shifts entries RIGHT, so it can
  /// re-offer a split already taken -- `_put` is idempotent, so that is free --
  /// and it can never move an unoffered one behind the cursor.
  ///
  /// Ordering the list pays for itself twice. Split order is a TOTAL order
  /// computable from the key alone -- by end, then by key -- so the walk that
  /// looks for the key does not have to be a walk: it bisects, and a search
  /// that misses has already stopped exactly where the new key belongs. The
  /// mean list holds 13.9 entries and the longest 90, so this replaces ~7
  /// comparisons with ~4 and finds the insertion point for nothing.
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    ...
    if (at == out.length) {
      ...
      out.insertAll(at, [key, cost, reg, why]);
    }
    return true;
  }
        ...
        if (!m.isMismatch) {
          ...
          if (m.len == 0) {
            ...
            if (!_guarded &&
                node is _Cons &&
                identical(node.tail, node) &&
                _notFirst(node.head.orig, _free) != _free) {
              nRelax++;
            }
            if (!_unmeetable(owed)) {
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        while (f.pc < alts.length) {
          ...
          if (!_guarded && _meet(c, _guardsOf(node)[f.pc]) != c) nRelax++;
          if (_unmeetable(owe)) {
      ...
      case _Cons():
        ...
        if (f.pc == 0) {
          if (loops) {
            ...
            if (!_guarded && _notFirst(node.head.orig, c) != c) nRelax++;
        ...
        // I29's tie-break, and the whole of it: among splits that reach the
        // same value, the one a recursive-descent PEG would reach FIRST -- the
        // shortest head -- is offered first, and `_keepBest`'s refusal of an
        // equal entry makes that the reason. It costs nothing to arrange here,
        // because `_keepBest` keeps every answer list in split order; the
        // alternative was arrival order, which offers the CHEAPEST head first
        // and so absorbs the repair into the head and re-reads its neighbours.
        // An alternation needs nothing: its branches merge in source order
        // already, which is the same law one node type over.
        //
        // Ordering does cost one thing an append never could: while this frame
        // is parked on a tail, a new split can land BEHIND its cursor, where an
        // appended one was always ahead of it. So the walk restarts whenever
        // the list has grown -- re-offering a split is free, `_put` being
        // idempotent, and growth is bounded by the number of distinct splits.
  ...
  bool _certified(int cost) {
    ...
    return lastVerified =
        _root != null && (skipVerify || _verify(_root!));
  }
