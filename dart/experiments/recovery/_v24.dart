...
class Squirrel {
  ...
  /// This round's budget, which [_budget] is the unspent remainder of. The
  /// probes ask at a budget of 0 from inside a round that has more; only the
  /// FIRST round is the frozen parser, so only the round can say so.
  int _round = 0;
  int _budget = 0;
  ...
  /// PEG's own way, or none: the farthest way that is still PEG's reading.
  ///
  /// [_prune] has already reduced [ws] to one way per end and demoted every
  /// PEG way that stops short of the farthest, so at most one survives and
  /// finding it is one pass.
  static List<_Way> _first1(List<_Way> ws) {
    if (ws.length <= 1) return ws;
    for (final w in ws) {
      if (w.peg) return [w];
    }
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      cur = _prune(next);
      // THE FIRST ROUND IS THE FROZEN PARSER, SO ITS SEQUENCE IS PEG'S.
      //
      // A prefix that is clean but is not PEG's reading is already dead in this
      // round. [then] takes the MINIMUM key, so any chain built on it is at
      // most `_far`, and `recover` refuses exactly `_far`; buying it back would
      // take a repair, and this round has nothing to spend. PEG asks slot i at
      // ONE position -- where slot i-1 stopped -- and this is that, restored.
      //
      // It is the whole of the cross product. `S <- W W W W W W W W` over clean
      // text was asking eight repetitions at every end the ones before them
      // reached, which is quadratic and got 3883x slower than PEG by n=1024; a
      // left-recursive rule was re-extending every iteration it had already
      // extended, which is the same shape and the same fix. Neither survives.
      //
      // Only the FOLD is cut, not the cell. What the cell hands back still
      // holds every clean reading, because the next round reads this round's
      // answer as its seed rather than deriving it again -- pruning the memo
      // too was measured, and it cost more in re-derivation than it saved.
      if (_round == 0) cur = _first1(cur);
    }
