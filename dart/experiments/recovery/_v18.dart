...
class Squirrel {
  ...
  /// Whether the answer being computed DEPENDS ON THE ROUND: set wherever the
  /// budget actually refused something, or a left-recursive seed was read
  /// before it had an answer. Everything else a round derives it would derive
  /// again, unchanged, at every larger budget.
  bool _clip = false;
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    var settled = false;
    while (true) {
      // ASK WHETHER THE ROUND MATTERED. Nothing below may answer for an outer
      // computation, so the flag is saved, cleared, and merged back.
      final outer = _clip;
      _clip = false;
      ...
      e.ways = got;
      e.has = true;
      // A CELL NO BUDGET CUT NEED NEVER BE ASKED AGAIN. Measured, 65.4% of
      // every re-computation a later round makes returns exactly what the
      // round before it had; this is the test for which ones can.
      e.at = settled ? _never : _budget;
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        if (i == 0) {
          var k = pos + 1;
          for (; k <= pos + _budget && k <= _in.length; k++) {
            final full = _budget;
            final held = _clip;
          ...
          if (k > pos + _budget && pos + _budget < _in.length) _clip = true;
          continue;
        }
        ...
        var k = w.end + 1;
        for (; k <= w.end + room && k <= _in.length; k++) {
          final full = _budget;
          final held = _clip;
          _budget =
        ...
        if (k > w.end + room && w.end + room < _in.length) _clip = true;
      }
      if (next.isEmpty) {
        whole = false;
        break;
  ...
  List<_Way> _terminal(Clause c, int pos) {
    ...
    if (c is Str) {
      for (var k = c.text.length - 1; k >= 1; k--) {
        if (c.text.length - k > _budget) {
          _clip = true;
          continue;
        }
        if (pos + k > _in.length) continue;
