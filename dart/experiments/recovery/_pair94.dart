// _pair94.dart -- m92 vs m94 (the swept repetition) interleaved within each case.
//
...
//
// Usage: dart run _pair94.dart [reps]
...
void main(List<String> argv) {
  ...
  final b = <String, MatchResult? Function(String)>{
    for (final c in corpora)
      c.name: g94.SuperDot3(rules: rulesOf[c.name]!, topRuleName: c.top).recover
  };
  ...
  for (var r = 0; r <= reps; r++) {
    ...
    print('pass $r${r == 0 ? ' (warm-up)' : ''}  m92 ${x.toStringAsFixed(0)} ms'
        '   m94 ${y.toStringAsFixed(0)} ms');
  }

  final ma = median(ta), mb = median(tb);
  print('');
  print('median of $reps passes');
  print('  m92  (wave: map + frontier set)   ${ma.toStringAsFixed(0)} ms');
  print('  m94  (sweep: one visit per end)   ${mb.toStringAsFixed(0)} ms'
