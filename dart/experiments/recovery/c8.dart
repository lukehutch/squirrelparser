// c8.dart -- THE STANDING ENGINE: c7 after an adversarial audit of every
// assumption, with ONE FROZEN-ANSWER PROTOCOL. freeze(pos) is the single
// way any node kind hands the pure parser's reading to the priced fiber
// -- the deny-scan, budget zero at references, budget zero at composites,
// and a terminal's own read are all the same call. go() is everywhere
// "freeze, or repair": a terminal owes, a literal folds, a reference
// lifts, a composite grows. The rule cell OWNS its frozen way (built once
// however many denials read it, invalidated only by seed growth), and a
// terminal's pin bit is the leaf case of the net walk, known without
// walking.
//
// The audit's refutations are recorded in LESSONS: a stateless predicate
// re-runs a literal fold two levels away (a front can be the only cache
// over an UNCACHED sub chain -- memo state acts at a distance); a freeze
// cache keyed by allocating a front per probed position costs more than
// the matches it saves; and the read-view cache was unresolvable under
// machine load and is dropped for purity (a front is read-only on reads).
//
// c8 keeps c7 whole: one left-recursion law for both fibers. c6's fold-in exposed a duplication the split
...
/// input it accepted would be input the assertion does not consume. (A
/// stateless predicate was measured: its front was the only cache over a
/// Ref-to-literal sub chain, and deleting it re-ran the literal fold per
/// query.)
...
class _Ref extends _N {
  ...
  /// The rule's finished subtree is reused WHOLE, as ONE way the cell
  /// owns: built once however many denials read it. Every reference to
  /// the rule shares it (the tree label differs only in which equal-named
  /// Ref instance it carries, which nothing reads).
  @override
  _Way? freeze(Squirrel e, int pos) {
    if (pos > e._n) return null;
    ...
    final m = c.res;
    if (m == null) return null;
    return c.way ??= _Way(pos + m.len, 0, 0, e._netOf(m), _peg,
...
abstract class _Term extends _N {
  ...
  /// A terminal's read IS its frozen way; [pin] is what [_netOf] computes
  /// on a leaf, known here without the walk.
  @override
  _Way? freeze(Squirrel e, int pos) {
    final m = match(e, pos);
    return m == null
        ? null
        : _Way(pos + m.len, 0, 0, pin ? m.len : 0, _peg, what: m);
  }
...
class Squirrel {
  ...
  List<_Way> grow(_Comp c, int pos) {
    ...
    if (_budget == 0 && f.at < 0 && pos < _n) {
      ...
      // pure parser for ANY node, so the frozen answer is the whole front
      // -- one protocol (freeze) at every node kind
      f.at = 0;
      final w = c.freeze(this, pos);
      if (w != null) f.add(w);
      return f.ways();
    }
    f.inPath = true;
