...
class SuperDot3 {
  ...
  int recoverCost(String input) {
    ...
    //
    // I13: THE BUDGET IS A BOUND, NOT A TARGET, SO THE LADDER DOUBLES. A cell's
    // value at budget k is the MINIMUM over every repair costing at most k, so the
    // goal's best entry is the exact answer at EVERY k that reaches it and not only
    // at the smallest -- `_editCount` reads the cost back out of the Delta, and a
    // Delta-tied rival costs the same, so it was admissible at the same k. Deepening
    // one at a time was therefore never a correctness requirement; it was a guess
    // that the cheapest round to find the answer in is the tightest one. MEASURED,
    // that guess is wrong by 2x, because the rounds BEFORE the answer are not free:
    // on the cost-10 latency case the eleven rounds cost 256919 relaxations of which
    // the last, the only one that answered, cost 44229. Doubling pays at most one
    // round at up to twice the budget it needed and skips the rest of the ladder.
    //
    // AND THE UNREPAIRABLE CASE STOPS BEING QUADRATIC. When no repair exists the
    // ladder must climb all the way to the derived ceiling, which is O(n): 2101
    // rounds become 14. That is the same asymmetry the `RR` tag names -- the price
    // of a NO is out of all proportion to the price of a YES -- charged to the
    // schedule this time rather than to the stack.
    for (var k = 0;
        k <= maxCost;
        k = k == 0
            ? 1
