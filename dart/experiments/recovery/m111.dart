// m111.dart -- m110 with I54 narrowed from a cross-ending dominance prune
// to the equal-cost tie the brief actually decided. Generated by mk111.py.
...
class SuperDot3 {
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    // I67: A TIE IS THE ONLY THING THE BRIEF DECIDED, SO IT IS THE ONLY THING
    // THIS MAY PRUNE.
    // `need < minSkip` compares two candidates that end in DIFFERENT PLACES --
    // the fill at `pos`, the skip past it -- so as a dominance rule it is
    // unsound: the continuation from `pos` can be cheaper than any continuation
    // from the skip's ending by more than the local difference. The brief only
    // ever settled the TIE (`[,2,` -> `[2,`, both cost 1), so settle the tie and
    // let the cost function judge the rest.
    if (need != null &&
        need <= _budget &&
