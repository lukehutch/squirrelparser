// m92 -- I58: A FREE WAY IS ALREADY THE PROOF, SO DO NOT GO AND FETCH IT.
//
...
// PROBE ONLY. Cell bodies executed, split by round index, so the deepening
// loop's later rounds can be priced against its first.
int _cells = 0;
final List<int> cellsByRound = [];
final List<int> casesByRounds = [];
int nElem = 0, nElemFree = 0, nElemPure = 0, nElemSearch = 0;
int nMc = 0, nMe = 0, nTerm = 0;
...
class SuperDot3 {
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= cap; _budget = _budget == 0 ? 1 : _budget * 2) {
      ...
      while (cellsByRound.length <= _ri) {
        cellsByRound.add(0);
      }
      cellsByRound[_ri] += _cells - _before;
      _ri++;
      if (best != null) {
        while (casesByRounds.length <= _ri) {
          casesByRounds.add(0);
        }
        casesByRounds[_ri]++;
        lastCost = best.cost;
  ...
  _Ways _fix(Map<Clause, List<_Cell?>> memo, Clause c, int pos,
    ...
    _cells++;
    if (identical(memo, _me)) {
      nMe++;
    } else if (identical(memo, _mc)) {
      nMc++;
    }
    cell
      ..inPath = true
