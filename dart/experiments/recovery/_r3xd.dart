// r2.dart -- r1, plus the fill: a repair may also supply what is absent.
//
...
//
// r1 could only ever SKIP input, so a truncated document -- whose only repair is
// a missing closing token -- was beyond what it could express. r2 adds the other
// half: at every frontier site the clause may also be GRANTED where it stands,
// at no width, behind the same anonymous mark. Nothing is spelled, so no
// terminal of an absent class is invented. Because that costs one obligation the
// input never supplied -- the same as skipping one character -- the fill is
// offered in the l = 1 pass and weighed against the one-character skip there,
// rather than in a pass of its own, where the widening order and not the cost
// would decide between them.
//
// What a fill may still get wrong is the TREE. A mark carries no rule label, so
// supplying `','` adds nothing a consumer can read; but supplying a `Value` puts
// a node in the tree that says an object or an array or a number was here, on no
// evidence, and picking which is exactly the invention the brief forbids. The
// line between them is not the clause's kind but whether the grammar DETERMINES
// its shape: `A <- [ab]` derives two strings and one tree, `Value` derives one
// tree per arm. So a fill is admissible when every zero-width node it creates
// has a shape the grammar fixes -- [_determined] -- and inadmissible otherwise.
// This is `_zerowidth`'s own test for invention, applied where the repair is
// chosen rather than only counted afterwards, exactly as [_explained] does for
// `_freespan`. It decides all three acceptance cases with no tie-break: `,3true`
// fills a `','`, `xa` fills an `A`, and `[,2,` is refused a `Value` and deletes.
...
class Squirrel {
  ...
  /// Memo for [_determined]. Keyed by clause identity, like every other table
  /// here, and independent of the input, so it outlives [_forget].
  ...
  bool _descend(Clause c, int pos) {
    ...
    if (c is First) {
      // The brief's rule exactly: a First whose arms all failed is not itself a
      // frontier node, and every arm is traversed, because a repair reachable
      // through any of them advances the parse.
      var any = false;
      for (final s in c.subClauses) {
        if (_walk(s, pos)) any = true;
      }
      return any;
    }
    if (c is Seq) {
  ...
  (int, int, int) _cost(MatchResult root) {
    final t = root.isMismatch ? _salvage(rules[topRuleName]!, 0) : root;
    if (t.isMismatch) return (_in.length + 1, 0, 0);
    final (del, gap) = _edits(t);
    return (del + (_in.length - t.len), gap, _net(t));
  }
  ...
  /// one consumes a character without explaining it. This is what separates two
  /// readings that cost the same and cover the same text: `[1,[2,` read as one
  /// String matches all six characters through `[^"\\]`, which asserts nothing
  /// about any of them, where the Array reading pins `[`, `,` and the digits.
  /// Preferring the reading that explains more is what stops the SWALLOW.
  int _net(MatchResult m) {
    final c = m.clause;
    var n = c is Str || c is Char || (c is CharSet && !c.inverted) ? m.len : 0;
    for (final k in m.subClauseMatches) {
      n += _net(k);
    }
    return n;
  ...
  /// Which characters the tree [root] explains: inside its span, and not inside
  /// any span a repair skipped.
  ///
  /// A repair that deletes one of these contradicts the parse it is trying to
  /// help -- it destroys a match the parser had already made. `Top <- Chunk 'z';
  /// Chunk <- 'a'* 'b'` on `abab` is the case: `Chunk` matches `ab`, and the
  /// stopping point of `'a'*` at 1 offers a repair that deletes the very `b`
  /// the `'b'` subclause matched, so `'a'*` can run on to the second `a`. It
  /// reads two characters further for one deletion, and it is still wrong: the
  /// input said `b` there and the parse had already agreed.
  ///
  /// This is `_freespan`'s rule, applied where the repair is chosen rather than
  /// only checked afterwards.
  List<bool> _explained(MatchResult root) {
    final out = List.filled(_in.length, false);
    final end = root.pos + root.len;
    for (var i = root.pos; i < end && i < _in.length; i++) {
      out[i] = true;
    }
    void skipped(MatchResult k) {
      if (k is SyntaxError) {
        final e = k.pos + k.len;
        for (var i = k.pos; i < e && i < _in.length; i++) {
          out[i] = false;
        }
      }
      k.subClauseMatches.forEach(skipped);
    }
  ...
  /// Fewest edits first; among equals, fewest DELETIONS, because destroying a
  /// character the input supplied contradicts evidence where a gap only records
  /// its absence; among those, the reading that EXPLAINS the most.
  static bool _cheaper((int, int, int) a, (int, int, int) b) {
    final ea = a.$1 + a.$2, eb = b.$1 + b.$2;
  ...
  ///
  /// This is what makes a fill honest. A clause the grammar pins to one shape
  /// stands for a hole whose contents were never in question -- `'}'`, or an
  /// `A <- [ab]` that is a leaf holding one character either way -- and marking
  /// it absent claims nothing the grammar had not already said. A clause with a
  /// choice in it does not: `Value` is an object or an array or a number, and a
  /// zero-width `Value` node asserts one of them happened. Choice is the whole
  /// of the difference, so `First`, `Repetition` and `Optional` are the whole of
  /// the exclusion; a predicate contributes no node at all.
  ///
  /// A rule that re-enters itself reads `false` while in progress, which is the
  /// conservative direction: a fill is refused, never wrongly allowed.
  ...
  /// Whether [root] holds a node of no width, standing over a mark, whose shape
  /// the grammar leaves open -- a subtree the tree claims on no evidence.
  ///
  /// Only a node that is BOTH empty and marked is asked: a `ZeroOrMore` that
  /// matched no times is ordinary PEG and says nothing was there, whereas the
  /// same node wrapped around a mark says something was required and is missing.
  bool _invents(MatchResult root) {
    var bad = false;
    bool hollow(MatchResult m) {
      if (m.len != 0) {
        m.subClauseMatches.forEach(hollow);
        return false;
      }
      var mark = m is SyntaxError;
      for (final k in m.subClauseMatches) {
        if (hollow(k)) mark = true;
      }
      final c = m.clause;
      if (mark && c != null && !_determined(c)) bad = true;
      return mark;
    }
  ...
  MatchResult? _round(MatchResult root, (int, int, int) base) {
    final tree = root.isMismatch ? _salvage(rules[topRuleName]!, 0) : root;
    final held = tree.isMismatch
        ? List.filled(_in.length, false)
        : _explained(tree);
    _frontier.clear();
    _seen.clear();
    _walked.clear();
    ...
    for (var l = 1; l <= _in.length; l++) {
      ...
      var bestPos = -1;
      var best = base;
      void consider(Clause c, int p, Repaired repair, bool fill) {
        (_repairs[c] ??= {})[p] = repair;
        final got = _parse();
        final cost = _cost(got);
        // Asked of exactly the tree the cost was read from, so the two agree.
        final invents = fill &&
            _invents(got.isMismatch ? _salvage(rules[topRuleName]!, 0) : got);
      ...
      for (final (c, p) in _frontier) {
        // The fill. It needs no room, so the length guard below is not about it:
        // a clause missing at the very end of the input is the truncation case,
        // and it lives exactly at `p == _in.length`. It is refused INSIDE what
        // the parse already read, which is [_explained]'s rule again in the only
        // form a fill can break it. A fill deletes nothing, so it cannot destroy
        // a match; what it can do is make the re-parse read those characters
        // some other way, and `[1,[2,` is the case -- supply the quotes at 0 and
        // 6 and `String <- '"' Character* '"'` swallows the whole document for
        // two marks, cheaper than the array the parser had already built out of
        // it. A fill belongs where the parse RAN OUT, and that is a position it
        // never explained. A predicate is excluded as well, because it consumes
        // nothing: granting it adds no structure and only asserts that what is
        // plainly there is absent.
        if (l == 1 && c is! FollowedBy && c is! NotFollowedBy) {
          consider(
              c, p, Repaired(p, 0, const [], [SyntaxError(pos: p, len: 0)]), true);
        }
        if (p + l > _in.length) continue;
        // Never delete a character the parse already explained.
        var destroys = false;
        for (var i = p; i < p + l; i++) {
          if (held[i]) destroys = true;
        }
        ...
        consider(c, p, Repaired(p, l + r.len, [r], [SyntaxError(pos: p, len: l)]),
            false);
      }
      final chosen = bestClause;
      if (chosen == null) continue; // No site helped; widen the span.
      _repairs[chosen]![bestPos] = bestRepair!;
      (_tried[chosen] ??= {}).add(bestPos);
      return _parse();
    }
    return null;
  }
  ...
  MatchResult _partial(Clause c, int pos) {
    ...
    if (c is Seq) {
      ...
      for (var i = 0; i < c.subClauses.length; i++) {
        ...
        if (kids.isNotEmpty) {
          for (var j = open; j < c.subClauses.length; j++) {
            kids.add(SyntaxError(pos: curr, len: 0));
          }
        }
        break;
      }
