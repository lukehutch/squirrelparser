// m36 -- m26, m35 and m27 as one mechanism.
//
// THE BUDGET MEASURES HOW MUCH OF THE INPUT IS STILL UNKNOWN.
//
// That single reading unifies three engines that each failed where the other two
// were strong:
//
//   * m27 guards a Repetition's stop with `!body` evaluated on the ORIGINAL input.
//     PEG quantifies over the REPAIRED string, so the guard is exact exactly when
//     no edit can occur -- which is the definition of budget 0. m27 fired it at
//     every budget and lost minimality (hist {1:478, 2:41}).
//   * m32/m35 "completed" level 0 by enumerating every iteration boundary and every
//     alternative. That is the CFG language, not the PEG one -- it is precisely the
//     bug peg_conformance reports -- and it is where the 20% battery cost came from.
//   * m26's greedy oracle level 0 was therefore never incomplete. It is the
//     PEG-EXACT level 0, and it is narrow and O(1) for the same reason.
//
// So: where the budget is 0 the input is KNOWN, PEG's deterministic semantics are
// exactly computable, and enforcing them is free correctness that also keeps the
// end-maps narrow. Where the budget is positive the input is UNKNOWN and the CFG
// relaxation is the correct sound over-approximation. The exact budget -- which
// m28/m30/m31/m33/m34 all failed to remove -- now does triple duty: it prunes, it
// bounds the descent, and it decides where PEG semantics are decidable.
//
...
class SuperDot3 {
  ...
  Map<int, int> _compute(Clause c, int dot, int pos, int b) {
    ...
    if (c is First || c is Optional) {
      ...
      for (final sub in _alts(c)) {
        ...
        // Ordered choice commits, by the same argument as the stop above: only
        // decidable, and so only enforced, where nothing can be edited.
        if (b == 0 && !_mayFall(sub, pos)) return out;
      }
      // The empty branch of `?` is just its last alternative, reached by falling
      // off the end of the same loop.
      if (c is Optional) out[pos] = 0;
      return out;
    }
  ...
  /// Whether the EARLIER option fails here, so a later one is reachable. PEG's
  /// ordered choice, greedy `?` and possessive `*` are all this one question. Only
  /// consulted at budget 0, where the original input is the repaired string and the
  /// parser therefore answers it exactly.
  bool _mayFall(Clause earlier, int pos) {
    final m = earlier.match(_parser, pos);
    return m.isMismatch || m.len == 0;
  }

  Map<int, int> _chain(Clause c, int dot, int pos, int b) {
    final out = <int, int>{};
    final limit = (b + 1) * _M;
    // A Repetition may stop here only where its body fails. That is PEG's rule, and
    // at budget 0 -- and ONLY at budget 0 -- the original input IS the repaired
    // string, so consulting the parser decides it exactly. Above budget 0 the stop
    // point depends on edits the continuation has not chosen yet, and the free stop
    // is the correct sound over-approximation.
    if (_done(c, dot) &&
        (b >= 1 || c is! Repetition || _mayFall(_elem(c, dot), pos))) {
      out[pos] = 0;
    }
