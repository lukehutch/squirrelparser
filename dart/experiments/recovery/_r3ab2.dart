...
class Squirrel {
  ...
  List<_Way> _terminal(Clause c, int pos) {
    ...
    if (c is Str) {
      for (var k = c.text.length - 1; k >= 1; k--) {
        ...
        final fills = _align(c.text, pos, k);
        ...
        out.add(_Way(
            ...
            k,
            false,
            Repaired(c, pos, k, const [],
                [for (final f in fills) SyntaxError(pos: f, len: 0)])));
      }
    }
  ...
  /// Where the missing characters of [text] go, if the [k] input characters at
  /// [pos] are exactly the ones it does supply, in order; null if they are not
  /// a subsequence of it at all. Greedy is exact for a subsequence test.
  List<int>? _align(String text, int pos, int k) {
    final fills = <int>[];
    var j = 0;
