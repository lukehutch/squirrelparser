// Scratch: the gate for m45's derived deepening ceiling. m44 and everything
// before it stop at `maxCost = 40` and report -1 -- the whole input as one error
// span -- for any repair that costs more. m45 derives the ceiling as
...
void main() {
  ...
  print('${'case'.padRight(58)} truth  m26  m42  m44  m45');
  var bad = 0;
  for (final (title, grammar, top, input, truth) in cases) {
    ...
    if (d == truth.toString()) {
      final r = g45.SuperDot3(rules: rules, topRuleName: top).recover(input);
      final spans = r.errorSpans.fold(0, (s, e) => s + e.len);
      print('${" " * 58}   m45 tree: root len ${r.root.len}/${input.length}, '
