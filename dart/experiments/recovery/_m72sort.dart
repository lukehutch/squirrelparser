...
class SuperDot3 {
  ...
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    ...
    // Only a new key needs a place, and a key is new once and rewritten many
    // times, so the walk above stays exactly what it was. A wider budget
    // reaches further, so a new split almost always belongs after the last one:
    // ask that first, and the ordering costs one comparison.
    out
      ..add(key)
      ..add(cost)
      ..add(reg)
  ...
  /// Put an answer list into split order -- by end, then by obligation -- in
  /// place and without allocating. Insertion sort is the right one here twice
  /// over: it costs one scan when the list is already ordered, which is the
  /// common case and the whole point of paying at the consumer, and its work is
  /// the number of inversions rather than the length.
  void _order(List<int> out) {
    for (var i = 4; i < out.length; i += 4) {
      final k = out[i], c = out[i + 1], r = out[i + 2], w = out[i + 3];
      final e = _endOf(k);
      var j = i - 4;
      while (j >= 0) {
        final pe = _endOf(out[j]);
        if (pe < e || (pe == e && out[j] < k)) break;
        out[j + 4] = out[j];
        out[j + 5] = out[j + 1];
        out[j + 6] = out[j + 2];
        out[j + 7] = out[j + 3];
