...
class SuperDot3 {
  ...
  Map<int, int> _ends(Clause c, int dot, int pos, int b) {
    ...
    _memoBudget[key] = b;
    _memo[key] = const {}; // reentrancy guard
  ...
  Map<int, int> _compute(Clause c, int dot, int pos, int b) {
    ...
    // A4: pure unions. No spans, no dots, no recovery logic at all.
    if (c is First) {
      final out = <int, int>{};
    ...
    if (c is Optional) {
      final out = {pos: 0};
      for (final e in _ends(c.subClause, 0, pos, b).entries) {
        _put(out, e.key, e.value);
      }
      return out;
    }
  ...
  MatchResult _build(Clause c, int pos, int end, int d, int b) {
    ...
    if (c is Optional) {
      if (_ends(c.subClause, 0, pos, b)[end] == d) {
        return Match(c, pos, end - pos,
            subClauseMatches: [_build(c.subClause, pos, end, d, b)]);
      }
