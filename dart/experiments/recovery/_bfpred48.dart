// Scratch: I6 (m48) against BRUTE-FORCE ground truth, next to m46.
//
...
int run(List<(String, String, String, String, List<String>)> block,
  ...
  for (final (title, grammar, top, alphabet, inputs) in block) {
    ...
    for (final s in inputs) {
      ...
      final c47 =
          cell(g48.SuperDot3(rules: rules, topRuleName: top).recoverCost, s);
        ...
        kind = n == null
            ...
            : n < 0
                ...
                : (t == null || n < t)
                    ? '  UNDER-REPORT'
                    : '  too high';
        worse.add('$title / "$s": true $want, m48 $c47 --$kind');
      }
  ...
  print('  block total: m46 ${total - bad46}/$total, m48 ${total - bad47}/$total');
  return total - bad47;
}
...
void main() {
  ...
  print('\nm47: $ok/45 on the comparable block, $okNull/14 on the nullable block');
  if (worse.isEmpty) {
    print('m48 matches brute force everywhere above');
  } else {
    print('m48 residual (${worse.length}):');
...
void spelling(String title, String asClass, String asPredicate, String alphabet,
  ...
  print('${'input'.padRight(9)}${'true'.padLeft(6)}${'m46[^]'.padLeft(8)}'
      '${'m46!'.padLeft(6)}${'m48[^]'.padLeft(8)}${'m48!'.padLeft(6)}   verdict');
  var bad = 0;
  for (final s in inputs) {
    ...
    final c = cell(
        g48.SuperDot3(rules: classSpelling, topRuleName: 'S').recoverCost, s);
    final d = cell(
        g48.SuperDot3(rules: predSpelling, topRuleName: 'S').recoverCost, s);
  ...
  print(bad == 0
      ? 'm48: both spellings agree, and agree with brute force'
