// c9.dart -- a PEG parser with built-in error recovery, in one file.
//
...
//
// WHAT c9 CHANGES OVER c8. Nothing in the judgment: over the whole
// mutation battery, c9 returns bit-identical trees to c8. The change is in
// the data the same algorithm runs on, worth 1.7x in latency (measured
// paired and interleaved on one warmed VM):
//
//   * Cells hold a plain LIST, not a map. The best-reading-per-end table
//     was a Map keyed by end position -- but the key always equaled the
//     stored reading's own `end`, and measured over the battery 71% of
//     consults see at most one reading (90% at most three), so a linear
//     scan by `end` beats hashing and drops the duplicate key.
//   * Views are CACHED BETWEEN CHANGES. A cell's filtered answer is
//     rebuilt only when a store lands or a (rare) tighter budget asks;
//     every other consult gets the same list object back. That identity
//     doubles as a change signal: a rule reference caches its own
//     labeled wrapping of the view and revalidates it by `identical`.
//   * Naming happens PER CHANGE, not per fill. Cells store raw readings;
//     the owner's name is wrapped around each one as the view is built.
//   * The plain parse's packaged answers (the zero-budget reading, its
//     one-element list, a reference's tree wrapper) are built at most
//     once per cell and reused.
//
...
class _RepairCell {
  ...
  /// and the one-element list it is served in ([plainList], built at most
...
abstract class Composite extends Clause {
  ...
  /// raw and labeled readings identically. Optional and the lookaheads
  ...
  List<_Reading> proposeReadings(Squirrel e, int pos) {
    ...
    for (var i = 0; i < subClauses.length; i++) {
      ...
      for (var j = 0; j < rs.length; j++) {
        final r = rs[j];
        if (r.preferred) anyPreferred = true;
      ...
      for (var i = 0; i < moved.length; i++) {
        ...
        for (var j = 0; j < steps.length; j++) {
          ...
          for (var k = 0; k < best.length; k++) {
            if (best[k].end == longer.end) {
              slot = k;
              break;
            }
          }
          if (slot >= 0) {
            if (Squirrel._compare(longer, best[slot], pos) >= 0) continue;
            best[slot] = longer;
            var seen = false;
...
class Optional extends Composite {
  ...
  List<_Reading> proposeReadings(Squirrel e, int pos) {
    ...
    for (var i = 0; i < rs.length; i++) {
      if (rs[i].preferred) anyPreferred = true;
    }
    final out = <_Reading>[
      _Reading.empty(pos)
          .withTree(lib.Match(source, pos, 0), anyPreferred ? _clean : _chosen)
    ];
    for (var i = 0; i < rs.length; i++) {
...
abstract class Terminal extends Clause {
  ...
  /// Slot value: null = not asked; _noMatch = asked, no match;
  /// a list = the matched reading.
  static const Object _noMatch = Object();
  List<Object?>? _hit;
  List<List<_Reading>?>? _miss;
  int _leafRun = -1;

  List<Object?> _leaves(Squirrel e) {
    if (_leafRun != e._runId) {
      _hit = List<Object?>.filled(e._len + 2, null);
      _miss = List<List<_Reading>?>.filled(e._len + 2, null);
      _leafRun = e._runId;
    }
    return _hit!;
  }
  ...
  _Reading? cleanReading(Squirrel e, int pos) {
    ...
    return identical(v, _noMatch) ? null : (v as List<_Reading>)[0];
  }
  ...
  List<_Reading> findReadings(Squirrel e, int pos) {
    ...
    if (!identical(v, _noMatch)) return v as List<_Reading>;
    ...
    // (see Str.recordMissing); [missingCacheable] excludes it.
    if (!missingCacheable) return recordMissing(e, pos);
    return _miss![pos] ??= recordMissing(e, pos);
  }

  bool get missingCacheable => true;
...
class Str extends Terminal {
  ...
  @override
  bool get missingCacheable => chars.isEmpty;
...
class Squirrel {
  ...
  List<_Reading> _grow(Composite clause, int pos) {
    final cell = clause.cells(this)[pos] ??= (_RepairCell(pos)..owner = clause);
    if (cell.inRecPath) {
    ...
    if (_budget == 0 && pos < _len) {
      ...
      final r = cell.plain;
      return r == null ? const [] : (cell.plainList ??= [r]);
    }
