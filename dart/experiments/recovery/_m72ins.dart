...
// _m72ins.dart -- m72 with _keepBest's ordered insert done in place.
...
class SuperDot3 {
  ...
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    ...
    out
      ...
      ..add(reg)
      ..add(why);
    // I30's order is the tie-break rule, so a new key has to land at `at` --
    // but `insertAll(at, [k, c, r, w])` ALLOCATES the literal every time, and
    // the battery's cells are short enough that most inserts are interior.
    // Grow by one slot-group and shift in place instead: same list, same
    // order, no garbage.
    for (var i = out.length - 5; i >= at; i--) {
      out[i + 4] = out[i];
    }
    if (at != out.length - 4) {
      out[at] = key;
