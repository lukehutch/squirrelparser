  ...
  List<_Way> _first(First c, int pos) {
    ...
    for (final s in c.subClauses) {
      ...
      for (final w in ws) {
        ...
        // the document, not by the budget: a later arm may still carry the parse
        // where it too reads cleanly, but it may not BUY the choice with a
        // repair. `[4,5,6]` is an Array because the input says so, whatever a
        // String could be made to say about the same characters.
    ...
    while (frontier.isNotEmpty) {
      ...
      for (final w in frontier) {
        ...
        for (final v in _ways(c.subClause, w.end)) {
          ...
          // NOTHING MARKS WHERE THE NEXT OCCURRENCE BEGINS BUT THE INPUT. A
          // sequence slot is placed by the slots around it, so a repair at its
          // start is located by the grammar; an occurrence has no such anchor,
          // and one that opens with a repair is asserting a boundary at a
          // position where the document offered no sign of one. `Character*`
          // reaching a `"` will supply the `\\` in front of it and read the
          // quote as an escape payload -- inventing an occurrence exactly where
          // the input said the repetition was over.
          if (v.fix == w.end) continue;
          here.add(v);
          grown.add(_Way(v.end, w.del + v.del, w.gap + v.gap, w.net + v.net,
              _min(w.fix, v.fix), w.peg && v.peg, v.node, w));
        }
        // AND WHERE NO OCCURRENCE CAN BEGIN AT ALL, the characters in front of
        // the next one are junk between items, not part of any of them. This is
        // the same resynchronization `_seq` already performs in front of a slot,
        // and it is offered on the same terms: only where the occurrence cannot
        // be read where it stands, and only as far as the nearest position where
        // it reads cleanly.
        if (here.isNotEmpty) continue;
        final room = _budget - w.del - w.gap;
        for (var k = w.end + 1; k <= w.end + room && k <= _in.length; k++) {
          final full = _budget;
          _budget = 0;
