...
class Squirrel {
  ...
  List<_Way> _prune(List<_Way> ws) {
    // THE ROUND AT BUDGET 0 IS THE FROZEN PARSER, SO IT SHOULD DO THE FROZEN
    // PARSER'S WORK -- not find PEG's answer among all the others.
    //
    // A clean way that is not PEG's reading is already dead when it is made.
    // [then] takes the MINIMUM key, so any chain holding one is at most `_far`,
    // and `recover` refuses exactly `_far`; making it part of an answer would
    // take a repair somewhere, and at budget 0 there is nothing to spend. So at
    // this budget a cell has ONE way -- PEG's -- or none, which is what a PEG
    // parser's memo entry holds. That deletes the whole cross product a
    // sequence of repetitions was exploring on input with nothing wrong with
    // it: `S <- W W W W W W W W` over clean text went from quadratic to linear.
    if (_budget == 0) {
      _Way? top;
      for (final w in ws) {
        if (!w.peg) continue;
