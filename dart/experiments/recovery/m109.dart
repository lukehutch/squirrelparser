// m109.dart -- m102 + `runs`: how SCATTERED the repairs are, as an
// additive key ranked below `site`. Replaces I65's `span`, which
// measured the same property but was not prefix-optimal.
//
// Generated from m102.dart by mk109.py.
//
// Generated from m100.dart by mkecho.py. See I64 in LESSONS_LEARNED.md.
//
// If taking `tail` out of the state is what the residual order dependence was,
// this file and m99 must now agree on every case.
//
// I63: THE ONE KEY THAT NEEDED STATE WAS THE ONE COUNTING RUNS.
//
// Five of the six keys are additive along an extension, and an additive key is
// prefix-optimal for free: tie on everything above it and the smaller sum stays
// smaller under any common suffix, so discarding the loser is justified. `site`
// was the exception. It counted maximal RUNS, so a repair closing the prefix and
// one opening the suffix had to be merged into one, and the merge read `w.tail`
// and `x.synth` -- fields the table does not key on. Two ways reaching the same
// position with different `tail` are therefore NOT interchangeable, and `_put`
// was discarding one of them anyway. That, and not any missing tie-break, is
// what left 3 of 1824 cases turning on linked-list order.
//
// The fix is to count what the engine actually does rather than what the
// characters look like afterwards. A skip of k contiguous characters is ONE
// `SyntaxError(pos, len: k)` -- already one event, already one position and one
// length, which is the whole of I33's argument. What the merge was folding
// together was a skip and a FILL that happen to touch, and those are two claims,
// not one: (position, length) for the discarded text and (position, spelling)
// for the asserted text. Describing them as one run does not make them one
// claim.
//
// So `site` counts repair events, the fold becomes `w.site + x.site`, and both
// `tail` and `_runs` are deleted -- a field out of the state, a function out of
// the engine, and a key that now has the same one-line soundness proof as
// `cost`, `blind` and `doubt`.
...
class _Way {
  _Way(this.end, this.cost, this.got, this.net, this.synth,
      {this.site = 0, this.runs = 0, this.tail = false,
      this.doubt = _never, this.echo = _never,
      this.blind = 0,
  ...
  /// HOW SCATTERED THE REPAIRS ARE -- the number of maximal RUNS of adjacent
  /// repair, where [site] is the number of repair events. Two events touching
  /// each other are one typo; two far apart are two mistakes, and a reader
  /// calls the second reading worse. Ranked directly below [site], so volume
  /// is settled first and this only separates readings that repair equally
  /// much.
  ///
  /// This is the key I65 tried to be. `span` = echo - doubt measured the same
  /// property from the outside and was not prefix-optimal; this is additive up
  /// to a correction of at most one, so a prune by it is never reversed by a
  /// suffix. See [_runs].
  final int runs;

  /// Whether this way ENDS in a repair, which is what makes [_runs] able to
  /// tell adjacency from separation without looking at positions.
  final bool tail;
...
_Way _cons(_Way w, _Way? next) => _Way(w.end, w.cost, w.got, w.net, w.synth,
    site: w.site, runs: w.runs, tail: w.tail,
    doubt: w.doubt, echo: w.echo, blind: w.blind,
...
bool _better(
        int cost, int net, int got, int blind, int site, int runs, int doubt,
        int echo, _Way? b) =>
    b == null ||
    cost < b.cost ||
    (cost == b.cost &&
...
/// Runs join where they touch: if [w] ends repairing and [x] opens repairing,
/// the two runs on either side of the join are one run, not two.
int _runs(_Way w, _Way x) => w.runs + x.runs - (w.tail && x.synth ? 1 : 0);

bool _beats(_Way a, _Way? b) =>
    _better(a.cost, a.net, a.got, a.blind, a.site, a.runs, a.doubt, a.echo, b);
    ...
    for (_budget = 0; _budget <= cap; _budget = _budget == 0 ? 1 : _budget * 2) {
      ...
      for (var e = _element(top, 0); e != null; e = e.next) {
        ...
        final cand = _Way(s.length, e.cost + trail, e.got, e.net, e.synth,
            ...
            runs: e.runs + (trail > 0 && !e.tail ? 1 : 0),
            tail: trail > 0 || e.tail,
    ...
    switch (c) {
      ...
      case FollowedBy f:
        ...
        return _Way(pos, w.length, 0, 0, true,
            doubt: pos, echo: pos, leaf: Filled(f, pos, w));
  ...
  _Ways _wrap(Clause c, int pos, _Ways w) {
    ...
    for (var e = w; e != null; e = e.next) {
      out = _Way(e.end, e.cost, e.got, e.net, e.synth,
          site: e.site, runs: e.runs, tail: e.tail,
          doubt: e.doubt, echo: e.echo, blind: e.blind,
      ...
      for (var w = ways; w != null; w = w.next) {
        for (var x = _element(sub, w.end); x != null; x = x.next) {
          ...
          if (!_better(cost, w.net + x.net, w.got + x.got,
              w.blind + x.blind, w.site + x.site, _runs(w, x),
              w.cost > 0 ? w.doubt : x.doubt,
              x.cost > 0 ? x.echo : w.echo, _at(next, x.end))) {
            continue;
          }
    ...
    for (var at = pos; at >= 0;) {
      ...
      for (var x = _element(r.subClause, at); x != null; x = x.next) {
        ...
        if (!_better(cost, w.net + x.net, w.got + x.got,
            w.blind + x.blind, w.site + x.site, _runs(w, x),
            w.cost > 0 ? w.doubt : x.doubt,
            x.cost > 0 ? x.echo : w.echo, _at(reach, x.end))) {
          continue;
        }
  ...
  _Ways _repair(Clause sub, int pos) {
    ...
    for (var k = 1; k <= _budget && pos + k <= _in.length; k++) {
      final skip = _Way(pos + k, 0, 0, 0, false,
          site: 1, runs: 1, tail: true,
          leaf: SyntaxError(pos: pos, len: k));
      ...
      out = _put(
          ...
          w != null
              ? _Way(pos, need, 0, 0, true,
                  site: 1, runs: 1, tail: true, doubt: pos, echo: pos,
                  leaf: Filled(sub, pos, w))
              : _Way(pos, need, 0, 0, true,
                  site: 1,
                  runs: 1,
                  tail: true,
