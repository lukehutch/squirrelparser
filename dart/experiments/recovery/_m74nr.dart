...
class SuperDot3 {
  ...
  int _skipRegret(int from, int to) => 0; // CONTROL: regret deleted
  ...
  int _cleanRegret(MatchResult m) {
    return 0; // CONTROL: regret deleted
    // ignore: dead_code
  ...
  bool _keepBest(List<int> out, int key, int cost, int reg, int why) {
    ...
    final at = lo << 2;
    out..add(0)..add(0)..add(0)..add(0);
    for (var i = out.length - 4; i > at; i -= 4) {
      out[i] = out[i - 4];
      out[i + 1] = out[i - 3];
      out[i + 2] = out[i - 2];
      out[i + 3] = out[i - 1];
  ...
  // node decides, not the number.
  static const int _wPure = -1; // the oracle's own match settles this cell
  static const int _wDemand = -2; // a lookahead obligation, no text at all
  static const int _wSub = -3; // spend the characters under `key` on this leaf
  static const int _wFab = -4; // write this leaf's spelling, reading nothing
  static const int _wStop = -5; // the repetition declined to go round again
  static const int _wNone = -6; // never written: the chase has left the table
          ...
          if (silent || _permits(c, emits)) {
            ...
            if (pos < _inputLen) {
              _put(f, _key(pos + 1, owed), 1,
                  2 * _skipRegret(pos, pos + 1), _wSub); // SUB
            }
            _put(f, _key(pos, owed), 1, 0, _wFab); // FAB
          }
        }
        return _finish(f);
  ...
  // tree and threw it away. Two machines, one answer -- and the search was the
  // expensive half (27.6 ms of the certificate's 35.4 on the battery, against
  // 7.8 for emit plus the whole re-parse).
  //
  ...
  int _reasonAt(_Node node, int pos, int c, int key) {
    final v = _entryAt(node, pos, c).value;
    if (v != null) {
      for (var i = 0; i < v.length; i += 4) {
  ...
  bool _chase(int cost) {
    _edits.clear();
    final st = <(_Node, int, int, int)>[(_goal, 0, _free, _goalKey)];
    while (st.isNotEmpty) {
      final (node, pos, c, key) = st.removeLast();
      final why = _reasonAt(node, pos, c, key);
      switch (why) {
        case _wNone:
          return false;
        case _wSub:
          _edits.add((pos, _endOf(key) - pos, _spelling(node.orig), node.orig));
        case _wFab:
          _edits.add((pos, 0, _spelling(node.orig), node.orig));
        case _wPure || _wDemand || _wStop:
          break; // edit-free: the oracle already reads this whole stretch
        default:
          switch (node) {
            case _Alt(:final alts):
              final guards = _guarded ? _guardsOf(node) : null;
              st.add((alts[why], pos,
                  guards == null ? c : _meet(c, guards[why]), key));
            case _Cons():
              // Tail first so the head pops first: left to right.
              st.add((node.tail, _endOf(why), _oweOf(why), key));
              st.add((node.head, pos, c, why));
            case _Term():
  ...
  String _repaired() {
    ...
    for (final (pos, drop, text, _) in _edits) {
      if (pos > cursor) out.write(_input.substring(cursor, pos));
      out.write(text);
      cursor = pos + drop;
    }
  ...
  /// A position in the repaired string, read back as one in the input. The map
  /// is monotone and onto, so a tree re-indexed through it still tiles the
  /// input exactly: adjacent leaves in y meet at one q, so they meet at one x,
  /// and the characters an edit deleted are absorbed by the leaf ending there.
  /// A boundary belongs to the leaf on its LEFT -- which is the one that has to
  /// grow, since the deleted characters precede it in the input. Position 0 is
  /// the sole boundary with nothing to its left, so there the deletion is
  /// absorbed rightwards instead; without that the root would start past 0 and
  /// stop covering its own input.
  int _xOf(int q) {
    if (q == 0) return 0;
    var xc = 0, yc = 0;
    for (final (pos, drop, text, _) in _edits) {
      if (q < yc + (pos - xc)) return xc + (q - yc);
      yc += pos - xc;
      if (q < yc + text.length) return pos;
      yc += text.length;
      xc = pos + drop;
    }
    return xc + (q - yc);
  ...
  /// deep as its input on a left-recursive grammar, so recursing here is what
  /// held the ladder at 2048 -- an explicit post-order walk keeps the witness
  /// off the VM stack, and the chase already gave up its own recursion.
  MatchResult _reindex(MatchResult root) {
    final nodes = <MatchResult>[root];
    final next = <int>[0];
    final built = <List<MatchResult>>[[]];
    while (true) {
      final d = nodes.length - 1;
      final m = nodes[d];
      final subs = m.subClauseMatches;
      if (next[d] < subs.length) {
        nodes.add(subs[next[d]++]);
        next.add(0);
        built.add(<MatchResult>[]);
        continue;
      }
      final s = _xOf(m.pos), e = _xOf(m.pos + m.len);
      final kids = built[d];
      final out = kids.isEmpty
          ? Match(m.clause, s, e - s)
          : Match(m.clause, s, e - s, subClauseMatches: kids);
      if (d == 0) return out;
      nodes.removeLast();
      next.removeLast();
      built.removeLast();
  ...
  SkipResult recover(String input) {
    ...
    // The edits already ARE the report: a repair that deletes input is a span
    // over what it deleted, and one that deletes nothing is an obligation the
    // input never met. The chase emits them in position order.
    final spans = <SyntaxError>[];
    final missing = <MissingObligation>[];
    for (final (pos, drop, _, what) in _edits) {
      if (drop > 0) {
        spans.add(SyntaxError(pos: pos, len: drop));
      } else {
