// _holes76.dart -- where does m76's ONE-CHARACTER obligation actually break?
//
// m76 carries a character class as its forward obligation. Three separate
// mechanisms produce one:
//   `_notFirst`  (m76.dart:260) -- taking branch k means every earlier branch
...
//   `_looks`     (m76.dart:292) -- `&P` / `!P`; same approximation, same drop.
...
void main() {
  ...
  for (final (g, top, alpha, what) in probes) {
    ...
    final engine = e76.SuperDot3(rules: r, topRuleName: top);
    ...
    for (final s in strings) {
      ...
      if (!ok) {
        ...
        if (examples.length < 3) {
          examples.add('"$s" true=${truth ?? ">3"} m76=$c');
        }
      }
    }
  ...
  stdout.writeln('m76 wrong / checked, by which obligation the grammar '
      'stresses\n');
