...
class Squirrel {
  ...
  final List<(Clause, int, int, bool)> _frontier = [];
  ...
  bool _leaf(Clause c, int pos) {
    ...
    // THE SHALLOW SIDE -- see [_r7c]; here offered on the grammar side only.
    final here = _add(c, pos, deep: deep);
    return deep || here;
  }
  ...
  MatchResult? _round(MatchResult root, (int, int) base) {
    ...
    for (var k = 1; k <= _in.length; k++) {
      ...
      for (final (c, p, fill, leaf) in _frontier) {
        ...
        // Only a leaf may do this: an ancestor stands at the same position as
        // its deepest stuck descendant, so the denial is already offered there.
        if (!leaf || p + k > _in.length) continue;
