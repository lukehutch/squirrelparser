...
class _Cell {
  ...
  /// I14: the largest LOCAL Delta this cell has already ANNOUNCED to its
  /// readers. A fact is settled once the watermark reaches `g + Delta(local)`;
  /// facts in `(delivered, watermark - g]` are the ones a pop announces, so a
  /// fact is announced exactly once, at its own priority.
  int delivered = -1;

  /// Scheduled for (re)relaxation at the next pop of one of this cell's queue
  /// entries: set at creation (the cell's first expansion runs at priority `g`,
  /// which is what keeps a frontier cell a leaf) and by every wake.
  ...
  ///
  /// A LIST, NOT A SET: one slot per grammar edge (I11's transpose) means a
  /// duplicate reverse edge cannot be constructed, and under I14 a wake is issued
  /// once per fact settled, gated by `needsRelax`, so nothing walks this list
  /// more than the announcements require.
...
class SuperDot3 {
  ...
  void _put(int key, int delta) {
    if (_keepBest(_out, key, delta)) _push(_cur.g + delta, _cur);
  }
  ...
  int dbgRegress = 0, dbgBelow = 0;
  bool dbgTrace = false;
  void _push(int prio, _Cell cell) {
    if (prio < _w) {
      dbgBelow++;
      if (dbgTrace) {
        print('BELOW push prio=$prio w=$_w g=${cell.g} '
            'node=${cell.node.runtimeType}#${cell.node.id} pos=${cell.pos} c=${cell.c} '
            'orig=${cell.node.orig}');
      }
    }
    if (dbgTrace) {
      print('  push p=$prio ${cell.node.runtimeType}#${cell.node.id}@${cell.pos}/${cell.c} g=${cell.g} ${cell.node.orig}');
    }
    var i = _qp.length;
    _qp.add(prio);
    _qc.add(cell);
      ...
      _cells.putIfAbsent(_addr(node, pos, c), () {
        ...
        if (node.noLook) {
          ...
          if (!m.isMismatch) {
            ...
            if (v.isNotEmpty) _push(g + v[1], cell);
          }
        }
        cell.needsRelax = true;
        _push(g, cell);
        return cell;
      });
  ...
  int recoverCost(String input) {
    ...
    while (_qp.isNotEmpty) {
      final cell = _popMin();
      if (dbgTrace) {
        print('POP  p=$_w ${cell.node.runtimeType}#${cell.node.id}@${cell.pos}/${cell.c} g=${cell.g} relax=${cell.needsRelax} v=${cell.value}');
      }
      if (cell.needsRelax) {
        cell.needsRelax = false;
        _relax(cell);
      }
      // ANNOUNCE: facts whose priority the watermark has just reached are final.
      // Deliver each exactly once -- readers re-relax and pick them up -- and
      // read the goal's own settlements for the answer.
      final avail = _w - cell.g;
      final v = cell.value;
      if (v == null) continue;
      var fresh = false;
      for (var i = 1; i < v.length; i += 2) {
        if (v[i] > cell.delivered && v[i] <= avail) {
          fresh = true;
          break;
        }
      }
      if (fresh && identical(cell, goalCell)) {
        var best = _impossible;
      ...
      if (avail > cell.delivered) cell.delivered = avail;
      if (fresh) {
        for (final reader in cell.readers) {
          if (!reader.needsRelax) {
            reader.needsRelax = true;
            _push(_w, reader);
          }
        }
      }
