// m29 -- m26 with two changes, each of which REMOVES a mechanism.
//
//   1. A3 is made literally true: the budget stops being an argument that
//      varies down the recursion, so it stops being a memo key by accident.
//   2. Ordered choice, greedy `?` and possessive `*` collapse into ONE
//      predicate, `_mayFall`, because all three are negative lookahead.
//
...
class SuperDot3 {
  ...
  Map<int, int> _compute(Clause c, int dot, int pos, int b) {
    ...
    if (c is First || c is Optional) {
      ...
      for (final sub in _alts(c)) {
        ...
        if (!_mayFall(sub, pos)) return out; // committed: no later option
      }
      // The empty branch of `?` is just its last alternative, so it is reached
      // by falling off the end of the same loop.
  ...
  /// PEG COMMITS, and it commits in three places that look like three features:
  ///
  ///   e1 / e2   is  e1 / (!e1 e2)      -- ordered choice
  ///   e?        is  e  / (!e)          -- greedy option
  ///   e*        stops exactly where !e -- possessive repetition
  ///
  /// Every one of them says the same thing: the LATER option is reachable only
  /// where the EARLIER one fails. That is a negative lookahead, so all three need
  /// one predicate rather than three rules, and `First`, `Optional` and
  /// `Repetition` below are one loop rather than three cases.
  ///
  /// A zero-width match counts as a failure for the same reason the pure parser
  /// treats it as one: it cannot make progress.
  ...
  Map<int, int> _chain(Clause c, int dot, int pos, int b) {
    final out = <int, int>{};
    final limit = (b + 1) * _M;
    if (_done(c, dot) && (c is! Repetition || _mayFall(_elem(c, dot), pos))) {
      out[pos] = 0;
    }
    ...
    for (final h in _ends(sub, 0, pos, b).entries) {
      ...
      for (final t in _ends(c, to, h.key, b).entries) {
    ...
    if (pos < _n) {
      ...
      for (final t in _ends(c, dot, pos + 1, b).entries) {
