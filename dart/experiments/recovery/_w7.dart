...
class Squirrel {
  ...
  static int _rank(_Way a, _Way b) {
    ...
    // AND ONLY THEN: the reading that is not still holding an obligation where
    // it stopped. The top of the loop refuses a way that owes at its end with
    // input still in front of it, but it can only refuse what the chart kept,
    // and one way survives per end position -- so where two are otherwise
    // indistinguishable, keep the one the top can still use.
    return a.owing == b.owing ? 0 : (a.owing ? 1 : -1);
  }
