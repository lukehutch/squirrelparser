...
class _Cell {
  ...
  /// I11 GENERALISED: the oracle's verdict at this cell, asked once. -2 for "not
  /// asked yet"; -1 is the oracle's own "nowhere", which is a real answer.
  int committed = -2;
}
  ...
  void _compute(_Cell cell) {
    ...
    switch (node) {
      ...
      case _Alt(:final alts):
        ...
        var committed = cell.committed;
        if (committed == -2) {
          final oracle = _parser.match(node.orig, pos);
          committed = cell.committed = oracle.isMismatch ? -1 : pos + oracle.len;
        }
