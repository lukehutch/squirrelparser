...
class Squirrel {
  ...
  static List<_Way> _prune(List<_Way> ws) {
    ...
    final best = <(int, bool), _Way>{};
    var far = -1;
    for (final w in ws) {
      if (w.peg && w.end > far) far = w.end;
    }
    for (final w in ws) {
      // A free chart derivation PEG did not choose is usable only after some
      // enclosing repair. It cannot dominate an already-repaired way to the
      // same end, because the root must reject the former and may accept the
      // latter.
      final owesRepair = w.free && !(w.peg && w.end == far);
      final key = (w.end, owesRepair);
      final b = best[key];
      if (b == null || _rank(w, b) < 0) best[key] = w;
    }
    final out = [
  ...
  MatchResult recover(String s) {
    ...
    for (_budget = 0; _budget <= ceiling; _budget++) {
      for (final w in _ways(rules[topRuleName]!, 0)) {
        ...
        if (tail == 0 && w.free && !w.peg) continue;
