// m34 -- m33 taken to its limit: the budget is either 0 or the round budget.
//
// If coarsening the budget is sound (see m33), the coarsest sound choice is a
// SINGLE bit: an entry is asked either at budget 0 -- the complete, cheap, local
// level-0 value that cuts the descent -- or at the full round budget. Every entry
// is then computed at most TWICE per round no matter how large K is, so the budget
// stops being a memo key in every sense that matters, and A3 becomes true with one
// line rather than by deleting the mechanism that made m26 fast.
//
...
class SuperDot3 {
  ...
  /// Either nothing left to spend, or everything. The coarsest sound rounding, so
  /// an entry is computed at most twice per round however large K is.
  int _up(int x) => x <= 0 ? x : _b;
