// Scratch: m48 against m46 on JSON -- a grammar with NO lookahead anywhere.
//
...
void main() {
  ...
  for (final s in cases) {
    ...
    final e47 = g48.SuperDot3(rules: rules, topRuleName: 'JSON');
    ...
    try {
      ...
      print('m48 THREW on "$s": $e');
      rethrow;
    }
    if (cost46 != cost47) {
      costDiff++;
      if (costDiff <= 8) print('COST  "$s"  m46=$cost46 m48=$cost47');
    }
    if (spans46 != spans47) {
      spanDiff++;
      if (spanDiff <= 8) print('SPANS "$s"  m46=$spans46 m48=$spans47');
    }
    // The strongest form of the zero-cost claim: not merely the same answer, the
    // same number of `_compute` calls to reach it.
    if (steps46 != steps47) {
      stepDiff++;
      if (stepDiff <= 8) print('STEPS "$s"  m46=$steps46 m48=$steps47');
    }
