// r5.dart -- r4 with its two questions about a reading merged into one number,
...
class Squirrel {
  ...
  /// [0, length], so a clause's cells are an array and not a dictionary: the
  /// inner lookup is an index rather than a hash, and the row is built once for
  /// each clause the parse actually reaches rather than once per cell.
  ...
  static List<_Way> _prune(List<_Way> ws) {
    ...
    // TWO WAYS NEED NO INDEX. Measured over the battery, a call with anything
    // to decide has two ways more often than every larger size put together,
    // and for two the map, the list it is copied into and the sort are all one
    // comparison: they reach the same end, or the nearer PEG way of the two is
    // the one that is not PEG's.
    if (ws.length == 2) {
      var a = ws[0], b = ws[1];
  ...
  List<_Way> _ways(Clause c, int pos) {
    ...
    // AND A REFERENCE NEEDS NO PRUNE. The rule body's own ways are already one
    // per end and already in rank order, [_lift] neither reorders them nor
    // merges two into one end, and it cannot drop the farthest PEG way because
    // a PEG way is free and free ways it keeps -- so every decision [_prune]
    // would make here has already been made. Measured: 2,354,118 calls, every
    // one of them returning its input unchanged.
  ...
  ///
  /// [_rank] orders by total cost first, so a cell's ways come out sorted by
  /// what they cost: the budget question and the ranking question are answered
  /// by the same order, and "which of these can I afford" is not a filter but a
  /// cut point. Where the last way is affordable there is no cut and no list to
  /// build -- the answer IS the argument. Measured over the battery: the input
  /// was cost-sorted on all 2,377,870 calls, and 91.1% of them cut nothing.
  ...
  List<_Way> _seq(Seq c, int pos) {
    ...
    for (var i = 0; i < c.subClauses.length; i++) {
      ...
      for (final w in cur) {
        ...
        // prefixes arrive in cost order, so the first one that overspends ends
        // the slot -- every prefix behind it costs at least as much.
  ...
  List<_Way> _rep(Repetition c, int pos) {
    ...
    // A WORKLIST, NOT A SERIES OF ROUNDS. Each step strictly advances, so the
    ...
    // way it already extended only rebuilds what the enclosing prune throws
    // away. What is expanded is exactly what improved.
    ...
    // AND CHOOSE BEFORE BUILDING. Two chains of different lengths can finish at
    // the same position, so `all` holds ways an enclosing prune will discard --
    // 35.1% of them, measured -- and closing a chain is the most expensive
    // thing here: it walks the whole chain and collects its children. Ranking
    // reads none of that, so the choice can be made first and the tree built
    // only for what survives it.
    return [for (final w in _prune(all)) w.over(_close(c, pos, w))];
  }
