...
class Squirrel {
  ...
  /// Every structural claim the design makes about a way, checked against the
  /// tree that way heads. A `_Way` is supposed to be a MEMO of four aggregates
  /// over its own tree; if that is true, the incremental maintenance and the
  /// tree can never disagree, and the field set is exactly the aggregates.
  ...
  void _check(Clause c, int pos, List<_Way> ws) {
    for (final w in ws) {
      _note('ways');
      if (w.peg && !w.free) _note('BROKEN peg-implies-free');
      if (w.free != (w.fix == _far)) _note('BROKEN free-iff-far');
      final n = w.node;
      if (n == null) continue;
      final (d, g) = _edits(n);
      if (d != w.del) _note('BROKEN del-is-tree-aggregate');
      if (g != w.gap) {
        _note('BROKEN gap-is-tree-aggregate');
        _note('  way.gap - tree.gap = ${w.gap - g}');
        _note('  clause ${c.runtimeType}');
      }
      var first = _far;
      ...
      walk(n);
      if (w.fix < _far && w.fix != first) _note('BROKEN fix-is-earliest-error');
      if (w.fix >= _far && first < _far) _note('BROKEN far-but-tree-has-errors');
      // The span identity: every character in [pos, end) is denied, pinned, or
      // merely consumed. `raw` is not stored because this makes it derivable.
      final raw = (w.end - pos) - w.del - w.net;
      if (raw < 0) _note('BROKEN span-identity-negative-raw');
    }
  }
      ...
      for (final w in cur) {
        ...
        if (i == 0) {
          for (var k = pos + 1; k <= pos + _budget && k <= _in.length; k++) {
            ...
            var chain = <_Way>[_Way(k, k - pos, 0, 0, pos, false, null, w,
                SyntaxError(pos: pos, len: k - pos))];
            for (final s in c.subClauses) {
              final step = <_Way>[];
              for (final x in chain) {
                for (final v in _ways(s, x.end)) {
                  if (!v.free) continue;
                  step.add(_Way(v.end, x.del, x.gap, x.net + v.net, x.fix,
                      false, v.node, x));
                }
              }
              chain = _prune(step);
...
// ---------------------------------------------------------------------------
// Driver: run the whole battery and report which structural claims survive.

void main() {
  for (final c in corpora) {
    final rules = MetaGrammar.parseGrammar(c.grammar);
    final sq = Squirrel(rules: rules, topRuleName: c.top);
    for (final k in weighted(buildBattery())) {
      if (k.grammar != c.name) continue;
      try {
        sq.recover(k.mutant);
      } catch (_) {}
    }
  }
  final keys = Squirrel.viol.keys.toList()..sort();
  for (final k in keys) {
    print('${k.padRight(34)} ${Squirrel.viol[k]}');
  }
}
